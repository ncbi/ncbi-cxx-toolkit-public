...
sub markupfile {
    ...
    # A C/C++ file 
    if ($fname =~ /\.([ch]|cpp?|cc|hpp|inl)$/i) { # Duplicated in genxref.
...
sub init {
    ...
    $HTTP->{'path_info'} = &http_wash($tmp);
	#print STDERR "path_info is $HTTP->{'path_info'}\n";
...
sub titleexpand {
	...
	my $i = $HTTP->{'param'}->{'i'};
	return('NCBI '.$Conf->sourcerootname.' Toolkit Identfier Search'.
	       ($i ? " \"$i\"" : ''));

    } elsif ($who eq 'search') {
	my $s = $HTTP->{'param'}->{'string'};
	return('NCBI '.$Conf->sourcerootname.' Toolkit Freetext Search'.
	       ($s ? " \"$s\"" : ''));

    } elsif ($who eq 'find') {
	my $s = $HTTP->{'param'}->{'string'};
	return('NCBI '.$Conf->sourcerootname.' Toolkit File Search'.
	       ($s ? " \"$s\"" : ''));
    }
}
