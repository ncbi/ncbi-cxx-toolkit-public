...
tds_put_n(TDSSOCKET *tds, const unsigned char *buf, int n)
{
	...
	while(n > 0){
		...
		to_copy = (n < left)?n:left;
		if(buf){
			memcpy(tds->out_buf+tds->out_pos,buf,to_copy);
			buf+=to_copy;
		} else {
			memset(tds->out_buf+tds->out_pos,'\0',to_copy);
		}
		tds->out_pos+=to_copy;
		n-=to_copy;
...
void (*oldsig)(int);
	...
	tds->out_buf[1]=final;
#ifdef NCBI_FTDS
	tds->out_buf[2]=(tds->out_pos) >> 8;
	tds->out_buf[3]=(tds->out_pos) & 0xFF;
#else
