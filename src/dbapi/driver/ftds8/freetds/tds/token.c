...
#ifdef NCBI_FTDS
             break; /* <-- looks like it should be here (v. soussov) */
#endif
      			case TDS_RET_STAT_TOKEN:
				tds->has_status=1;
				tds->ret_status=tds_get_int(tds);
...
#ifdef NCBI_FTDS
             tds_get_n(tds,curcol->full_column_name,tabnamesize);
             curcol->full_column_name[tabnamesize]='.';
             strcpy(curcol->full_column_name+tabnamesize+1,curcol->column_name);
#else             
...
#ifdef NCBI_FTDS
 	memset(info->current_row,'\0',info->null_info_size); /*** clear nulls ***/
#endif
...
{
int size, type,env_left;
char oldval[1024], newval[1024];
int  new_block_size;
TDSENVINFO *env = tds->env;

	env_left = tds_get_smallint(tds);
	...
	*/
	...
	switch(type){
	 case TDS_ENV_BLOCKSIZE:
		...
		if(size) tds_get_string(tds,newval,size);
		newval[size]='\0';

		/* fetch the old value */
		size = tds_get_byte(tds);
		if(size) tds_get_string(tds,oldval,size);
		oldval[size]='\0';

		new_block_size = atoi(newval);
		if (new_block_size > env->block_size) {
			tdsdump_log(TDS_DBG_INFO1, "%L increasing block size from %d to %d\n",
   ...
   case TDS_ENV_CHARSET: 
       tds_get_n(tds,NULL,env_left);
		break;
	 default:
		tdsdump_log(TDS_DBG_INFO1, "%L unknown TDS_ENV type %d, skipping %d bytes block\n", type, env_left);
		tds_get_n(tds,NULL,env_left);
		break;
	}
	return TDS_SUCCEED;
...
*/
#ifdef NCBI_FTDS
static unsigned char s_null_mask[8]= {0x1,0x2,0x4,0x8,0x10,0x20,0x40,0x80};
void tds_set_null(unsigned char *current_row, int column)
{
    current_row[column >> 3]|= s_null_mask[column & 0x7];
}
