...
if echo $FEATURES | grep "\-connext" > /dev/null ; then
    server_list="MSDEV1 MS2008DEV1 DBAPI_DEV3"
    server_mssql="MSDEV1"

    server_mssql2008="MS2008DEV1"
else
    # server_list="MS_DEV2"
    server_list="DBAPI_MS2008_TEST DBAPI_MS2014_TEST DBAPI_SYB155_TEST"
    # server_mssql="MS_DEV2"
    server_mssql="DBAPI_MS2014_TEST"

    server_mssql2008="DBAPI_MS2008_TEST"
fi
            ...
            case "$v_flag" in
              -v\ 7?)
                if test $server != $server_mssql \
                     -a $server != $server_mssql2008; then
                    continue
                fi
                ;;
            esac

            if test \( $driver = "ctlib" -o $driver = "dblib" \)  -a  \( $server = $server_mssql -o $server = $server_mssql2008 -o \( $static_config = 1 -a $win_config = 1 \) \) ; then
                continue
            fi

            if test $driver = "odbc" -a  $server != $server_mssql -a  $server != $server_mssql2008 ; then
                continue
            fi
...
EOF
        ...
        # lang_query
            ...
            if test $driver != 'dblib' -o \( $driver = 'dblib' -a $server != $server_mssql -a $server != $server_mssql2008 \) ; then
        ...
        # simple tests
            ...
            if test $driver = "dblib"  -a  \( $server = $server_mssql -o $server = $server_mssql2008 \) ; then
                continue
            fi
            ...
            elif test \( $driver = "ftds64" -o $driver = "ftds95" \) -a  $server != $server_mssql -a  $server != $server_mssql2008 ; then
