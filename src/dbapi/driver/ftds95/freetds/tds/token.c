...
tds_process_default_tokens(TDSSOCKET * tds, int marker)
{
	...
	case TDS_RETURNSTATUS_TOKEN:
        ...
        if (marker != TDS_PARAM_TOKEN  &&  marker != TDS_DONEPROC_TOKEN
            &&  marker != TDS_DONE_TOKEN  &&  marker != TDS5_PARAMFMT2_TOKEN)
			break;
		tds->has_status = 1;
...
tds5_process_dyn_result2(TDSSOCKET * tds)
{
	...
	for (col = 0; col < info->num_cols; col++) {
         ...
         * query (prepared statement) declarations with IMAGE or (N)TEXT
         * parameters.  However, subsequent attempts to instantiate
         * these queries have been failing with message 3805, "The token
         * datastream length was not correct."  In such cases, switch on
         * dynamic query emulation (as already needed for older Sybase
         * versions that immediately reject these declarations) and
         * explicitly discard column information to avoid misconstruing
         * the status of subsequent queries that yield no row results.
         */
        if (dyn != NULL  &&  !TDS_IS_MSSQL(tds)  &&  is_blob_col(curcol)) {
            dyn->emulated = 1;
            tds_dynamic_deallocated(tds->conn, tds->cur_dyn);
            dyn = NULL;
        }
