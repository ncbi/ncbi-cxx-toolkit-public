	...
	if (con->locale) {
		if (con->locale->charset) {
            if (!tds_dstr_copy(&login->server_charset, con->locale->charset)
                ||  !tds_dstr_copy(&login->client_charset, con->locale->charset))
				goto Cleanup;
		}
...
ct_param(CS_COMMAND * cmd, CS_DATAFMT * datafmt, CS_VOID * data, CS_INT datalen, CS_SMALLINT indicator)
{
	...
	case CS_RPC_CMD:
		if (cmd->rpc == NULL) {
            tdsdump_log(TDS_DBG_ERROR, "RPC is NULL ct_param\n");
			return CS_FAIL;
		}
...
paraminfoalloc(TDSSOCKET * tds, CS_PARAM * first_param)
{
	...
	for (i = 0, p = first_param; p != NULL; p = p->next, i++) {
        ...
        if (temp_type == CS_VARCHAR_TYPE || temp_type == CS_NVARCHAR_TYPE ||
		...
		if (pcol->column_varint_size) {
            if ((pcol->column_varint_size == 2  &&  *(p->datalen) > 8000)
                ||  (pcol->column_varint_size == 1  &&  *(p->datalen) > 255)) {
			...
			if (p->maxlen < 0) {
				...
				return NULL;
            } else if (p->maxlen == 0  &&  is_unicode_type(tds_type)) {
                /* Auto-detect; account for possible conversion to UCS-2. */
