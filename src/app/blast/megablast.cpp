...
/** @file megablast.cpp
 * Search one or more nucleotide query sequences against a database 
...
class CMegablast : public CNcbiApplication
{
private:
    virtual void Init();
    virtual int Run();
    ...
    CRef<CMegablastArgs> m_ArgDesc;
};


void CMegablast::Init()
{
    ...
    m_ArgDesc.Reset(new CMegablastArgs());
...
int CMegablast::Run(void)
{
    ...
    BlastSeqSrc* seq_src = SeqDbBlastSeqSrcInit(m_ArgDesc->GetDatabase(args),
                                                false);
    ...
    if (m_ArgDesc->GetOutputType(args) != 2) {
        NCBI_THROW(CBlastException, eInvalidArgument, 
                   "only -D 2 is supported");
    }

    CBlastFormat format(string("megablast"),
                        m_ArgDesc->GetDatabase(args),
...
{
    return CMegablast().AppMain(argc, argv, 0, eDS_Default, 0);
}
#endif /* SKIP_DOXYGEN_PROCESSING */
