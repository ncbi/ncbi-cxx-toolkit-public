 ...
 * IGBLASTN command line application experimental multithreaded version
 */
...
printf("XIGB: ig_opts->m_IsProtein: %s\n",(ig_opts->m_IsProtein?"TRUE":"FALSE"));
	...
	CFormattingArgs::EOutputFormat fmt_output_choice = fmt_args->GetFormattedOutputChoice();
        if(fmt_output_choice == CFormattingArgs::eFlatQueryAnchoredIdentities || fmt_output_choice == CFormattingArgs::eFlatQueryAnchoredNoIdentities){
	...
	// it will be pre-allocated vector, to preserve order between threads/batches
	list< CRef<CSearchResultSet> > all_result_sets;
	list< CRef<CBlastQueryVector> > all_queries_list;
        /*** Process the input ***/
// ********************************************************************************************
// ZONE A
// ********************************************************************************************
	    ...
	    all_result_sets.push_back( results ); // ADD
	    all_queries_list.push_back( query );  // ADD
        }
...
// ZONE B.START
// ********************************************************************************************
	list< CRef<CSearchResultSet> >::iterator  results_it;
	list< CRef<CBlastQueryVector> >::iterator queries_it;
	for( results_it = all_result_sets.begin(), queries_it = all_queries_list.begin(); results_it != all_result_sets.end(); results_it++, queries_it ++)
	{
            CRef<CSearchResultSet> results;
	    CRef<CBlastQueryVector> query;

	    // SOME CODE TO ADD QUERIES TO SCOPE ( OPPOSITE, WHAT formatter.ResetScopeHistory() IS DOING

	    if( queries_it == all_queries_list.end() ) {
		fprintf(stderr,"EMPTY QUERIES  SET\n"); exit(0);
	    }

	    results.Reset(      *results_it );
	    query.Reset(  *queries_it );

            BlastFormatter_PreFetchSequenceData(*results, scope, fmt_output_choice );

            //ITERATE(CSearchResultSet, result, *results) 
	    //  vector< CRef<CSearchResults> > m_Results;
	    //  CSearchResultSet::const_iterator result
            for( vector< CRef<CSearchResults> >::const_iterator result = results->begin(); result != results->end(); result ++) { //ITERATE(CSearchResultSet, result, *results) 
                ...
                formatter.PrintOneResultSet(ig_result, query, clone_info, true /* not protein */ ) ; 
                total_input ++;
...
//--END DEFFERED RESULTS PROCESSING ========================================================
