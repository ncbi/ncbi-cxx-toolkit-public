...
static CRef<COrg_ref> PerformLookup(const COrg_ref& org_ref, const string& taxname)
{
    ...
    if (taxon1.IsAlive() || taxon1.Init()) {
        ...
        ERR_POST_EX(0, 0, Fatal << "Taxonomy lookup failed for " << taxname << ". Error: [" << taxon1.GetLastError() << "]. Cannot generate ASN.1 for this entry.");
    }
...
void LookupCommonOrgRefs(list<COrgRefInfo>& org_refs)
{
    ...
    if (taxon1.IsAlive() || taxon1.Init()) {
        ...
        ERR_POST_EX(0, 0, Fatal << "Taxonomy lookup failed to initialize. Error: [" << taxon1.GetLastError() << "]. Cannot proceed.");
    }
}
