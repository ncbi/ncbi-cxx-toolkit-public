    ...
    sequenceViewer(parentSequenceViewer), taxonomyTree(NULL)
{
    ...
    editMenu->Append(MID_SQ_REFINER, "Test Refiner...");
...
//   Using block aligner handler as a template...
void SequenceViewerWindow::OnRefineAlignment(wxCommandEvent& event)
{
    BMARefiner* refiner = (sequenceViewer) ? sequenceViewer->alignmentManager->bmaRefiner : NULL; 

    //  need any of this business???
//    CancelAllSpecialModesExcept(MID_SQ_REFINER);
//    if (DoRefineAlignment())
//        SetCursor(*wxCROSS_CURSOR);
//    else
//        RefineAlignmentOff();
    ...
    if (!refiner) {
        ERRORMSG("SequenceViewerWindow::OnRefineAlignment() - no refiner object!");
        return;
    }

    BlockMultipleAlignment *refinedAlignment = NULL;
    BlockMultipleAlignment *alignment = sequenceViewer->GetCurrentAlignments().front();
    ...
    //struct_util::AlignmentUtility* au = new struct_util::AlignmentUtility(cdCRef->GetSequences(), cdCRef->GetSeqannot());

    //  Define structures for refiner parameters, pass them to the refiner and execute.
    align_refine::BlockEditingParams beParams;
    BMARefiner::Cn3DBMAList refinedAlignments;

    /*  Current CJL hack:  Needs real indoor plumbing. */
    bool refinerResult = refiner->RefineMultipleAlignment(alignment, &refinedAlignments, NULL, sequenceViewer->alignmentManager->structureSet_Hack);

    //  Grab the refiner object -- will need to do different things if block structure has been edited.
    const align_refine::CBMARefinerEngine* refEngine = refiner->GetRefiner();

    if (refinerResult && refinedAlignments.size() > 0) { //refEngine && refEngine->GetBestRefinedAlignment()) {
        refinedAlignment = refinedAlignments.front();
        
        //  If we allowed block structure to change...
        if (refEngine->GetBEParams(beParams) && beParams.editBlocks) {

            /*  Current CJL non-hack:  Needs real indoor plumbing. */
            //  need to re-adjust to refined block structure somehow ... or globally change the bma 

        //  Block structure remains the same --> install the new alignment 
        } else {  

            bool mergeSucceeded, deleteSucceeded = true;
            int firstDeleteFailure = -1, nRows = alignment->NRows();
            BlockMultipleAlignment *clone = alignment->Clone();

            //  master by def'n doesn't undergo LOO so won't change if no block edits.
            //  remove all other rows ...
            for (int i = nRows - 1; i > 0; --i) {
                deleteSucceeded &= alignment->DeleteRow(i);
                if (!deleteSucceeded) {
                    if (firstDeleteFailure < 0) firstDeleteFailure = i;
                }
            }

            //  ... and replace them w/ those from the new BMA.
            mergeSucceeded = alignment->MergeAlignment(refinedAlignment);


            /*  Current CJL hack:  Needs real indoor plumbing. */
            //  Some magic to force redraw of sequence windows...
            if (mergeSucceeded) {
                sequenceViewer->CreateSequenceWindow(true);
            } else {
                //  Get rid of anything added before failure...
                nRows = alignment->NRows();
                for (int i = nRows - 1; i > 0 ; --i) {
                    alignment->DeleteRow(i);
                }
                alignment->MergeAlignment(clone);                
            }
            delete clone;
                //  Merge the refined version of row n-1.
//                vector<int> toRemove;  toRemove.push_back(nRows - 1);
//                BlockMultipleAlignment::AlignmentList extractedBMA;
//                bool extractedLastRow = refinedAlignment->ExtractRows(toRemove, &extractedBMA);
//            if (nRows > 2 && alignment->DeleteRow(nRows - 1)) {  //  remove the last row first...
//            }
        }

        //  Some cleanup of BMA's.
        for (BMARefiner::Cn3DBMAListIt it = refinedAlignments.begin(); it != refinedAlignments.end(); ++it) {
            delete *it;
        }

    } else {

        //  Make sure the original alignment has been restored....

        ERRORMSG("SequenceViewerWindow::OnRefineAlignment() - refinement failed.  Alignment unchanged.");
    }
}
