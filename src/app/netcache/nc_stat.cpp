      ...
      m_ChunksWCntBySize(40, 0)
{
    ...
    m_UnfinishedBlobs.Initialize();
    m_NumOfFiles    .Initialize();
    m_DBSize        .Initialize();
    m_GarbageSize   .Initialize();
    m_CacheSize     .Initialize();
    m_CacheCnt      .Initialize();
    m_CntWriteTasks .Initialize();
}
...
CNCStat::x_CollectTo(CNCStat* dest)
{
    ...
    dest->m_UnfinishedBlobs .AddValues(m_UnfinishedBlobs);
    dest->m_NumOfFiles      .AddValues(m_NumOfFiles);
    dest->m_DBSize          .AddValues(m_DBSize);
    dest->m_GarbageSize     .AddValues(m_GarbageSize);
    dest->m_CacheSize       .AddValues(m_CacheSize);
    dest->m_CacheCnt        .AddValues(m_CacheCnt);
    dest->m_CntWriteTasks   .AddValues(m_CntWriteTasks);
...
CNCStat::Print(CPrintTextProxy& proxy)
{
    ...
    proxy << endl
          << "Storage - "
                        ...
                        << g_ToSmartStr(stat.m_CntBlobs.GetMaximum()) << " max), "
                        << stat.m_UnfinishedBlobs.GetAverage() << " avg unfinished ("
                        << stat.m_UnfinishedBlobs.GetMaximum() << " max), "
                        << stat.m_CntWriteTasks.GetAverage() << " avg write queue ("
                        << stat.m_CntWriteTasks.GetMaximum() << " max)" << endl
          << "Cache - "
                        << g_ToSizeStr(stat.m_CacheSize.GetAverage()) << " avg size ("
                        << g_ToSizeStr(stat.m_CacheSize.GetMaximum()) << " max),"
                        << g_ToSmartStr(stat.m_CacheCnt.GetAverage()) << " avg records ("
                        << g_ToSmartStr(stat.m_CacheCnt.GetMaximum()) << " max)," << endl
          << "Disk data - "
