...
class CAgpValException : std::exception
{
public:
    CAgpValException(const string& msg) :
        m_Msg(msg) {}
...
public:
    ...
    void SaveRow(const string& s, CRef<CAgpRow> row, TRangeColl* runs_of_Ns) override;
...
void CAgpValidateApplication::Init(void)
{
    ...
    // -comp args
    arg_desc->AddOptionalKey("loadlog", "FILE", "specifies where we write our loading log for -comp", CArgDescriptions::eOutputFile);
    arg_desc->AddFlag("ignoreagponly", "");
    arg_desc->AddFlag("ignoreobjfileonly", "");
    arg_desc->AddDefaultKey("diffstofind", "", "", CArgDescriptions::eInteger, "0");

    arg_desc->AddFlag("species", "allow components from different subspecies");

    arg_desc->AddOptionalKey("out", "FILE", "add missing version 1 to component accessions", CArgDescriptions::eOutputFile);

    arg_desc->AddOptionalKey("v", "ver", "AGP version", CArgDescriptions::eString);

    arg_desc->AddOptionalKey("skip", "error_or_warning", "Message or message code to skip", CArgDescriptions::eString, CArgDescriptions::fAllowMultiple);

    arg_desc->AddOptionalKey("only", "error_or_warning", "Message or message code to print (hide other)", CArgDescriptions::eString, CArgDescriptions::fAllowMultiple);

    arg_desc->AddOptionalKey("show", "error_or_warning", "Message or message code to print (if not printed by default)", CArgDescriptions::eString, CArgDescriptions::fAllowMultiple);

    arg_desc->AddDefaultKey("limit", "ErrorCount", "Print at most ErrorCount lines with a particular error", CArgDescriptions::eInteger, "100");

    arg_desc->AddFlag("list", "all possible errors and warnings");

    // file list for file processing
    arg_desc->AddExtra(0, 10000, "files to be processed", CArgDescriptions::eString
        ...
        if (! args["comp"]) {
            ...
            if (m_ValidationType == VT_Context) {
                ...
                if (m_ValidationType & VT_Taxid)
                    taxid_check_failed = ! m_AltValidator->CheckTaxids(cout, m_use_xml);
            ...
            if (CAgpFastaComparator::eResult_Success !=
                agpFastaComparator.Run(filenames, comploadlog, agp_as_fasta_file, diffsToHide, diffsToFind)) {
                cerr << "AGP/FASTA comparison failed." << endl;
            }
        }

    } catch (const CAgpValException& e) {
        cerr << e.what() << "\n";
        return 1;
    }
    ...
    if (m_reader.m_is_chr) {
        if (m_reader.m_explicit_scaf) {
            if (! m_use_xml) {
                ...
                if (out)
                    *out << "===== Chromosome from scaffold AGP =====" << endl;
            }
        }
        ...
        if (! m_use_xml) {
            ...
            if (out)
                *out << "===== Scaffold from component AGP =====" << endl; // header for details that are printed below
        }
    }
    ...
    if (args.GetNExtra() == 0) {
        ...
        for (unsigned int i = 1; i <= args.GetNExtra(); i++) {
            ...
            if (m_CurrentFileName == "-chr") {
                if (m_reader.m_is_chr) {
                    throw CAgpValException("Error -- second -chr is not supported.");
                }
                if (! m_reader.m_explicit_scaf) {
                    throw CAgpValException("Error -- -chr after a file, but no preceding -scaf. Expecting:\n"
            ...
            if (! istr) {
                throw CAgpValException("Error -- unable to open file : " + m_CurrentFileName);
            }
            ...
            if (ch == '>') {
                ...
                if (allowFasta && num_fasta_files)
                    x_ReportFastaSeqCount();
                if (args.GetNExtra() - num_fasta_files > 1)
                    pAgpErr->StartFile(m_CurrentFileName);
                x_ValidateFile(istr);
                allowFasta = false;
            }
        }
...
static void s_ThrowLengthRedefinedException(int    prev_len,
                                            int    len,
                                            string seq_id,
                                            string filename,
                                            int    header_line_num,
...
{
    auto msg =
        format("ERROR - sequence length redefined from {} to {}\n"
               "  sequence id: {}\n"
               "  File: {}\n"
               "  Lines: {}..{}\n",
               prev_len,
    ...
    while (NcbiGetline(istr, line, "\r\n")) {
        ...
        if (line[0] == '>') {
            if (acc.size()) {
                ...
                if (acc_long != acc)
                    prev_len = m_comp2len.AddCompLen(acc_long, len, false);
                if (prev_len) {
                    s_ThrowLengthRedefinedException(prev_len, len, acc_long, filename, header_line_num, line_num);
                }
            ...
            if (acc.size() == 0) {
                throw CAgpValException(format(
                    "ERROR - expecting >fasta_header at start of file {}, got:\n{}\n",
                    filename,
                    line.substr(0, 100)));
            }
    ...
    if (row) {
        ...
        if (runs_of_Ns && runs_of_Ns->size()) {
            ...
            if (row->GetVersion() == eAgpVersion_auto) {
                NCBI_THROW(CException, eUnknown, "FATAL: need AGP version (for adding gap lines). Please use -v 1 or -v 2");
            }
            /*
            ...
            */
            ...
            if (tmp_row->component_beg <= row->component_end) {
                ...
                return; // ends with Ns => skip printing the component row below
        }
