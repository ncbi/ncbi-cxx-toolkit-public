...
int CQueryHash::AddQuery( CQuery * query, int component )
{
    ...
    if( GetOffset() == 0 ) {
        ...
        // TODO: modify to make two-word working!
		CHashPopulator hashPopulator( *m_permutators[m_maxMism], m_winLen, query, m_strands, offset, component, m_maxSimplicity, fwindow, query->GetCoding() ); 
...
int CQueryHash::x_GetNcbi4na_ncbi8na( Uint8& window, const unsigned char * data, unsigned length )
{
    ...
    window = Ncbi8na2Ncbi4na( t, m_winLen ); 
    return off;
}
...
int CQueryHash::x_GetNcbi4na_quality( Uint8& window, const unsigned char * data, unsigned length, TCvt * fun, int incr, unsigned short score, TDecr * decr )
{
    ...
    while( ac > m_maxAlt ) {
        ...
        Uint8 mwin = 0;
        if( left > 0 ) {
            owin = fun( t + incr, m_winLen, score );
            aco  = Ncbi4naAlternativeCount( owin, m_winLen );
        }
        if( score ) { 
            mwin = fun( t, m_winLen, decr( score ) );
            acm  = Ncbi4naAlternativeCount( mwin, m_winLen );
        }
