...
BOOST_AUTO_TEST_CASE(TestInitTreeFeaturesWithBadInput)
{
    ...
    BOOST_REQUIRE_THROW(CGuideTree(btc, ids, *scope), CGuideTreeException);
...
BOOST_AUTO_TEST_CASE(TestInitLabels)
{
    ...
    CGuideTree tree(btc, CGuideTree::eSeqTitle);
...
BOOST_AUTO_TEST_CASE(TestIsSingleBlastName)
{
    ...
    auto_ptr<CGuideTree> tree(new CGuideTree(btc, ids, *scope));
    ...
    tree.reset(new CGuideTree(btc, ids, *scope));
...
BOOST_AUTO_TEST_CASE(TestSimplifyByBlastName)
{
    ...
    CGuideTree tree(btc, ids, *scope);
    tree.SimplifyTree(CGuideTree::eByBlastName);
    ...
    BOOST_REQUIRE_EQUAL(node->GetFeature(
        CGuideTree::GetFeatureTag(CGuideTree::eTreeSimplificationTagId)), "1");

    BOOST_REQUIRE_EQUAL(tree.GetSimplifyMode(), CGuideTree::eByBlastName);
}
...
BOOST_AUTO_TEST_CASE(TestFullyExpand)
{
    ...
    tree.SimplifyTree(CGuideTree::eFullyExpanded);
    ...
    ITERATE (CNodeSet::Tdata, node, tree_cont->GetNodes().Get()) {
        ...
        ITERATE (CNodeFeatureSet::Tdata, it, (*node)->GetFeatures().Get()) {
            ...
            if ((*it)->GetFeatureid() == CGuideTree::eTreeSimplificationTagId) {
    ...
    BOOST_REQUIRE_EQUAL(tree.GetSimplifyMode(), CGuideTree::eFullyExpanded);
}
...
BOOST_AUTO_TEST_CASE(TestExpandCollapse)
{
    ...
    BOOST_REQUIRE_EQUAL(tree.GetNonNullNode(kNodeId)->GetFeature(
               CGuideTree::GetFeatureTag(CGuideTree::eTreeSimplificationTagId)),
                        "1");
    ...
    BOOST_REQUIRE_EQUAL(tree.GetSimplifyMode(), CGuideTree::eNone);
}
...
BOOST_AUTO_TEST_CASE(TestPrintTreeNewick)
{
    ...
    auto_ptr<CGuideTree> tree(new CGuideTree(btc, CGuideTree::eSeqTitle));
    ...
    tree.reset(new CGuideTree(btc, CGuideTree::eTaxName));
    tree->PrintNewickTree(ostr);
    ...
    tree.reset(new CGuideTree(btc, CGuideTree::eBlastName));
    tree->PrintNewickTree(ostr);
    output = CNcbiOstrstreamToString(ostr);
    ...
    tree.reset(new CGuideTree(btc, CGuideTree::eSeqId));
    tree->PrintNewickTree(ostr);
    output = CNcbiOstrstreamToString(ostr);
    ...
    tree.reset(new CGuideTree(btc, CGuideTree::eSeqIdAndBlastName));
    tree->PrintNewickTree(ostr);
    output = CNcbiOstrstreamToString(ostr);
...
{
    vector<CGuideTree::EFeatureID> feature_ids(CGuideTree::eLastId + 1);
    feature_ids[CGuideTree::eLabelId] = CGuideTree::eLabelId;
    feature_ids[CGuideTree::eDistId] = CGuideTree::eDistId;
    feature_ids[CGuideTree::eSeqIdId] = CGuideTree::eSeqIdId;
    feature_ids[CGuideTree::eOrganismId] = CGuideTree::eOrganismId;
    feature_ids[CGuideTree::eTitleId] = CGuideTree::eTitleId;
    feature_ids[CGuideTree::eAccessionNbrId] = CGuideTree::eAccessionNbrId;
    feature_ids[CGuideTree::eBlastNameId] = CGuideTree::eBlastNameId;
    feature_ids[CGuideTree::eAlignIndexId] = CGuideTree::eAlignIndexId;
    feature_ids[CGuideTree::eNodeColorId] = CGuideTree::eNodeColorId;
    feature_ids[CGuideTree::eLabelColorId] = CGuideTree::eLabelColorId;
    feature_ids[CGuideTree::eLabelBgColorId] = CGuideTree::eLabelBgColorId;
    feature_ids[CGuideTree::eLabelTagColorId] = CGuideTree::eLabelTagColorId;
    feature_ids[CGuideTree::eTreeSimplificationTagId]
        = CGuideTree::eTreeSimplificationTagId;

    vector<bool> found(CGuideTree::eLastId + 1, false);
    ...
    ITERATE (CFeatureDictSet::Tdata, it, dict.Get()) {
        ...
        BOOST_REQUIRE(id >= 0 && id <= CGuideTree::eLastId);
        BOOST_REQUIRE_EQUAL((*it)->GetName(),
                            CGuideTree::GetFeatureTag(feature_ids[id]));
...
{
    vector<bool> features(CGuideTree::eLastId + 1, false);
    ITERATE (CNodeFeatureSet::Tdata, it, node.GetFeatures().Get()) {
        int id = (*it)->GetFeatureid();
        BOOST_REQUIRE(id >= 0 && id <= CGuideTree::eLastId);

        if (!is_leaf) {
            BOOST_REQUIRE(id == CGuideTree::eDistId
                          || id == CGuideTree::eTreeSimplificationTagId);
        }
    ...
    if (is_leaf) {
        ...
        for (int i=0;i < CGuideTree::eLabelColorId;i++) {
            BOOST_REQUIRE(features[i]);
        }
    }
    else {
        BOOST_REQUIRE(features[CGuideTree::eDistId]
                      && features[CGuideTree::eTreeSimplificationTagId]);
    }
