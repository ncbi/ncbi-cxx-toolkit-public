...
if (NOT WIN32)
    ...
    set(NCBI_ASNTOOL //snowman/win-coremake/Lib/Ncbi/C_Toolkit/vs2017.64/c.sc-30/bin/asntool.exe)
endif()
...
function(NCBI_generate_by_asntool)
    ...
    if (ASNTOOL_LOCAL)
        ...
        string(APPEND _generatesrc "echo ${NCBI_CURRENT_SOURCE_DIR}/${_dataspec}\n")
        string(APPEND _generatesrc "cd ${NCBI_CURRENT_SOURCE_DIR}\n")
        string(APPEND _generatesrc "${CMAKE_COMMAND} -E copy_if_different ${_specpath} ${NCBI_CURRENT_SOURCE_DIR}\n")
        string(APPEND _generatesrc "${NCBI_ASNTOOL} -w 100 -m ${_dataspec} -o ${_header} ${_redir}\n")
        string(APPEND _generatesrc "${NCBI_ASNTOOL} -w 100 -G -m ${_dataspec} -B ${_basename} ${_externals} -K ${_header} ${ASNTOOL_GENFLAGS} ${_redir}\n")
        if(WIN32)
        ...
        string(APPEND _generatesrc "echo ${NCBI_CURRENT_SOURCE_DIR}/${_dataspec}\n")
        string(APPEND _generatesrc "cd ${NCBI_CURRENT_SOURCE_DIR}\n")
        string(APPEND _generatesrc "${CMAKE_COMMAND} -E make_directory ${_inc_dir}\n")
        string(APPEND _generatesrc "${CMAKE_COMMAND} -E copy_if_different ${_specpath} ${NCBI_CURRENT_SOURCE_DIR}\n")
        string(APPEND _generatesrc "${NCBI_ASNTOOL} -w 100 -m ${_dataspec} -o ${_inc_dir}/${_header} ${_redir}\n")
        string(APPEND _generatesrc "${NCBI_ASNTOOL} -w 100 -G -m ${_dataspec} -B ${_basename} ${_externals} -K ${_header} ${ASNTOOL_GENFLAGS} ${_redir}\n")
        if(WIN32)
            string(APPEND _generatesrc "if errorlevel 1 (set GENERATESRC_RESULT=1)\n")
        else()
        ...
        string(APPEND _generatesrc "${CMAKE_COMMAND} -E copy_if_different ${_basename}.h ${_inc_dir}\n")
        string(APPEND _generatesrc "${CMAKE_COMMAND} -E remove -f ${_basename}.h\n")
    endif()
    file(APPEND ${NCBI_GENERATESRC_ASNTOOL} ${_generatesrc})
endfunction()
