...
function(NCBI_add_subdirectory)
    ...
    if(NCBI_PTBMODE_COLLECT_DEPS)
        set(_cursrc ${NCBI_CURRENT_SOURCE_DIR})
        foreach(_sub IN LISTS ARGV)
            if(IS_ABSOLUTE "${_sub}")
                set(_curdir ${_sub})
            else()
                set(_curdir ${_cursrc}/${_sub})
            endif()
            NCBI_util_std_path( "${_curdir}" _current)
            get_property(_module_roots GLOBAL PROPERTY NCBI_PTBPROP_MODULE_ROOTS)
            if (EXISTS "${_curdir}/CMakeLists.txt")
                set(NCBI_CURRENT_SOURCE_DIR ${_curdir})
                if("${_current}" IN_LIST _module_roots)
                    get_filename_component(_name ${_curdir} NAME_WE)
                    NCBI_module(${_name})
                endif()
                NCBI_internal_include("${_curdir}/CMakeLists.txt")
            else()
                NCBI_notice("WARNING: directory not found: ${_curdir}")
            endif()
        endforeach()
    else()
        get_property(_module_roots GLOBAL PROPERTY NCBI_PTBPROP_MODULE_ROOTS)
        foreach(_sub IN LISTS ARGV)
            ...
            if("${_current}" IN_LIST _module_roots)
                get_filename_component(_name ${NCBI_CURRENT_SOURCE_DIR} NAME_WE)
                NCBI_module(${_name})
            endif()
...
function(NCBI_add_library)
    ...
    foreach(_lib IN LISTS ARGV)
        ...
        elseif (EXISTS ${NCBI_CURRENT_SOURCE_DIR}/${_lib} AND NOT IS_DIRECTORY ${NCBI_CURRENT_SOURCE_DIR}/${_lib})
            ...
            NCBI_notice("WARNING: Library project not found: ${_lib} (${NCBI_CURRENT_SOURCE_DIR})")
        endif()
    endforeach()
...
function(NCBI_add_app)
    ...
    foreach(_app IN LISTS ARGV)
        ...
        elseif (EXISTS ${NCBI_CURRENT_SOURCE_DIR}/${_app} AND NOT IS_DIRECTORY ${NCBI_CURRENT_SOURCE_DIR}/${_app})
            ...
            NCBI_notice("WARNING: App project not found: ${_app} (${NCBI_CURRENT_SOURCE_DIR})")
        endif()
    endforeach()
    unset(NCBI_PROJECT_expected)
endfunction()
...
macro(NCBI_module _name)
    ...
    if (NOT EXISTS "${NCBI_SRC_ROOT}")
        NCBI_notice("WARNING: Module ${_name} does not have ${NCBI_DIRNAME_SRC} directory (${NCBI_CURRENT_SOURCE_DIR})")
        set(NCBI_SRC_ROOT   ${NCBI_TREE_ROOT})
    endif()
    if (NOT EXISTS "${NCBI_INC_ROOT}")
        NCBI_notice("WARNING: Module ${_name} does not have ${NCBI_DIRNAME_INCLUDE} directory (${NCBI_CURRENT_SOURCE_DIR})")
...
function(NCBI_declare_module_root)
    ...
    foreach(_dir IN LISTS ARGV)
        ...
        if(EXISTS ${_dir}/${NCBI_DIRNAME_SRC})
            ...
            NCBI_notice("WARNING: Module root does not have ${NCBI_DIRNAME_SRC} directory (${_dir})")
            NCBI_declare_module_src_root(_unknown ${_dir})
        endif()
    endforeach()
...
function(NCBI_add_target)
    ...
    foreach(_prj IN LISTS ARGV)
        ...
        elseif (EXISTS ${NCBI_CURRENT_SOURCE_DIR}/${_prj} AND NOT IS_DIRECTORY ${NCBI_CURRENT_SOURCE_DIR}/${_prj})
            ...
            NCBI_notice("WARNING: Target not found: ${_prj} (${NCBI_CURRENT_SOURCE_DIR})")
        endif()
    endforeach()
endfunction()
...
macro(NCBI_internal_analyze_tree)
    ...
    if(NCBI_PTBCFG_ENABLE_COLLECTOR)
        ...
        NCBI_notice("${_elapsed}: Analyzing source tree...")
        NCBI_notice("Scanning folders: ${NCBI_PTBCFG_KNOWN_FOLDERS}")
        ...
        NCBI_util_elapsed(_elapsed)
        NCBI_notice("${_elapsed}: Collecting projects...")
        ...
        foreach(_prj IN LISTS NCBI_PTB_ALLOWED_PROJECTS)
            if(NOT TARGET "${_prj}" AND
                (NCBI_VERBOSE_ALLPROJECTS OR NCBI_VERBOSE_PROJECT_${_prj} OR NCBI_VERBOSE_DEPENDENCIES))
                NCBI_internal_print_project_info(${_prj})
            endif()
        endforeach()
        foreach(_prj IN LISTS NCBI_VERBOSE_PROJECTS)
            if(TARGET "${_prj}")
                NCBI_internal_report_interface_library_info(${_prj})
            elseif(DEFINED NCBI_COMPONENT_${_prj}_FOUND)
                NCBI_internal_report_NCBIptb_component(${_prj})
            endif()
        endforeach()
        ...
        NCBI_notice("Source tree analysis skipped")
    endif()
    NCBI_util_elapsed(_elapsed)
    if(NOT NCBI_PTBCFG_COLLECT_REQUIRES)
        NCBI_notice("${_elapsed}: Configuring projects...")
...
function(NCBI_internal_verify_targets)
    ...
    if( NOT "${NCBI_PTBCFG_PROJECT_TARGETS}" STREQUAL "" OR
        ...
        if(NOT "${_notfound}" STREQUAL "")
            NCBI_notice("WARNING: The following required build targets were not found: ${_notfound}")
        endif()
    endif()
endfunction()
    ...
    foreach(_part IN LISTS NCBI_${_hostproject}_PARTS)
        ...
        foreach(_root IN LISTS _all_roots)
            ...
            if (EXISTS ${_path}/CMakeLists.${_lib}.lib.txt OR
                ...
                if(_part_found)
                    if(NOT NCBI_PTBMODE_COLLECT_DEPS)
                        NCBI_notice("${NCBI_PROJECT}: Project part ${_part} (${_root}) is replaced with one in (${NCBI_SRC_ROOT${_part}})")
                    endif()
                else()
...
##############################################################################
function(NCBI_internal_identify_libs _link _linkext _deps)
    set(_linklibs)
    set(_prjdeps)
    foreach( _item IN LISTS NCBITMP_NCBILIB)
    ...
    set(_ext)
    if(NCBITMP_EXTLIB)
        foreach( _item IN LISTS NCBITMP_EXTLIB)
            if(TARGET ${_item})
                get_property(_type TARGET ${_item} PROPERTY TYPE)
                if(${_type} MATCHES "LIBRARY")
                    list(APPEND _ext ${_item})
                else()
                    list(APPEND _prjdeps ${_item})
                endif()
            else()
    ...
    set(${_linkext} ${_ext} PARENT_SCOPE)
    ...
    if (DEFINED NCBI_EXTERNAL_TREE_ROOT AND MSVC)
        if (${NCBI_${NCBI_PROJECT}_TYPE} STREQUAL "CONSOLEAPP" OR ${NCBI_${NCBI_PROJECT}_TYPE} STREQUAL "GUIAPP")
            ...
            foreach(_lib IN LISTS _libs)
                get_target_property(_deps ${_lib} INTERFACE_LINK_LIBRARIES)
                foreach(_dep IN LISTS _deps)
                    if(TARGET ${_dep} AND DEFINED NCBI_COMPONENT_NCBINAME_${_dep})
                        list(APPEND _reqs ${NCBI_COMPONENT_NCBINAME_${_dep}})
                    elseif(NOT TARGET ${_dep})
...
#                message("         because there is already a target with the same name in ${_dir}")
        ...
        if(NOT "${_altdir}" STREQUAL "" AND NOT "${_altdir}" STREQUAL "${_dir}")
            NCBI_notice("${NCBI_PROJECT} (${_altdir}): is replaced with one in (${_dir})")
        ...
        if(NCBI_UNRELATED_TREES OR "${_altdir}" STREQUAL "")
            ...
            if(NOT "${_dir}" STREQUAL "${NCBI_CURRENT_SOURCE_DIR}" AND NOT "${_dir}" STREQUAL "")
                NCBI_notice("WARNING: ${NCBI_PROJECT} (${NCBI_CURRENT_SOURCE_DIR}) is excluded")
                NCBI_notice("         because there is already a target with the same name in ${_dir}")
                if ("${ARGC}" GREATER "0")
                    set(${ARGV0} FALSE PARENT_SCOPE)
                endif()
                return()
...
message("  TAGS = ${NCBI_${NCBI_PROJECT}_PROJTAG}")
    ...
    if ( NOT "${NCBITMP_REQUIRE_NOTFOUND}" STREQUAL "" AND NOT NCBI_PTBMODE_COLLECT_DEPS)
        NCBI_notice("${NCBI_PROJECT} (${NCBI_CURRENT_SOURCE_DIR}) is excluded because of unmet requirements: ${NCBITMP_REQUIRE_NOTFOUND}")
        if (NOT NCBI_PTBMODE_PARTS)
            if ("${ARGC}" GREATER "0")
                set(${ARGV0} FALSE PARENT_SCOPE)
            endif()
    ...
    if ( NOT "${NCBITMP_COMPONENT_NOTFOUND}" STREQUAL "" AND NOT NCBI_PTBMODE_COLLECT_DEPS)
        NCBI_notice("${NCBI_PROJECT} (${NCBI_CURRENT_SOURCE_DIR}): cannot find optional component: ${NCBITMP_COMPONENT_NOTFOUND}")
    endif()
    if(NCBI_VERBOSE_ALLPROJECTS OR NCBI_VERBOSE_PROJECT_${NCBI_PROJECT})
    ...
    if ( NOT "${NCBITMP_TARGET_NOTFOUND}" STREQUAL "" AND NOT NCBI_PTBMODE_COLLECT_DEPS)
        NCBI_notice("${NCBI_PROJECT} (${NCBI_CURRENT_SOURCE_DIR}) is excluded because it depends on missing targets: ${NCBITMP_TARGET_NOTFOUND}")
        if (NOT NCBI_PTBMODE_PARTS)
            if ("${ARGC}" GREATER "0")
                set(${ARGV0} FALSE PARENT_SCOPE)
            endif()
    ...
    if (DEFINED NCBI_${NCBI_PROJECT}_OUTPUT)
        if(NOT DEFINED NCBI_EXTERNAL_TREE_ROOT AND NOT ${NCBI_PROJECT} STREQUAL ${NCBI_${NCBI_PROJECT}_OUTPUT})
            ...
            if (NOT ${_dir} STREQUAL "" AND (NOT ${_dir} STREQUAL ${NCBI_CURRENT_SOURCE_DIR} OR TARGET ${NCBI_${NCBI_PROJECT}_OUTPUT}))
                NCBI_notice("WARNING: App target ${NCBI_${NCBI_PROJECT}_OUTPUT} (${NCBI_CURRENT_SOURCE_DIR}) cannot be created")
                NCBI_notice("         because there is already a target with the same name in ${_dir}")
                NCBI_notice("         App target ${NCBI_${NCBI_PROJECT}_OUTPUT} will be renamed into ${NCBI_PROJECT}")
            elseif(NOT NCBI_PTBCFG_ENABLE_COLLECTOR)
                NCBI_notice("WARNING: App target ${NCBI_${NCBI_PROJECT}_OUTPUT} (${NCBI_CURRENT_SOURCE_DIR}) cannot be created")
                NCBI_notice("         because there is already a target with the same name elsewhere")
                NCBI_notice("         App target ${NCBI_${NCBI_PROJECT}_OUTPUT} will be renamed into ${NCBI_PROJECT}")
            endif()
        endif()
...
# this does not seem to have any effect
        ...
        NCBI_internal_identify_libs(_linklibs _linkext _prjdeps)
        if ("${_linklibs}" STREQUAL "")
            set(_linklibs ${ORIG_LIBS})
        endif()
        target_link_libraries(     ${NCBI_PROJECT} PUBLIC ${_linklibs})
        target_link_libraries(     ${NCBI_PROJECT} PUBLIC ${_linkext})
        if (NOT "${_prjdeps}" STREQUAL "")
            add_dependencies(${NCBI_PROJECT} ${_prjdeps})
        endif()
...
message("-----------------------------------")
    ...
    if ("${ARGC}" GREATER "0")
        set(${ARGV0} ${NCBI_PROJECT} PARENT_SCOPE)
    endif()
endfunction()
...
macro(NCBI_util_load_file _file _result)
    ...
    if (EXISTS "${_file}" AND NOT IS_DIRECTORY "${_file}")
        ...
        NCBI_notice("WARNING: unable to load ${_file}")
    endif()
endmacro()
