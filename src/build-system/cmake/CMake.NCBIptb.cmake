...
function(NCBI_internal_install_component_files _comp)
    ...
    foreach(_cfg IN LISTS NCBI_CONFIGURATION_TYPES)
        ...
        if (DEFINED NCBI_COMPONENT_${_comp}_BINPATH_${_src_cfg}${_rt})
        ...
        elseif (DEFINED NCBI_COMPONENT_${_comp}_BINPATH)
            if (IS_DIRECTORY ${NCBI_COMPONENT_${_comp}_BINPATH}/${_src_cfg}${_rt})
...
function(NCBI_internal_rank_projects)
    ...
    foreach(_r RANGE ${_count} 0 -1)
        ...
        message("${_r}:   ${_lst}")
        if(OFF)
            foreach(_t IN LISTS _lst)
                get_property(_x GLOBAL PROPERTY NCBI_PTBPROP_DIRECT_DEPS_${_t})
                string(REPLACE ";" "\", \"" _x "${_x}")
                message("        if \"${_t}\" in allexports:")
                message("            self.cpp_info.components[\"${_t}\"].libs = [\"${_t}\"]")
                message("            self.cpp_info.components[\"${_t}\"].requires = [\"${_x}\"]")
            endforeach()
        endif()
    endforeach()
    message("=============================================================================")
...
if(DEFINED NCBI_PTBCFG_PROJECT_TAGS)
...
if(DEFINED NCBI_PTBCFG_PROJECT_TARGETS)
