...
#message("running collector")
        set(NCBI_PTBMODE_COLLECT_DEPS ON)
        set_property(GLOBAL PROPERTY NCBI_PTBPROP_ALLPROJECTS "")

        NCBI_add_subdirectory(${ARGV})

        set(NCBI_PTBMODE_COLLECT_DEPS OFF)
        get_property(_allprojects GLOBAL PROPERTY NCBI_PTBPROP_ALLPROJECTS)

        if(OFF)
            message("NCBI_PTBPROP_ALLPROJECTS: ${_allprojects}")
        ...
        endforeach()
        if ( "${NCBI_PTB_ALLOWED_PROJECTS}" STREQUAL "")
            set(NCBI_PTB_ALLOWED_PROJECTS ${_allprojects})
        endif()
        if ( "${NCBI_PTB_ALLOWED_PROJECTS}" STREQUAL "")
            message(WARNING "ERROR: List of projects is empty")
            return()
        endif()
...
function(NCBI_internal_add_project)
    ...
    if (NOT NCBI_PTBMODE_PARTS AND NOT NCBI_PTBMODE_COLLECT_DEPS AND NCBI_EXPERIMENTAL_ENABLE_COLLECTOR)
        list(FIND NCBI_PTB_ALLOWED_PROJECTS ${NCBI_PROJECT_ID} _found)
        if (${_found} LESS "0")
...
#message("${NCBI_PROJECT_ID} is hosted in  ${_hosted}")
#    if (NOT "${_hosted}" STREQUAL "" AND NOT "${_hosted}" STREQUAL "${NCBI_PROJECT_ID}")
...
message("  DEFINES = ${NCBI_${NCBI_PROJECT}_DEFINES}")
    ...
    if (NCBI_PTBMODE_COLLECT_DEPS)
        ...
        if(NOT NCBI_PTBMODE_PARTS)
            get_property(_allprojects GLOBAL PROPERTY NCBI_PTBPROP_ALLPROJECTS)
            set(_allprojects ${_allprojects} ${NCBI_PROJECT_ID})
            set_property(GLOBAL PROPERTY NCBI_PTBPROP_ALLPROJECTS ${_allprojects})
        endif()
        if ("${ARGC}" GREATER "0")
            set(${ARGV0} FALSE PARENT_SCOPE)
        endif()
