...
message("TEAMCITY_BUILD_PROPERTIES_FILE = $ENV{TEAMCITY_BUILD_PROPERTIES_FILE}")
    execute_process(COMMAND "ls" "-laFh" "$ENV{TEAMCITY_BUILD_PROPERTIES_FILE}" OUTPUT_FILE CMD_OUTPUT)
    MESSAGE( STATUS "CMD_OUTPUT:" ${CMD_OUTPUT})
    execute_process(COMMAND cat "$ENV{TEAMCITY_BUILD_PROPERTIES_FILE}" OUTPUT_FILE CMD_OUTPUT)
    MESSAGE( STATUS "CMD_OUTPUT:" ${CMD_OUTPUT})
    if(True OR EXISTS "$ENV{TEAMCITY_BUILD_PROPERTIES_FILE}")
        message("Reading $ENV{TEAMCITY_BUILD_PROPERTIES_FILE}")
        file(STRINGS "$ENV{TEAMCITY_BUILD_PROPERTIES_FILE}" _list)
        message("Contents: <${_list}>")
...
message("NCBI_TEAMCITY_BUILD_ID = ${NCBI_TEAMCITY_BUILD_ID}")
                break()
            endif()
        endforeach()
    else()
