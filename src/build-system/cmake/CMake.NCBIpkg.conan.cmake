...
###############################################################################
function(NCBI_collect_NCBIpackages)
    if("${NCBI_CONFIGURATION_TYPES_COUNT}" EQUAL 1)
        return()
    endif()
    set(_pkglist)
    foreach(_name IN LISTS ARGV)
        foreach(_cfg IN LISTS NCBI_CONFIGURATION_TYPES)
            string(TOUPPER ${_cfg} _cfg)
            if(DEFINED ${_name}_BUILD_MODULES_PATHS_${_cfg})
                list(APPEND _pkglist ${${_name}_BUILD_MODULES_PATHS_${_cfg}})
            endif()
        endforeach()
    endforeach()
    list(REMOVE_DUPLICATES _pkglist)
    foreach(_next IN LISTS _pkglist)
        get_filename_component(_dir "${_next}" DIRECTORY)
        get_filename_component(_root "${_dir}/../.." ABSOLUTE)
        get_filename_component(_name "${_next}" NAME_WE)
        if (EXISTS "${_root}/lib")
            file(GLOB _config_files "${_dir}/${_name}-*.cmake")
            foreach(_file IN LISTS _config_files)
