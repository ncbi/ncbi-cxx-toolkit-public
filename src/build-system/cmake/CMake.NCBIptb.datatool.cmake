...
if(DEFINED NCBI_DATATOOL)
    if(NOT EXISTS "${NCBI_DATATOOL}" AND NOT "${NCBI_DATATOOL}" STREQUAL "datatool")
        message(FATAL_ERROR "NCBI_DATATOOL = ${NCBI_DATATOOL}: not found")
    endif()
else()
if(NCBI_PTBCFG_PACKAGED)
    ...
    if (NOT EXISTS "${NCBI_DATATOOL}")
        ...
        get_filename_component(_candidate "${NCBI_DATATOOL}" NAME)
        set(_candidate ${NCBI_BUILD_ROOT}/${NCBI_DIRNAME_BUILD}/CMakeFiles/${_candidate})
        execute_process(
            COMMAND ${CMAKE_COMMAND} -E compare_files "${_candidate}" "${NCBI_DATATOOL}"
            RESULT_VARIABLE _ret
            OUTPUT_QUIET ERROR_QUIET
        )
        if(NOT _ret EQUAL 0)
            file(COPY "${NCBI_DATATOOL}" DESTINATION "${NCBI_BUILD_ROOT}/${NCBI_DIRNAME_BUILD}/CMakeFiles")
        endif()
        if(EXISTS "${_candidate}")
            set(NCBI_DATATOOL_LOCALCOPY "${_candidate}")
        endif()
    endif()
endif()
endif()
...
#    get_filename_component(_incfiles  "${NCBI_INC_ROOT}/${_relpath}"     ABSOLUTE)
    ...
    if(IS_ABSOLUTE "${NCBI_DATATOOL_LOCALCOPY}")
        set(_dtcmd "${NCBI_DATATOOL_LOCALCOPY}")
        set(_dtdep "${NCBI_DATATOOL}")
    elseif(IS_ABSOLUTE "${NCBI_DATATOOL}")
        set(_dtcmd "${NCBI_DATATOOL}")
        set(_dtdep "${NCBI_DATATOOL}")
    else()
        set(_dtcmd "$<TARGET_FILE:${NCBI_DATATOOL}>")
        set(_dtdep)
    endif()
    set(_cmd ${_dtcmd} ${_oex} ${_pch} -m ${DT_DATASPEC} -oA -oc ${_oc} -od ${_od} -odi ${_ocvs} -or ${_relpath} -oR ${NCBI_TREE_ROOT} ${_opm} ${_imports})
#message("_cmd = ${_cmd}")
    set(_depends ${_dtdep} ${DT_DATASPEC})
    if(EXISTS ${_od})
...
function(NCBI_internal_allow_datatool_2)
    ...
    if(NOT "${NCBI_DATATOOL}" IN_LIST _allowedprojects)
        ...
        if(NOT "${NCBI_DATATOOL}" IN_LIST _allprojects)
            set(_dt)
            find_program(_dt datatool${CMAKE_EXECUTABLE_SUFFIX})
            if(_dt)
                message(STATUS "Datatool location (only found in PATH): ${_dt}")
                set(NCBI_DATATOOL "${_dt}" PARENT_SCOPE)
            else()
