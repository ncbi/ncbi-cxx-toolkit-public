...
if (WIN32)
	...
	if(${NCBI_COMPILER_VERSION} LESS 1300)
            message(FATAL_ERROR "GCC13 or newer is required")
	endif()
...
if (WIN32)
    ...
    if(OptDebug IN_LIST NCBI_PTBCFG_PROJECT_FEATURES)
        set(CMAKE_CXX_FLAGS_DEBUG  "/O2" CACHE STRING "Reset the configuration" FORCE)
        set(CMAKE_C_FLAGS_DEBUG   "/O2" CACHE STRING "Reset the configuration" FORCE)
    endif()
...
elseif (XCODE)
    ...
    if(OptDebug IN_LIST NCBI_PTBCFG_PROJECT_FEATURES)
        string(REPLACE "-O0" "-Os" CMAKE_CXX_FLAGS_DEBUG "${CMAKE_CXX_FLAGS_DEBUG}")
        string(REPLACE "-O0" "-Os" CMAKE_C_FLAGS_DEBUG "${CMAKE_C_FLAGS_DEBUG}")
    endif()
    if(Symbols IN_LIST NCBI_PTBCFG_PROJECT_FEATURES)
...
if(OptDebug IN_LIST NCBI_PTBCFG_PROJECT_FEATURES)
    string(REPLACE "-O0" "-O3" CMAKE_CXX_FLAGS_DEBUG "${CMAKE_CXX_FLAGS_DEBUG}")
    string(REPLACE "-O0" "-O3" CMAKE_C_FLAGS_DEBUG "${CMAKE_C_FLAGS_DEBUG}")
endif()
if(Symbols IN_LIST NCBI_PTBCFG_PROJECT_FEATURES)
    if(DEFINED CMAKE_C_FLAGS_RELWITHDEBINFO)
        set(CMAKE_C_FLAGS_RELEASE   "${CMAKE_C_FLAGS_RELWITHDEBINFO}")
