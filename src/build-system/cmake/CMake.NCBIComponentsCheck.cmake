...
#############################################################################
function(NCBI_component_find_default_module _name _module)
    if(NCBI_COMPONENT_${_name}_DISABLED)
        return()
    endif()
    if(NCBI_TRACE_COMPONENT_${_name} OR NCBI_TRACE_ALLCOMPONENTS)
        message("NCBI_component_find_default_module: ${_name}: ${_module}")
    endif()
    ...
    if(${_name}_FOUND)
        ...
        message(STATUS "Found ${_name}: (version ${_pkg_version})")
        if(NCBI_TRACE_COMPONENT_${_name} OR NCBI_TRACE_ALLCOMPONENTS)
            message("${_name}: include dir = ${_pkg_include}")
            message("${_name}: libs = ${_pkg_libs}")
            if(NOT "${_pkg_defines}" STREQUAL "")
        ...
        set(NCBI_COMPONENT_${_name}_FOUND NO PARENT_SCOPE)
        if(NCBI_TRACE_COMPONENT_${_name} OR NCBI_TRACE_ALLCOMPONENTS)
            message("NCBI_component_find_default_module: ${_name}: module ${_module} not found")
        endif()
    endif()
endfunction()

#############################################################################
macro(NCBI_find_package _name _pkg)
...
macro(NCBI_find_library _name)
    if(NOT NCBI_COMPONENT_${_name}_DISABLED)
        ...
        set(_notfound_libs "")
        if(NCBI_TRACE_COMPONENT_${_name} OR NCBI_TRACE_ALLCOMPONENTS)
            message("NCBI_find_library: ${_name}: ${_args}")
        endif()
        foreach(_lib IN LISTS _args)
...
#############################################################################
function(NCBI_component_find_include _name)
    if(NCBI_COMPONENT_${_name}_DISABLED OR NOT NCBI_COMPONENT_${_name}_FOUND)
        return()
    endif()
    set(_args ${ARGN})
    if(NCBI_TRACE_COMPONENT_${_name} OR NCBI_TRACE_ALLCOMPONENTS)
        message("NCBI_component_find_include: ${_name}: finding ${_args}")
    endif()
    set(_found "")
        ...
        if(IS_ABSOLUTE ${_inc})
            ...
            if(NOT _res)
                ...
                if(_NCBI_COMPONENT_${_name}_INC)
                    list(APPEND _found ${_NCBI_COMPONENT_${_name}_INC})
                endif()
    ...
    if(NCBI_TRACE_COMPONENT_${_name} OR NCBI_TRACE_ALLCOMPONENTS)
        message("NCBI_component_find_include: ${_name}: include dirs: ${_found}")
    endif()
endfunction()

#############################################################################
macro(NCBI_component_add  _name)
    if(NOT NCBI_COMPONENT_${_name}_DISABLED AND NCBI_COMPONENT_${_name}_FOUND)
    endif()
    set(_args ${ARGN})
    if(NCBI_TRACE_COMPONENT_${_name} OR NCBI_TRACE_ALLCOMPONENTS)
        message("NCBI_component_add: ${_name}: adding ${_args}")
    endif()
...
macro(NCBI_define_Xcomponent)
    cmake_parse_arguments(DC "" "NAME;PACKAGE;MODULE" "LIB;INCLUDE;COMPONENT" ${ARGN})
    ...
    elseif(NCBI_COMPONENT_${DC_NAME}_DISABLED)
        ...
        if(NOT NCBI_COMPONENT_${DC_NAME}_FOUND)
            ...
            if(NOT NCBI_COMPONENT_${DC_NAME}_FOUND AND NOT "${DC_MODULE}" STREQUAL "" AND PKG_CONFIG_FOUND)
                NCBI_component_find_default_module(${DC_NAME} ${DC_MODULE})
            ...
            if(NOT NCBI_COMPONENT_${DC_NAME}_FOUND AND NOT "${DC_LIB}" STREQUAL "")
                NCBI_find_library(${DC_NAME} ${DC_LIB})
            endif()

            if(_find_add AND NCBI_COMPONENT_${DC_NAME}_FOUND)
                if(NOT "${DC_INCLUDE}" STREQUAL "")
                    NCBI_component_find_include(${DC_NAME} ${DC_INCLUDE})
                endif()
                if(NOT "${DC_COMPONENT}" STREQUAL "")
                    NCBI_component_add(${DC_NAME} ${DC_COMPONENT})
                endif()
            endif()
        endif()
        if(NOT NCBI_COMPONENT_${DC_NAME}_FOUND)
