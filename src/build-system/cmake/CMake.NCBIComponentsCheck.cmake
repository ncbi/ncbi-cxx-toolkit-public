...
function(NCBI_internal_report_interface_library_info _name)
    set(_libs ${_name})
    set(_done)
    get_target_property(_cfgs ${_name} IMPORTED_CONFIGURATIONS)
    if(_cfgs)
...
if(OFF AND NOT APPLE)
    ...
    if(_all_found)
        ...
        if(NCBI_TRACE_COMPONENT_${_name} OR NCBI_TRACE_ALLCOMPONENTS)
            message("----------------------")
            message("NCBIcomponent_find_module: ${_name}")
            message("${_name}: libs = ${_pkglib}")
        endif()
...
# root
    ...
    if(_all_found)
        ...
        if(NCBI_TRACE_COMPONENT_${_name} OR NCBI_TRACE_ALLCOMPONENTS)
            message("----------------------")
            message("NCBIcomponent_find_cmake_package: ${_name}")
...
message("${_name}: DC_INCPATH_SUFFIX = ${DC_INCPATH_SUFFIX}")
        endif()
        return()
    endif()
...
function(NCBIcomponent_find_default_module _name _pkglib _module)
    ...
    if(${_name}_FOUND)
        ...
        if(NCBI_TRACE_COMPONENT_${_name} OR NCBI_TRACE_ALLCOMPONENTS)
            message("----------------------")
            message("NCBIcomponent_find_default_module: ${_name}")
            message("${_name}: libs = ${_pkglib}")
        endif()
...
#            message(STATUS "Found ${_name}: ${_pkg_libs} ${_pkg_version}")
            ...
            if(NCBI_TRACE_COMPONENT_${_name} OR NCBI_TRACE_ALLCOMPONENTS)
                message("----------------------")
                message("NCBIcomponent_find_package: ${_name}")
                message("${_name}: include dir = ${_pkg_include}")
                message("${_name}: libs = ${_pkg_libs}")
                if(NOT "${_pkg_defines}" STREQUAL "")
                    message("${_name}: defines = ${_pkg_defines}")
...
#                    NCBI_define_component(${DC_NAME} ${DC_LIB})
            ...
            elseif(NCBI_TRACE_COMPONENT_${DC_NAME} OR NCBI_TRACE_ALLCOMPONENTS)
                ...
                NCBI_internal_report_interface_library_info(${_pkglib})
                message("----------------------")
            endif()
        endif()
    endif()
