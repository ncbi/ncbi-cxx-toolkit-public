...
//    ofs << "MAKE = make" << endl;
    ofs << "prefixt:=.ncbi.signal." << endl;
    ofs << "signaturet:=$(prefixt)$(shell date +%Y%b%e%T | sed -e 's/://g' | sed -e 's/ //g')" << endl;
    ofs << "export prefix?=$(prefixt)" << endl;
    ofs << "export s?=$(signaturet)" << endl;
    ofs << "sign=rm -f $(prefix)*.$@; touch $(s).$@" << endl << endl;
