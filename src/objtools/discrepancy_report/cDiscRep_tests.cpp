...
void CBioseq_on_SUSPECT_RULE :: GetReportForStaticList(CRef <CClickableItem> c_item)
{
   ...
   unsigned fixtp, cnt;
   string strtmp, pattern, test_name, test_desc;
...
{
  string strtmp = NStr::Join(c_item->item_list, ",");
  c_item->item_list.clear();
...
void CBioseq_on_base :: TestOnObj(const CBioseq& bioseq)
{
    ...
    for (CSeqVector_CI it =seq_vec.begin(); it; ++it, i++) {
      if ( (*it) == 'N') {
        if (!it.IsInGap()) {  // gaps represented by N
...
void CBioseq_test_on_protfeat :: TestOnObj(const CBioseq& bioseq) 
{
   ...
   string desc, head_desc, func, strtmp;
   bool run_cds = (thisTest.tests_run.find(GetName_cds()) != end_it);
...
void CBioseq_DISC_CDS_PRODUCT_FIND :: GetReport(CRef <CClickableItem> c_item)
{
   ...
   c_item->description 
         = GetContainsComment(c_item->item_list.size(), "coding region product")
            + " suspect phrase or characters";
};
...
void CBioseq_test_on_rna :: TestOnObj(const CBioseq& bioseq)
{
  ...
  if (!run_tcnt && !run_rcnt && !run_rdup && !run_tdup && run_strand) {
      return;
  }
...
void CBioseq_FIND_STRAND_TRNAS :: GetReport(CRef <CClickableItem> c_item)
{
  ...
  string strtmp = c_item->item_list[0].substr(0, pos);
  ...
  if (all_feat.empty()) {
    ...
    ITERATE (vector <const CSeq_feat*>, it, all_feat) {
       if ((*it)->GetLocation().IsMix() || (*it)->GetLocation().IsPacked_int()){
          ...
          thisInfo.test_item_list[GetName_joined()]
                     .push_back(strtmp + "$" + desc);
          thisInfo.test_item_objs[GetName_joined() + "$" + strtmp]
                      .push_back(CConstRef <CObject>(*it));
       }
    }
  }
...
void CSeqEntry_test_on_quals :: GetMultiSubSrcVlus(const CBioSource& biosrc, const string& type_name, vector <string>& multi_vlus)
{
  ...
  ITERATE (list <CRef <CSubSource> >, it, biosrc.GetSubtype()) {
     ...
     if ( (*it)->GetSubtypeName( type ) == type_name ) {
        strtmp = (*it)->GetName();
        if (strtmp.empty() 
                && (type == CSubSource::eSubtype_germline
...
{
   thisInfo.test_item_list[setting_name].clear();
   GetQualDistribute(comb_qual2src_idx, comb_desc_ls, comb_src_ls, objs,
                       setting_name, 0, comb_src_ls.size());
   c_item->item_list = thisInfo.test_item_list[setting_name];
// if (setting_name != GetName_bad() && setting_name != GetName_sq_oncall())
...
*/
   ...
   bool run_dup = (thisTest.tests_run.find(GetName_dup()) != end_it);
   bool run_dt = (thisTest.tests_run.find(GetName_dt()) != end_it);
...
//      this_src_ls.push_back(CConstRef <CBioSource> (&biosrc));
   ...
   // all include DISC_MISSING_SRC_QUAL/DISC_DUP_SRC_QUAL_DATA/DISC_DUP_SRC_QUAL
   ...
   strtmp = run_sq? GetName_sq() 
                     : (run_oncall? GetName_sq_oncall()
                         : (run_dup? GetName_dup() 
                             : (run_dt? GetName_dt() : GetName_miss())));
   if (has_newqual && ( run_sq || run_oncall || run_dup || run_dt || run_miss)
                   && ( thisInfo.test_item_list.find(strtmp)
                            == thisInfo.test_item_list.end())) {
       thisInfo.test_item_list[strtmp].push_back("yes");
   }
...
void CSeqEntry_test_on_quals :: x_GetRep4DupSrcQualAndData(CRef <CClickableItem> c_item, const string& setting_name)
{
   ...
   if (thisInfo.test_item_list.find(strtmp) != thisInfo.test_item_list.end()) {
      ...
      strtmp = CSeqEntry_test_on_quals::GetName_sq_oncall();
      if (thisInfo.test_item_list.find(strtmp) !=thisInfo.test_item_list.end()){
         GetReport_quals(c_item, strtmp);  
      } 
      else {
         GetReport_quals(c_item, setting_name);  
      }
   }
};

void CSeqEntry_DISC_MISSING_SRC_QUAL :: GetReport(CRef <CClickableItem> c_item)
{
   x_GetRep4DupSrcQualAndData(c_item, GetName());
};

void CSeqEntry_DISC_DUP_SRC_QUAL :: GetReport(CRef <CClickableItem> c_item)
{
   x_GetRep4DupSrcQualAndData(c_item, GetName());
};

void CSeqEntry_DISC_DUP_SRC_QUAL_DATA :: GetReport(CRef <CClickableItem> c_item)
{
   x_GetRep4DupSrcQualAndData(c_item, GetName());
};
...
bool CSeqEntry_test_on_biosrc :: x_HasUnnecessaryEnvironmental(const CBioSource& biosrc)
{
   ...
   string strtmp = biosrc.GetTaxname();
...
{
  bool has_collection_date, has_country, has_specific_host;
  has_collection_date = has_country = has_specific_host = false;
...
void CSeqEntry_test_on_biosrc ::RunTests(const CBioSource& biosrc, const string& desc, CConstRef <CObject> obj_ref, int idx)
{
  ...
  request_ls.push_back(rq);
  if (m_run_tmiss || m_run_tbad) {
     CTaxon3 taxon3;
     taxon3.Init();
     CRef <CTaxon3_reply> reply = taxon3.SendOrgRefList(request_ls);
     if (reply->GetReply().empty()) {
        NCBI_USER_THROW("Taxon3 does not reply anything");
     }
     else {
        ITERATE (list <CRef <CT3Reply> >, rit, reply->GetReply()) {
           if ( (*rit)->IsError()) {
              const CT3Error& err = (*rit)->GetError();
              if (err.GetLevel() == CT3Error::eLevel_error
                    && err.GetMessage() == "Organism not found"
                    && m_run_tmiss) {
              ...
              const COrg_ref& db_org = (*rit)->GetData().GetOrg();
              db_tax = db_org.CanGetTaxname()? db_org.GetTaxname() : kEmptyStr;
              if (original_tax != db_tax
                     || !DoTaxonIdsMatch(biosrc.GetOrg(), db_org)) {
  ...
  if (m_run_sp) {
    unsigned len;
    if (!original_tax.empty()) {
       len = original_tax.size();
       if (len >= 4 
             && CTempString(original_tax).substr(len-4) == " sp."
             && !NStr::EqualNocase(original_tax.substr(0, 11), "uncultured ")) {
  ...
  if (m_run_pcr && biosrc.CanGetPcr_primers()) {
     ...
     for (it = pcr_ls.begin(); it != pcr_ls.end(); it++) {
...
bool CSeqEntry_test_on_biosrc :: SamePrimerList(const list <CRef <CPCRPrimer> >& ls1, const list <CRef <CPCRPrimer> >& ls2)
{
   ...
   Str2Int::iterator it1, it2;
   for (it1 = map1.begin(), it2 = map2.begin(); it1 != map1.end(); it1++, it2++)
   {
     if (it1->first != it1->first || it1->second != it2->second) {
        return false;
     }
   }
   return true;
...
void CSeqEntry_test_on_biosrc :: IsFwdRevDataPresent(CRef <CPCRPrimer> primer, bool& has_seq, bool& has_name)
{
...
void CSeqEntry_DIVISION_CODE_CONFLICTS :: GetReport(CRef <CClickableItem> c_item)
{
   ...
   if (div2item.size() > 1) {
     ITERATE (Str2Strs, it, div2item) {
       AddSubcategory(c_item, GetName() + "$" + it->first, &(it->second), 
                   "bioseq", "division code " + it->first, e_HasComment, false);
     }
...
void CSeqEntry_DISC_MISSING_VIRAL_QUALS :: GetReport(CRef <CClickableItem> c_item)
{
   ...
   ITERATE (Str2Strs, it, qual2src) {
      AddSubcategory(c_item, GetName() + "$" + it->first, &(it->second), 
                "virus organism", "missing suggested qualifier " + it->first);
   } 
   RmvRedundancy(c_item->item_list, c_item->obj_list);
   unsigned cnt = c_item->item_list.size();
   c_item->description = GetIsComment(cnt, "virus organism") 
                          + GetNoun(cnt, "missing required qualifier");
};
...
void CSeqEntry_DISC_BACTERIA_SHOULD_NOT_HAVE_ISOLATE :: GetReport(CRef <CClickableItem> c_item)
{
   c_item->obj_list = thisInfo.test_item_objs[GetName()];
   c_item->description 
      = GetHasComment(c_item->item_list.size(), "bacterial biosrouce")
         + "isolate.";
};
...
bool CSeqEntry_DISC_HAPLOTYPE_MISMATCH :: SubSeqsMatch(CConstRef <CBioseq> seq1, unsigned beg1, CConstRef <CBioseq> seq2, unsigned beg2, unsigned& len, bool allow_Ndiff)
{
   ...
   if (authors.CanGetAffil()) {
     ...
     if (affil.IsStd()) {
        if (affil.GetStd().CanGetAffil()) {
             ...
             if (!strtmp.empty()) {
                 m_submit_text.push_back(strtmp);
             }
        }
...
void CSeqEntry_DISC_HAPLOTYPE_MISMATCH :: MakeCitem4SameSeqs(CRef <CClickableItem> c_item, const vector <string>& idx_seqs, const string& match_tp, bool Ndiff)
{
   ...
   string strtmp = (string)"identical " 
                     + (Ndiff ? "(allowing N to match any)" : "(strict match)")
...
void CSeqEntry_test_on_pub :: TestOnObj(const CSeq_entry& seq_entry)
{
   ...
   if (thisInfo.seq_submit.NotEmpty()) {
      ...
      if (m_run_dup && auths.CanGetAffil() 
          ...
          string strtmp = desc.substr(0, pos + 2) + "Cit-sub for " 
...
bool CSeqEntry_test_on_pub :: NameIsBad(CRef <CAuthor> auth_nm) 
{
