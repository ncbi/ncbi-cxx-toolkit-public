...
static const char* s_FindImpFeatType(const CImp_feat& imp)
{
    // keep sorted in ASCII-betical order
    static const char* allowed_types[] = {
        "-10_signal", "-35_signal", "3'UTR", "3'clip", "5'UTR", "5'clip", 
        "CAAT_signal", "CDS", "C_region", "D-loop", "D_segment", "GC_signal", "Import", 
        "J_segment", "LTR", "N_region", "RBS", "STS", "S_region", "Site-ref", 
        "TATA_signal", "V_region", "V_segment", 
        "allele", "attenuator", "centromere", "conflict", 
        "enhancer", "exon", "gap", "iDNA", "intron", "mat_peptide", "misc_RNA", 
        "misc_binding", "misc_difference", "misc_feature", "misc_recomb", "misc_signal", 
        "misc_structure", "mobile_element", "modified_base", "mutation", "old_sequence", 
        "operon", "oriT", "polyA_signal", "polyA_site", "precursor_RNA", "prim_transcript", 
        "primer_bind", "promoter", "protein_bind", "regulatory", "rep_origin", "repeat_region", 
        "repeat_unit", "satellite", "sig_peptide", "source", "stem_loop", "telomere", "terminator", 
    ...
    if (! RAW_FIELD_IS_EMPTY_OR_UNSET(imp, Key)) {
        ...
        if (binary_search(allowed_types, allowed_types + kAllowedTypesNumElems, key, PCase_CStr())) {
            return key;
        }
    }
...
static const char* s_FindKeyFromFeatDefType(const CSeq_feat& feat)
{
    static const char*  kFeatBad = "???";
        ...
        if (fdata.GetRna().IsSetType()) {
            ...
            case NCBI_RNAREF(other):
                if (FIELD_IS_SET_AND_IS(rna, Ext, Name)) {
                    ...
                    if (NStr::EqualNocase(name, "misc_RNA"))
                        return "misc_RNA";
                    if (NStr::EqualNocase(name, "ncRNA"))
                        return "ncRNA";
                    if (NStr::EqualNocase(name, "tmRNA"))
                        return "tmRNA";
                }
                return "misc_RNA";
            default:
...
CGBQualCleanup::x_ProtGBQualBC(CProt_ref& prot, const CGb_qual& gb_qual, EGBQualOpt opt)
{
    ...
    static const char* const    ignored_quals[] = { "label", "allele", "experiment", "inference", "UniProtKB_evidence", "dbxref", "replace", "rpt_unit_seq", "rpt_unit_range" };
    ...
    static CMutex ignored_quals_raw_initialization_mutex;
    {
        ...
        if (ignored_quals_raw.empty()) {
            copy(ignored_quals, ignored_quals + sizeof(ignored_quals) / sizeof(ignored_quals[0]), inserter(ignored_quals_raw, ignored_quals_raw.begin()));
        }
    }
...
CGBQualCleanup::x_SeqFeatCDSGBQualBC(CSeq_feat& feat, CCdregion& cds, const CGb_qual& gb_qual)
{
    ...
    if (NStr::Equal(qual, "product") || NStr::Equal(qual, "function") || NStr::EqualNocase(qual, "EC_number") || NStr::Equal(qual, "prot_note")) {
