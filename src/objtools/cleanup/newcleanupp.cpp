...
// Constructor
CNewCleanup_imp::CNewCleanup_imp(CRef<CCleanupChange> changes, CScope& scope, Uint4 options) :
    m_Changes(changes),
    m_Options(options)
{
...
void CNewCleanup_imp::BasicCleanupSeqEntry(
    CSeq_entry& se)
{
    se.Parentize(); // This is what the old code does
                    // Is it still needed?
    auto seh = m_Scope->GetSeq_entryHandle(se, CScope::eMissing_Null);
    if (! seh) {
        seh = m_Scope->AddTopLevelSeqEntry(se);
    }
    BasicCleanupSeqEntryHandle(seh);
}
...
void CNewCleanup_imp::BasicCleanupSeqAnnot(
    CSeq_annot& sa)
{
    auto sah = m_Scope->AddSeq_annot(sa, CScope::kPriority_Default, CScope::eExist_Get);
    BasicCleanupSeqAnnotHandle(sah);
}
...
void CNewCleanup_imp::BasicCleanupBioseqSet(
    CBioseq_set& bss)
{
    auto bssh = m_Scope->GetBioseq_setHandle(bss);
    BasicCleanupBioseqSetHandle(bssh);
}
...
{
    SetGlobalFlags(*(seh.GetCompleteSeq_entry()));
    CAutogeneratedCleanup auto_cleanup(*m_Scope, *this);
    auto_cleanup.BasicCleanupSeqEntryHandle(seh);
    xUpdateSeqfeatCitPub(seh);

    for (CBioseq_CI bioseq_it(seh); bioseq_it; ++bioseq_it) {
        SetGeneticCode(*bioseq_it);
    }
}


void CNewCleanup_imp::BasicCleanupBioseqHandle(const CBioseq_Handle& bsh)
{
    SetGlobalFlags(*(bsh.GetCompleteBioseq()));
    CAutogeneratedCleanup auto_cleanup(*m_Scope, *this);
    auto_cleanup.BasicCleanupBioseqHandle(bsh);

    xUpdateSeqfeatCitPub(bsh);
    SetGeneticCode(bsh);
}
...
{
    SetGlobalFlags(*(bssh.GetCompleteBioseq_set()));
    CAutogeneratedCleanup auto_cleanup(*m_Scope, *this);
    auto_cleanup.BasicCleanupBioseqSetHandle(bssh);

    xUpdateSeqfeatCitPub(bssh);
    for (CBioseq_CI bioseq_it(bssh); bioseq_it; ++bioseq_it) {
        SetGeneticCode(*bioseq_it);
    }
}
    ...
    CSeq_annot_Handle& sah)
{
    ResetGlobalFlags();
    CAutogeneratedCleanup auto_cleanup(*m_Scope, *this);
    auto_cleanup.BasicCleanupSeqAnnotHandle(sah);
    xUpdateSeqfeatCitPub(sah);
}
...
void CNewCleanup_imp::SetGeneticCode(const CBioseq_Handle& bsh)
{
    if (! m_SyncGenCodes)
        return;
...
void CNewCleanup_imp::StripSpacesMarkChanged(string& str)
{
    if (StripSpaces(str)) {
...
void CNewCleanup_imp::RemoveSpacesBetweenTildesMarkChanged(std::string& str)
{
...
void CNewCleanup_imp::TrimInternalSemicolonsMarkChanged(std::string& str)
{
    const size_t old_str_size = str.length();
...
void CNewCleanup_imp::SetClass(const CBioseq_set_Handle& bssh)
{
    if (bssh.IsSetClass() &&
        (bssh.GetClass() != CBioseq_set::eClass_not_set &&
         bssh.GetClass() != CBioseq_set::eClass_other)) {
        return;
    }

    if (s_LooksLikeNucProtSet(*(bssh.GetCompleteBioseq_set()))) {
        bssh.GetEditHandle().SetClass(CBioseq_set::eClass_nuc_prot);
    } else {
        bssh.GetEditHandle().SetClass(CBioseq_set::eClass_genbank);
    }
...
static bool s_UpdatePartialness(const CSeq_feat& feat)
{ // return true if feature is marked as partial but location is complete
    return feat.IsSetPartial() && feat.GetPartial() && feat.IsSetLocation() &&
           (! feat.GetLocation().IsPartialStart(eExtreme_Biological)) &&
           (! feat.GetLocation().IsPartialStop(eExtreme_Biological));
}


void CNewCleanup_imp::ProtSeqBC(const CBioseq_Handle& bsh)
{
    // Pre-load segments
    ...
    auto        it         = find_if(id_handles.begin(), id_handles.end(), [](auto handle) { return (handle.GetSeqId() && handle.GetSeqId()->IsGibbsq()); });
...
bool CNewCleanup_imp::BasicCleanupSeqIds(const CBioseq_Handle& bsh)
{
    if (! bsh.IsSetId()) {
        return false;
    }
...
static int ncbi_lexicographical_compare_3way(
    Iter1 first1, Iter1 last1, Iter2 first2, Iter2 last2, Compare compare)
{
...
static int s_CompareNoCaseCStyle(const string& s1, const string& s2)
{
    return ncbi_lexicographical_compare_3way(
        s1.begin(), s1.end(), s2.begin(), s2.end(), PNocase_CompareChar());
}

static const string& s_GenomeToPlastidName(const CBioSource& biosrc)
{
...
static bool s_RemoveInitial(string& str, const string& prefix, NStr::ECase case_to_use)
{
...

{
    ...
    if (gbk.IsSetKeywords()) {
        for (auto& keyword : gbk.SetKeywords()) {
            if (ctre::match<"^tpa(?:_|[_:]re)assembly$", ctre::case_insensitive>(keyword)) {
...
static bool s_StringHasOrgModPrefix(const string& str, string::size_type& out_val_start_pos, TORGMOD_SUBTYPE& out_subtype)
{
    SIZE_TYPE pos = str.find_first_of(": ="), pos2;
    if (pos != 0 && pos != NPOS && (pos2 = str.find_first_not_of(": =", pos)) != NPOS) {
        try {
...
// returns true if we found anything
static bool s_StringHasSubSourcePrefix(const string& str, string::size_type& out_val_start_pos, TSUBSOURCE_SUBTYPE& out_subtype)
{
    SIZE_TYPE pos = str.find_first_of(": ="), pos2;
    if (pos != 0 && pos != NPOS && (pos2 = str.find_first_not_of(": =", pos)) != NPOS) {
        try {
...

{
    ...
    if (FIELD_IS_SET(sbs2, Name)) {
        if (! FIELD_IS_SET(sbs1, Name))
            return true;
        if (s_CompareNoCaseCStyle(GET_FIELD(sbs1, Name), GET_FIELD(sbs2, Name)) < 0)
            return true;
    }
...

{
    ...
    if (FIELD_IS_SET(sbs1, Name) && FIELD_IS_SET(sbs2, Name)) {
        if (NStr::EqualNocase(GET_FIELD(sbs1, Name), GET_FIELD(sbs2, Name)))
            return true;
    }
    if (! FIELD_IS_SET(sbs1, Name) && ! FIELD_IS_SET(sbs2, Name))
        return true;
    ...
    CBioSource& biosrc)
{
    ...
    if (FIELD_IS_SET(biosrc, Subtype)) {
        ...
        EDIT_EACH_SUBSOURCE_ON_BIOSOURCE(it, biosrc)
        {
            ...
            if (CSubSource::NeedsNoText(chs)) {
                ...
                RemoveFlankingQuotes(GET_MUTABLE(sbs, Name));
                CLEAN_STRING_MEMBER(sbs, Attrib);
            }
            ...
            if (chs == NCBI_SUBSOURCE(altitude)) {
                ...
                static constexpr auto pattern = ctll::fixed_string{ "^([+\\-]?[0-9]+(\\.[0-9]+)?) ?(m|meter[s]?|metre[s]?)\\.?$" };

                if (auto match = ctre::match<pattern, ctre::case_insensitive>(altitude)) {
                    auto new_altitude = match.get<1>().to_string() + " m";
    ...
    PostBiosource(biosrc);
    if (biosrc.IsSetOrg()) {
        PostOrgRef(biosrc.SetOrg());
    }
}

void CNewCleanup_imp::PostBiosource(CBioSource& biosrc)
{
...

{
    ...
    if (! FIELD_IS_SET(dbt, Tag))
        return true;
    const CObject_id& oid = GET_FIELD(dbt, Tag);

    if (FIELD_IS(oid, Id)) {
        if (GET_FIELD(oid, Id) == 0)
            return true;
...
void CNewCleanup_imp::PostOrgRef(COrg_ref& org)
{
    EDIT_EACH_DBXREF_ON_ORGREF(it, org)
    {
        CDbtag& dbt = **it;
...

{
    ...
    const string& subname1 = GET_FIELD(omd1, Subname);
    const string& subname2 = GET_FIELD(omd2, Subname);
    if (! NStr::EqualNocase(subname1, subname2))
        return false;

    const string& attrib1 = (FIELD_IS_SET(omd1, Attrib) ? GET_FIELD(omd1, Attrib) : kEmptyStr);
    const string& attrib2 = (FIELD_IS_SET(omd2, Attrib) ? GET_FIELD(omd2, Attrib) : kEmptyStr);
    if (! NStr::EqualNocase(attrib1, attrib2))
        return false;
    ...
    EDIT_EACH_ORGMOD_ON_ORGNAME(it, onm)
    {
        ...
        if (omd.GetSubtype() == NCBI_ORGMOD(other)) {
            ...
            try {
                ...
                {
                    if ((*match_it)->GetSubtype() == subtype && NStr::EqualCase((*match_it)->GetSubname(), otherval)) {
                        do_erase = true;
                        break;
                    }
                }
...

{
    ...
    return ct::search_replace_all<"[ ]*:[ ]*", ":">(str);
}
    ...
    COrgMod& omd)
{
    ...
    if (FIELD_IS_SET(omd, Subname)) {
        TrimInternalSemicolonsMarkChanged(GET_MUTABLE(omd, Subname));
        RemoveFlankingQuotes(GET_MUTABLE(omd, Subname));
    }
...
{
    if (! FIELD_IS_SET(dbtag, Db))
        return;
    if (! FIELD_IS_SET(dbtag, Tag))
        return;
    ...
    if (NStr::EqualNocase(db, "Swiss-Prot") || NStr::EqualNocase(db, "SWISSPROT") || NStr::EqualNocase(db, "UniProt/Swiss-Prot")) {
    ...
    if (NStr::EqualNocase(db, "HPRD") && NStr::StartsWith(str, "HPRD_")) {
        ...
        if (NStr::StartsWith(str, "MGI:")) {
            ...
            */
            ...
            if (CCleanupUtils::IsAllDigits(str.substr(2))) {
    ...
    if (CCleanupUtils::IsAllDigits(str) && !NStr::StartsWith(str, "0")) {
        try {
...
void CNewCleanup_imp::LabelMapAppend(
    const vector<string>& old_labels,
    const vector<string>& new_labels)
{
    for (vector<string>::size_type index = 0; index < old_labels.size(); ++index) {
        if ((! new_labels[index].empty()) &&
            (new_labels[index] != old_labels[index])) {
            m_OldLabelToNewLabel.emplace(old_labels[index], new_labels[index]);
        }
    }
}
...
static bool cmpSortedvsOld(const TCit& e1, const CRef<CPub>& e2)
{
    return e1.second == e2;
}
...
//
{
    ...
    static const string* endIntegronValues = integronValues + sizeof(integronValues) / sizeof(*integronValues);
...
void CNewCleanup_imp::ExpandCombinedQuals(CSeq_feat::TQual& quals)
{
...
void CNewCleanup_imp::AddToComment(CSeq_feat& feat, const string& comment)
{
...
void CNewCleanup_imp::x_AddEnvSamplOrMetagenomic(CBioSource& biosrc)
{
    ...
    if (org.IsSetOrgname()) {
        ...
        if (orgname.IsSetDiv() && NStr::Equal(biosrc.GetOrg().GetOrgname().GetDiv(), "ENV")) {
            needs_env_sample = true;
        }
...
void CNewCleanup_imp::BothStrandBC(CSeq_loc& loc)
{
    switch (loc.Which()) {
    case CSeq_loc::e_Int:
...
inline static bool s_CodonCompare(const int& codon1, const int& codon2)
{
    return (codon1 < codon2);
}

inline static bool s_CodonEqual(int codon1, int codon2)
{
    return (codon1 == codon2);
}

static char s_ConvertTrnaAaToLetter(const CTrna_ext::C_Aa& trna_aa, CSeqUtil::ECoding coding, char* out_aa_char = nullptr)
{
    char temp_aa = '\0';
...
void CNewCleanup_imp::SeqFeatTRNABC(CTrna_ext& tRNA)
{
...
static void s_ParsePCRComponent(vector<string>& out_list, const string* component)
{
    out_list.clear();
...
static void s_ParsePCRSet(const CBioSource& biosrc, list<CPCRParsedSet>& out_pcr_set)
{
    out_pcr_set.clear();
...
static void s_ParsePCRColonString(vector<string>& out_list, const string& str)
{
...
static CRef<CPCRPrimerSet> s_ModernizePCRPrimerHalf(const string& seq, const string& name)
{
...
void CNewCleanup_imp::ModernizePCRPrimers(CBioSource& biosrc)
{
...
static void s_SplitAtSingleTildes(list<string>& piece_vec, const string& str)
{
    if (str.empty()) {
        return;
    }
    ...
    while (search_pos < str.length()) {
        ...
        const bool tilde_is_usable = ((tilde_pos == 0 || str[tilde_pos - 1] != ' ') &&
...
static bool s_CleanupOrgModAndSubSourceOther_helper(
    string&                      subname,
...
void CNewCleanup_imp::CleanupOrgModAndSubSourceOther(COrgName& orgname, CBioSource& biosrc)
{
...
void CNewCleanup_imp::x_OrgnameModBC(COrgName& orgname, const string& org_ref_common)
{
...
void CNewCleanup_imp::SubSourceBC(CSubSource& subsrc)
{
...
void CNewCleanup_imp::OrgModBC(COrgMod& orgmod)
{
...
void CNewCleanup_imp::FixUnsetMolFromBiomol(CMolInfo::TBiomol biomol, const CBioseq_Handle& bsh)
{
    if (bsh.IsSetInst()) {
...
void CNewCleanup_imp::AddPartialToProteinTitle(const CBioseq_Handle& bsh)
{
    if (CCleanup::AddPartialToProteinTitle(bsh)) {
...
void CNewCleanup_imp::CleanupECNumber(string& ec_num)
{
...
void CNewCleanup_imp::CleanupECNumberList(CProt_ref::TEc& ec_num_list)
{
...
void CNewCleanup_imp::FixUpEllipsis(string& str)
{
    if (ct::search_replace_all<"[,.][,.][,.]$", "...">(str)) {
        ChangeMade(CCleanupChange::eChangeComment);
    }
}

void CNewCleanup_imp::RemoveFlankingQuotes(string& val)
{
    if (val.empty()) {
        return;
    }
...
static bool s_IsIllegalQual(const string& qual)
{
...

{
    ...
    if (NStr::CompareNocase(vl1, vl2) < 0)
        return true;
...

{
    ...
    if (! NStr::EqualNocase(ql1, ql2))
        return false;

    const string& vl1 = GET_FIELD(gbq1, Val);
    const string& vl2 = GET_FIELD(gbq2, Val);

    if (! NStr::EqualNocase(vl1, vl2))
        return false;
...
static bool s_SeqLocAnyNull(const CSeq_loc& loc)
{
...
CGBQualCleanup& CNewCleanup_imp::x_GetGBQualCleanup()
{
    if (! m_GBQualCleanup) {
        m_GBQualCleanup = make_unique<CGBQualCleanup>(*m_Scope, *this);
    }
    return *m_GBQualCleanup;
}
...
void CNewCleanup_imp::x_CleanSeqFeatQuals(CSeq_feat& sf)
{
    ...
    {
        CGb_qual& gbq = **gbq_it;
        if (x_GetGBQualCleanup().Run(gbq, sf) == CGBQualCleanup::eAction_Erase) {
    ...
    CSeq_feat& sf)
{
    ...
    if (! CSeqFeatData::AllowStrandBoth(sf.GetData().GetSubtype())) {
        BothStrandBC(sf.SetLocation());
    }
}

void CNewCleanup_imp::PostSeqFeat(CSeq_feat& sf)
{
    ...
    const unsigned int partial_loc_mask = (sequence::eSeqlocPartial_Start |
...

{
    ...
    if (! gene_syns_to_add.empty()) {
        copy(gene_syns_to_add.begin(), gene_syns_to_add.end(), back_inserter(gr.SetSyn()));
        ChangeMade(CCleanupChange::eChangeGeneRef);
    }
...

{
    if (FIELD_IS_SET(gr, Locus))
        return false;
    if (FIELD_IS_SET(gr, Allele))
        return false;
    if (FIELD_IS_SET(gr, Desc))
        return false;
    if (FIELD_IS_SET(gr, Maploc))
        return false;
    if (FIELD_IS_SET(gr, Db))
        return false;
    if (FIELD_IS_SET(gr, Syn))
        return false;
    if (FIELD_IS_SET(gr, Locus_tag))
        return false;
...
{
    if (STRING_FIELD_MATCH(gene_ref, Locus_tag, comm))
        return true;
    if (STRING_SET_MATCH(gene_ref, Syn, comm))
        return true;

    return false;
}

static CRef<CDbtag> s_DbtagParse(const string& dbtag_str)
{
    ...
    if (CCleanupUtils::IsAllDigits(id_str) &&
...
static CConstRef<CUser_object> s_FindUserObjectTypeRecursive(const CUser_object& user_obj, const string& sought_type_label);

static CConstRef<CUser_object> s_FindUserObjectTypeRecursive_helper(const CUser_field& field, const string& sought_type_label)
{
...

{
    ...
    if (prot_ref.IsSetName()) {
        for (auto& it : prot_ref.SetName()) {
            ProtNameBC(it);
            CompressStringSpacesMarkChanged(it);
        }
    }
    ...
    if (m_IsEmblOrDdbj) {
        EDIT_EACH_NAME_ON_PROTREF(it, prot_ref)
        {
            ...
            if (NStr::Find(*it, "ribulose") != string::npos && NStr::Find(*it, "bisphosphate") != string::npos && NStr::Find(*it, "methyltransferase") == string::npos && ! NStr::EqualNocase(*it, "ribulose-1,5-bisphosphate carboxylase/oxygenase large subunit") && ! NStr::EqualNocase(*it, "ribulose-1,5-bisphosphate carboxylase/oxygenase small subunit") && (NStr::EqualNocase(*it, "ribulose 1,5-bisphosphate carboxylase/oxygenase large subunit") || NStr::EqualNocase(*it, "ribulose 1,5-bisphosphate carboxylase large subunit") || NStr::EqualNocase(*it, "ribulose bisphosphate carboxylase large subunit") || NStr::EqualNocase(*it, "ribulose-bisphosphate carboxylase large subunit") || NStr::EqualNocase(*it, "ribulose-1,5-bisphosphate carboxylase large subunit") || NStr::EqualNocase(*it, "ribulose-1,5-bisphosphate carboxylase, large subunit") || NStr::EqualNocase(*it, "large subunit of ribulose-1,5-bisphosphate carboxylase/oxygenase") || NStr::EqualNocase(*it, "ribulose-1,5-bisphosphate carboxylase oxygenase large subunit") || NStr::EqualNocase(*it, "ribulose bisphosphate carboxylase large chain") || NStr::EqualNocase(*it, "ribulose 1,5-bisphosphate carboxylase-oxygenase large subunit") || NStr::EqualNocase(*it, "ribulose bisphosphate carboxylase oxygenase large subunit") || NStr::EqualNocase(*it, "ribulose 1,5 bisphosphate carboxylase large subunit") || NStr::EqualNocase(*it, "ribulose-1,5-bisphosphate carboxylase/oxygenase, large subunit") || NStr::EqualNocase(*it, "large subunit of ribulose-1,5-bisphosphate carboxylase/oxgenase") || NStr::EqualNocase(*it, "ribulose bisphosphate carboxylase/oxygenase large subunit") || NStr::EqualNocase(*it, "ribulose-1,5-bisphosphate carboxylase oxygenase, large subunit") || NStr::EqualNocase(*it, "ribulose 5-bisphosphate carboxylase, large subunit") || NStr::EqualNocase(*it, "ribulosebisphosphate carboxylase large subunit") || NStr::EqualNocase(*it, "ribulose bisphosphate large subunit") || NStr::EqualNocase(*it, "ribulose 1,5 bisphosphate carboxylase/oxygenase large subunit") || NStr::EqualNocase(*it, "ribulose 1,5-bisphosphate carboxylase/oxygenase large chain") || NStr::EqualNocase(*it, "large subunit ribulose-1,5-bisphosphate carboxylase/oxygenase") || NStr::EqualNocase(*it, "ribulose-bisphosphate carboxylase, large subunit") || NStr::EqualNocase(*it, "ribulose-1, 5-bisphosphate carboxylase/oxygenase large-subunit"))) {
                *it = "ribulose-1,5-bisphosphate carboxylase/oxygenase large subunit";
                ChangeMade(CCleanupChange::eChangeQualifiers);
            }
        }
...
{
    if (STRING_SET_MATCH(pr, Name, comm))
        return true;
    if (STRING_FIELD_MATCH(pr, Desc, comm))
        return true;
    if (STRING_SET_MATCH(pr, Ec, comm))
        return true;
...
{
    const TPROTREF_PROCESSED processed = (FIELD_IS_SET(pr, Processed) ? GET_FIELD(pr, Processed) : NCBI_PROTREF(not_set));
...
static bool s_NotExceptedRibosomalName(const string& name)
{
    // we are "not excepted" if there is a non-space/non-digit somewhere after " ribosomal"
    return ctre::search<" ribosomal.*[^ 0-9]">(name);
}


void CNewCleanup_imp::x_RRNANameBC(string& name)
{
    ...
    if (name.length() > 5 && s_NotExceptedRibosomalName(name)) {
        ...
        if (auto match = ctre::search<" (ribosomal|rRNA) ( ?RNA)?( ?DNA)?( ?ribosomal)?">(name)) {
            // extract suffix
            const auto suff_pos = distance(name.begin(), match.get<0>().end());
            string     suff     = name.substr(suff_pos); // returns an empty string of suff_pos == name.length().

            NStr::TruncateSpacesInPlace(suff);

            // cut ribosomal stuff off of name
            const auto ribosomal_pos = distance(name.begin(), match.get<0>().begin());
    ...
    do {
        StripSpacesMarkChanged(name);
    } while (ct::search_replace_all<"ribosomal +ribosomal", "ribosomal ">(name) ||
             ct::search_replace_all<"RNA +RNA", "RNA ">(name) ||
             ct::search_replace_all<"ribosomal +RNA +ribosomal", "ribosomal RNA ">(name) ||
             ct::search_replace_all<"ribosomal +rRNA", "ribosomal RNA ">(name) ||
             ct::search_replace_all<"RNA +rRNA", "RNA ">(name));
...
void CNewCleanup_imp::x_AddNonCopiedQual(
...
void CNewCleanup_imp::GBQualToOrgRef(COrg_ref& org, CSeq_feat& seqfeat)
{
...
void CNewCleanup_imp::CleanupStringMarkChanged(std::string& str)
{
...
void CNewCleanup_imp::CleanupStringJunkMarkChanged(std::string& str)
{
    if (CleanVisStringJunk(str)) {
        ChangeMade(CCleanupChange::eTrimSpaces);
    }
...
bool CNewCleanup_imp::CompressSpaces(string& str)
{
    return Asn2gnbkCompressSpaces(str);
}

void CNewCleanup_imp::CompressStringSpacesMarkChanged(std::string& str)
{
    const string::size_type old_length = str.length();
...
void CNewCleanup_imp::ConvertDoubleQuotesMarkChanged(std::string& str)
{
...
static bool s_IsEmpty(const CTrna_ext& trna)
{
    if (trna.IsSetAa())
        return false;
    if (trna.IsSetCodon() && trna.GetCodon().size() > 0)
        return false;
...

{
    ...
    if ((! FIELD_IS_SET(rna, Ext) || FIELD_IS(rna.GetExt(), Gen)) &&
        ...
        string codon;
        char   aa = CCleanupUtils::ParseSeqFeatTRnaString(GET_FIELD(seq_feat, Comment),
                                                        &justTrnaText,
                                                        codon,
                                                        true);
        if (aa != '\0') {
    ...
    if (STRING_FIELD_NOT_EMPTY(seq_feat, Comment) &&
        ...
        if (comment.length() == 1 && comment[0] == aa) {
            ...
            const auto& trna_inverse_keys = g_GetTrnaInverseKeys();
            // find the letter, 3-letter, and full name of the given aa (amino acid)
            auto        aa_iter           = trna_inverse_keys.lower_bound(aa);
            auto        aa_end            = trna_inverse_keys.upper_bound(aa);
...
public:
    CCodeBreakCompare(const CSeq_loc& seq_feat_location, CRef<CScope> scope) :
        m_Seq_feat_location(seq_feat_location),
        m_Scope(scope)
    {
    }
    ...
    bool operator()(const CRef<CCode_break> break1, const CRef<CCode_break> break2) const
    {
        ...
        TSeqPos seq_pos1 =
            sequence::LocationOffset(m_Seq_feat_location, loc1, sequence::eOffset_FromStart, &*m_Scope);
        TSeqPos seq_pos2 =
            sequence::LocationOffset(m_Seq_feat_location, loc2, sequence::eOffset_FromStart, &*m_Scope);
...
static void s_MoveNonDuplicatedItems(TDest& dest, TSrc& src, const TLessThan& less_than, EMoveNonDuplicatedItemsOpt opt)
{
    ...
    copy(dest.begin(), dest.end(), inserter(dest_items_set, dest_items_set.end()));
...
void s_CopyProtXrefToProtFeat(CProt_ref& prot_ref, CProt_ref& cds_prot_ref)
{
    ...
    if (FIELD_IS_SET(cds_prot_ref, Db)) {
        copy(GET_FIELD(cds_prot_ref, Db).begin(), GET_FIELD(cds_prot_ref, Db).end(), back_inserter(GET_MUTABLE(prot_ref, Db)));
...
void CNewCleanup_imp::x_PCRPrimerSetBC(CPCRPrimerSet& primer_set)
{
    ...
    EDIT_EACH_PCRPRIMER_IN_PCRPRIMERSET(primer_iter, primer_set)
    {
        ...
        if (FIELD_IS_SET(primer, Name)) {
            ...
            CleanupStringMarkChanged(name);
            CompressStringSpacesMarkChanged(name);
            if (before != name) {
                ChangeMade(CCleanupChange::eChangePCRPrimers);
            }
            if (name.empty()) {
...
class CCopyGenbankDivToOrgname
{
public:
    template <class T, typename = enable_if_t<is_same_v<T, CBioseq_Handle> || is_same_v<T, CBioseq_set_Handle>, T>>
    static bool Apply(const T& handle)
    {
        if (handle.IsSetDescr()) {
            if (auto desc_pair = x_GetOrgnameDescs(handle.GetDescr())) {
                handle.GetEditHandle().ReplaceSeqdesc(*(desc_pair->first), *(desc_pair->second));
                return true;
            }
        }
        return false;
...
private:
    using TDescPair = pair<CConstRef<CSeqdesc>, CRef<CSeqdesc>>;
    static optional<TDescPair> x_GetOrgnameDescs(const CSeq_descr& descr);

    template <class T, typename = enable_if_t<is_same_v<T, CGB_block> || is_same_v<T, COrgName>, T>>
    static bool x_HasValidDiv(const T& obj)
    {
        return (obj.IsSetDiv() && (! obj.GetDiv().empty()));
    }
};


optional<CCopyGenbankDivToOrgname::TDescPair> CCopyGenbankDivToOrgname::x_GetOrgnameDescs(const CSeq_descr& descr)
{
    if (! descr.IsSet() || descr.Get().empty()) {
        return nullopt;
    }

    CConstRef<CSeqdesc>  pOrgnameDesc;
    CConstRef<COrgName>  pOrgname;
    CConstRef<CGB_block> pGBblock;

    for (auto pDesc : descr.Get()) {
        if (pDesc->IsGenbank()) {
            pGBblock = &(pDesc->GetGenbank());
        } else if (pDesc->IsOrg() && pDesc->GetOrg().IsSetOrgname()) {
            pOrgnameDesc = pDesc;
            pOrgname     = &(pOrgnameDesc->GetOrg().GetOrgname());
        } else if (pDesc->IsSource() && pDesc->GetSource().IsSetOrg() &&
                   pDesc->GetSource().GetOrg().IsSetOrgname()) {
            pOrgnameDesc = pDesc;
            pOrgname     = &(pOrgnameDesc->GetSource().GetOrg().GetOrgname());
        }
    }

    if (pOrgname && pGBblock && x_HasValidDiv(*pGBblock) && (! x_HasValidDiv(*pOrgname))) {
        auto pNewDesc = Ref(new CSeqdesc());
        pNewDesc->Assign(*pOrgnameDesc);
        if (pNewDesc->IsOrg()) {
            pNewDesc->SetOrg().SetOrgname().SetDiv() = pGBblock->GetDiv();
        } else {
            pNewDesc->SetSource().SetOrg().SetOrgname().SetDiv() = pGBblock->GetDiv();
        }

        return optional<TDescPair>{ make_pair(pOrgnameDesc, pNewDesc) };
    }
...
void CNewCleanup_imp::CopyGBBlockDivToOrgnameDiv(const CSeq_entry_Handle& entry_handle)
{
    bool change_made{ false };
    if (entry_handle.IsSeq()) {
        change_made = CCopyGenbankDivToOrgname::Apply(entry_handle.GetSeq());
    } else {
        change_made = CCopyGenbankDivToOrgname::Apply(entry_handle.GetSet());
    }
...
void CNewCleanup_imp::AuthListBCWithFixInitials(CAuth_list& al)
{
    CCleanup::CleanupAuthList(al, true);
}


static bool s_ConvertMuidToPmid(CPub& pub, const map<TEntrezId, TEntrezId>& muid_to_pmid)
{
    if (pub.IsMuid()) {
        const auto muid = pub.GetMuid();
        auto       it   = muid_to_pmid.find(muid);
        if (it != muid_to_pmid.end()) {
            const auto pmid = it->second;
            pub.SetPmid().Set(pmid);
            return true;
        }
    } else if (pub.IsEquiv() && pub.GetEquiv().IsSet()) {
        bool result{ false };
        for (auto pPub : pub.SetEquiv().Set()) {
            result |= s_ConvertMuidToPmid(*pPub, muid_to_pmid);
        }
        return result;
    }
    return false;
...
void CNewCleanup_imp::xUpdateSeqfeatCitPub(const CBioseq_Handle& bsh)
{
    CSeq_annot_CI annot_it(bsh.GetParentEntry(), CSeq_annot_CI::eSearch_entry);
    for (; annot_it; ++annot_it) {
        xUpdateSeqfeatCitPub(*annot_it);
    }
}

void CNewCleanup_imp::xUpdateSeqfeatCitPub(const CBioseq_set_Handle& bssh)
{
    CSeq_annot_CI annot_it(bssh, CSeq_annot_CI::eSearch_recursive);
    for (; annot_it; ++annot_it) {
        xUpdateSeqfeatCitPub(*annot_it);
    }
}

void CNewCleanup_imp::xUpdateSeqfeatCitPub(const CSeq_entry_Handle& seh)
{
    CSeq_annot_CI annot_it(seh, CSeq_annot_CI::eSearch_recursive);
    for (; annot_it; ++annot_it) {
        xUpdateSeqfeatCitPub(*annot_it);
    }
}
...
bool CNewCleanup_imp::xUpdateSeqfeatCitGenLabel(CPub& pub)
{
    bool change_made{ false };

    if (pub.IsGen() && pub.GetGen().IsSetCit()) {
        const auto& cit = pub.GetGen().GetCit();
        if (auto it = m_OldLabelToNewLabel.find(cit); it != m_OldLabelToNewLabel.end()) {
            pub.SetGen().SetCit(it->second);
            change_made = true;
        } else if ((cit.length() > 1) && NStr::EndsWith(cit, ">")) {
            string cit_copy = cit;
            cit_copy.resize(cit_copy.length() - 1); // chop off final ">"
            // s_RegexpReplace(cit_copy, "Unpublished[ ]+", "Unpublished", 1);
            ct::search_replace_one<"Unpublished[ ]+", "Unpublished">(cit_copy);

            // check if the cit is a strict prefix of any of the cit-gen labels from before
            for (const auto& label : m_PubdescCitGenLabelVec) {
                if ((label.length() > cit_copy.length()) && NStr::StartsWith(label, cit_copy)) {
                    pub.SetGen().SetCit(label);
                    change_made = true;
                    break;
                }
            }
        ...
        for (auto pPub : pub.SetEquiv().Set()) {
            change_made |= xUpdateSeqfeatCitGenLabel(*pPub);
        }
    }
...
void CNewCleanup_imp::xUpdateSeqfeatCitPub(const CSeq_annot_Handle& annotHandle)
{
    if (m_MuidToPmidMap.empty() &&
        m_OldLabelToNewLabel.empty() &&
        m_PubdescCitGenLabelVec.empty()) {
        return;
    }

    if (! annotHandle.IsFtable()) {
        return;
    }

    for (CFeat_CI feat_it(annotHandle); feat_it; ++feat_it) {
        if (feat_it->IsSetCit() && feat_it->GetCit().IsPub()) {
            auto pFeat = Ref(new CSeq_feat());
            pFeat->Assign(*(feat_it->GetSeq_feat()));
            bool change_made{ false };
            for (auto pPub : pFeat->SetCit().SetPub()) {
                if (s_ConvertMuidToPmid(*pPub, m_MuidToPmidMap)) {
                    ChangeMade(CCleanupChange::eChangePublication);
                    change_made = true;
                }

                if (xUpdateSeqfeatCitGenLabel(*pPub)) {
                    ChangeMade(CCleanupChange::eCleanCitonFeat);
                    change_made = true;
                }
            }
            if (change_made) {
                CSeq_feat_EditHandle sfeh(*feat_it);
                sfeh.Replace(*pFeat);
            }
        }
    }
...
void CNewCleanup_imp::x_PostProcessing(void)
{
    ...
    if (! m_MuidPubContainer.empty()) {
        for (auto pPub : m_MuidPubContainer) {
            const auto muid     = pPub->GetMuid();
            // attempt to find that muid in the muid-to-pmid mapping created earlier
            auto       map_iter = m_MuidToPmidMap.find(muid);
            if (map_iter != m_MuidToPmidMap.end()) {
                const auto pmid = map_iter->second;
    ...
    if (! m_OldLabelToPubMap.empty() && ! m_PubToNewPubLabelMap.empty() &&
        ...
        for (auto pPub : m_SeqFeatCitPubContainer) {
            if (pPub->IsGen() && pPub->GetGen().IsSetCit()) {
                CCit_gen&     gen    = pPub->SetGen();
                const string& cit    = gen.GetCit();
                auto          iter   = m_OldLabelToPubMap.lower_bound(cit);
                auto          finish = m_OldLabelToPubMap.upper_bound(cit);
...
void CNewCleanup_imp::ClearEmptyDescr(const CBioseq_Handle& bsh)
{
...
void CNewCleanup_imp::ClearEmptyDescr(const CBioseq_set_Handle& bssh)
{
    if (bssh.IsSetDescr() && bssh.GetDescr().Get().empty()) {
        bssh.GetEditHandle().ResetDescr();
    }
}
...
bool CNewCleanup_imp::RemoveSingleStrand(const CBioseq_Handle& bsh)
{
...
void CNewCleanup_imp::NotePubdescOrAnnotPubs(
    const CPub_equiv& pub_equiv)
{
    ...
    // equivalent.
    if ((muid != ZERO_ENTREZ_ID) && (pmid != ZERO_ENTREZ_ID)) {
...
void CNewCleanup_imp::RememberPubOldLabel(CPub& pub)
{
    string old_label;
...
void CNewCleanup_imp::RememberMuidThatMightBeConvertibleToPmid(CPub& pub)
{
    m_MuidPubContainer.push_back(CRef<CPub>(&pub));
}

void CNewCleanup_imp::RememberSeqFeatCitPubs(CPub& pub)
{
    switch (pub.Which()) {
    case NCBI_PUB(Equiv):
        // recurse into equivs
        ...
        {
            RememberSeqFeatCitPubs(**pub_iter);
        }
        break;
    default:
...
void CNewCleanup_imp::DecodeXMLMarkChanged(std::string& str)
{
...
int s_LegalNcbieaaValues[] = { 42, 65, 66, 67, 68, 69, 70, 71, 72, 73, 74, 75, 76, 77, 78, 79, 80, 81, 82, 83, 84, 85, 86, 87, 88, 89, 90 };
    ...
    if (bioseq_set.IsSetDescr() && bioseq_set.IsSetSeq_set() && ! bioseq_set.GetSeq_set().empty()) {
        ...
        if (! biosources.empty()) {
            for (auto pSubEntry : bioseq_set.SetSeq_set()) {
                auto& descrs = pSubEntry->IsSeq() ? pSubEntry->SetSeq().SetDescr().Set() : pSubEntry->SetSet().SetDescr().Set();
...
void CNewCleanup_imp::x_SetPartialsForProtein(CBioseq& seq, bool partial5, bool partial3, bool feat_partial)
{
    ...
    if (changed) {
        auto bsh = m_Scope->GetBioseqHandle(seq);
        AddPartialToProteinTitle(bsh);
    }
}
        ...
        if (f->IsSetQual()) {
            ...
            if (has_citation) {
                ...
                while (it != newqual.end()) {
                    bool do_remove = false;
                    if ((*it)->IsSetQual() && NStr::Equal((*it)->GetQual(), "citation")) {
                        if (! (*it)->IsSetVal() || ! CCleanupUtils::IsAllDigits((*it)->GetVal())) {
                            // just delete
                            do_remove = true;
                        } else {
...
void CNewCleanup_imp::SubmitblockBC(CSubmit_block& sb)
{
    if (sb.IsSetCit() && sb.GetCit().IsSetAuthors()) {
        AuthListBCWithFixInitials(sb.SetCit().SetAuthors());
    }
}
