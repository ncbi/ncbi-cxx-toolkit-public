...
void CNewCleanup_imp::xUpdateSeqfeatCitPub(CSeq_entry& entry)
{
    if (entry.IsSeq()) {
        xUpdateSeqfeatCitPub(entry.SetSeq());
    }
    else if (entry.IsSet()) {
        xUpdateSeqfeatCitPub(entry.SetSet());
    }
}


void CNewCleanup_imp::xUpdateSeqfeatCitPub(CBioseq& bioseq)
{
...
void CNewCleanup_imp::xUpdateSeqfeatCitPub(CBioseq_set& bioset)
{
    if (bioset.IsSetAnnot()) {
        for (auto pAnnot : bioset.SetAnnot()) {
            if (pAnnot) {
                xUpdateSeqfeatCitPub(*pAnnot);
            }
        }
    }

    if (bioset.IsSetSeq_set()) {
        for (auto pEntry : bioset.SetSeq_set()) {
            if (pEntry) {
                xUpdateSeqfeatCitPub(*pEntry);
            }
        }
    }
}
...
void CNewCleanup_imp::xUpdateSeqfeatCitPub(CSeq_annot& annot)
{
    ...
    for (auto pFeat : annot.SetData().SetFtable()) {
        if (pFeat->IsSetCit() && pFeat->GetCit().IsPub()) {
            bool                 change_made{ false };
