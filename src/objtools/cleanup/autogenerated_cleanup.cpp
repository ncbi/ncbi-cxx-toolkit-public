...
void CAutogeneratedCleanup::x_BasicCleanupBioseq_inst(CSeq_inst & inst)
{ // type Sequence
  if( inst.IsSetExt() ) {
    const auto repr = inst.IsSetRepr() ? 
          inst.GetRepr() :
          CSeq_inst::eRepr_not_set;
    x_BasicCleanupSeqExt(inst.SetExt(), repr);
  }
  if( inst.IsSetHist() ) {
    x_BasicCleanupSeqHist( inst.SetHist() );
  }
...
{ // type Sequence
  ...
  if (mBiomol.has_value()) { 
      m_NewCleanup.FixUnsetMolFromBiomol(*mBiomol, bioseq);
  }

  if(bioseq.IsSetInst()) {
    x_BasicCleanupBioseq_inst(bioseq.SetInst());
  }
