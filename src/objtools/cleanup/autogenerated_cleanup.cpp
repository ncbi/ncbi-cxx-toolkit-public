...
{ // type Sequence
    ...
    if (pBiosrc) {
        m_NewCleanup.x_CleanupOrgModAndSubSourceOther(arg0, *pBiosrc);
    }
...
{ // type Sequence
    if (arg0.IsSetSource()) {
        m_NewCleanup.x_SubSourceBC(arg0.SetSource());
    }
...
void CAutogeneratedCleanup::x_BasicCleanupBiosource(CBioSource& biosrc)
{
    ...
    if (biosrc.IsSetSubtype()) {
        for (auto pSubSource : biosrc.SetSubtype()) {
            m_NewCleanup.x_SubSourceBC(*pSubSource);
        }
    }
    ...
    m_NewCleanup.x_PostBiosource(biosrc);
    m_NewCleanup.x_ModernizePCRPrimers(biosrc);
}
