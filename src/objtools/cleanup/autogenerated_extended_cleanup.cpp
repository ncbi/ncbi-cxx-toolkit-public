...
{ // type Sequence
  if( arg0.IsSetAttrib() ) {
    m_NewCleanup.TrimInternalSemicolonsMarkChanged( arg0.SetAttrib() );
  }
  if( arg0.IsSetLineage() ) {
    m_NewCleanup.TrimInternalSemicolonsMarkChanged( arg0.SetLineage() );
  }
  if( arg0.IsSetName() ) {
...
{ // type Sequence
  ...
  if( arg0.IsSetLocus_tag() ) {
    m_NewCleanup.TrimInternalSemicolonsMarkChanged( arg0.SetLocus_tag() );
  }
  if( arg0.IsSetMaploc() ) {
    m_NewCleanup.TrimInternalSemicolonsMarkChanged( arg0.SetMaploc() );
  }
...
{ // type Sequence
  if( arg0.IsSetKey() ) {
    m_NewCleanup.TrimInternalSemicolonsMarkChanged( arg0.SetKey() );
  }
  if( arg0.IsSetLoc() ) {
    m_NewCleanup.TrimInternalSemicolonsMarkChanged( arg0.SetLoc() );
  }
...
{ // type Sequence
  if( arg0.IsSetComment() ) {
    m_NewCleanup.TrimInternalSemicolonsMarkChanged( arg0.SetComment() );
  }
...
void CAutogeneratedExtendedCleanup::ExtendedCleanupSeqFeat(const CSeq_feat_Handle& fh)
{ 

  auto new_feat = Ref(new CSeq_feat());
  new_feat->Assign(*(fh.GetSeq_feat()));

  CSeq_feat &arg0 = *new_feat;
  m_LastArg_ExtendedCleanupSeqFeat = &arg0;

  m_NewCleanup.BondEC( arg0 );
  m_NewCleanup.tRNACodonEC( arg0 );
  ...
  m_NewCleanup.MoveSeqfeatOrgToSourceOrg( arg0 );
  if( arg0.IsSetData() ) {
    x_ExtendedCleanupSeqFeatData( arg0.SetData() );
  }
  if( arg0.IsSetExcept_text() ) {
    m_NewCleanup.ExceptTextEC(arg0.SetExcept_text());
  }
  if( arg0.IsSetTitle() ) {
    m_NewCleanup.TrimInternalSemicolonsMarkChanged( arg0.SetTitle() );
  }
  if( arg0.IsSetXref() ) {
  ...
  CSeq_feat_EditHandle(fh).Replace(arg0);
...
void CAutogeneratedExtendedCleanup::x_FallBackSeqAnnotMethod(const CSeq_annot_Handle& annotHandle)
{
    auto* pAnnot = const_cast<CSeq_annot*>(annotHandle.GetCompleteSeq_annot().GetPointer());
    ExtendedCleanupSeqAnnot(*pAnnot);
    annotHandle.GetEditHandle().Update();
}


void CAutogeneratedExtendedCleanup::ExtendedCleanupSeqAnnotHandle(const CSeq_annot_Handle& annotHandle)
{
    if (annotHandle.Seq_annot_IsSetDesc()) {
        x_FallBackSeqAnnotMethod(annotHandle);
    }
    else if (annotHandle.IsFtable()) {
        m_NewCleanup.RemoveEmptyFeatures(annotHandle);
        
        for (CFeat_CI feat_it(annotHandle); feat_it; ++feat_it) {
            ExtendedCleanupSeqFeat(*feat_it);
        }
    }
}


void CAutogeneratedExtendedCleanup::x_ExtendedCleanupSeqAnnots(const CBioseq_Handle& bsh)
{
    CSeq_annot_CI annot_it(bsh.GetParentEntry(), CSeq_annot_CI::eSearch_entry);
    for (; annot_it; ++annot_it) {
        ExtendedCleanupSeqAnnotHandle(*annot_it);
    }
}
...
{ // type Sequence
  auto bsh = m_Scope.GetBioseqHandle(arg0);
  m_NewCleanup.ExtendProteinFeatureOnProteinSeq(bsh); 
  m_NewCleanup.MoveCitationQuals(bsh);
  m_NewCleanup.CreateMissingMolInfo(bsh);

  m_NewCleanup.ExtendSingleGeneOnMrna(bsh);
  m_NewCleanup.RemoveOldFeatures(bsh);
  m_NewCleanup.RemoveEmptyFeatureTables(bsh);

  if(bsh.HasAnnots()) {
    m_NewCleanup.MergeAdjacentFeatureTables(bsh);
    x_ExtendedCleanupSeqAnnots(bsh);
  }

  if( arg0.IsSetDescr() ) {
    m_NewCleanup.FixStructuredCommentKeywords(arg0.SetDescr()); // Modifies structured-comment keywords on GB-block descriptors
    m_NewCleanup.CleanupGenbankBlock( arg0 ); // Modifies GB-block descriptor and tech field on molinfo descriptor
    m_NewCleanup.RescueMolInfo( arg0 ); // Instantiate or update molinfo descriptor; remove redundant method and moltype descriptors.
...
{ // type Sequence
  ...
  if( arg0.IsSetDescr() ) {
    ...
    m_NewCleanup.CleanupGenbankBlock( arg0 );
    x_ExtendedCleanupSeqdescr( arg0.SetDescr() );
  }
...
void CAutogeneratedExtendedCleanup::x_ExtendedCleanupSeqAnnots(list<CRef<CSeq_annot>>& annots)
{
