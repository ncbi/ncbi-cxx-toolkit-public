...
const string extOutput("bedgraph");
const string extErrors("errors");
const string extKeep("new");
const string dirTestFiles("bedgraphwriter_test_cases");
// !!!
...
CBedGraphWriter* sGetWriter(CScope& scope, CNcbiOstream& ostr)
{
    return new CBedGraphWriter(scope, ostr);
}
