...
const string extOutput("psl");
const string extErrors("errors");
const string extKeep("new");
const string dirTestFiles("pslwriter_test_cases");
// !!!
...
void sUpdateCase(CDir& test_cases_dir, const string& test_name)
{
    ...
    *pI >> *pAnnot;
    ...
    cerr << "    Produced new PSL file " << output << "." << endl;
    cerr << " ... Done." << endl;
}
...
NCBITEST_INIT_CMDLINE(arg_descrs)
{
    ...
    arg_descrs->AddDefaultKey("update-case", "UPDATE_CASE",
        "Produce .asn1 and .error files from given name for new or updated test case.",
