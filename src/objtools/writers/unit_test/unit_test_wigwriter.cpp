...
const string extOutput("wig");
const string extErrors("errors");
const string extKeep("new");
const string dirTestFiles("wigwriter_test_cases");
// !!!
...
CWiggleWriter* sGetWriter(CScope& scope, CNcbiOstream& ostr)
{
    return new CWiggleWriter(ostr);
}
