...
{
    cerr << "Testing " << testInfo.mInFile.GetName() << " against " << testInfo.mOutFile.GetName() << " and " << testInfo.mErrorFile.GetName() << endl;
...
{
    arg_descrs->AddDefaultKey("test-dir", "TEST_FILE_DIRECTORY", "Set the root directory under which all test files can be found.", CArgDescriptions::eDirectory, dirTestFiles);
    arg_descrs->AddDefaultKey("update-case", "UPDATE_CASE", "Produce .asn and .error files from given name for new or updated test case.", CArgDescriptions::eString, "");
...
BOOST_AUTO_TEST_CASE(Test_RW_2238)
{
    string input_string{ ">Feature mSeq1\n"
                         "100 200 bogus_feat\n"
                         "            note    no such feature" };
    ...
    auto feat_filter = make_unique<CSimpleTableFilter>(ITableFilter::eAction_Disallowed);

    auto pAnnot = CFeature_table_reader::ReadSequinFeatureTable(*pLineReader, 0, &listener, feat_filter.get());

    BOOST_CHECK_EQUAL(listener.Count(), 1);
    string expected_msg =
        "On SeqId 'lcl|mSeq1', line 2, severity Warning: 'Unrecognized feature name', with feature name 'bogus_feat'";
