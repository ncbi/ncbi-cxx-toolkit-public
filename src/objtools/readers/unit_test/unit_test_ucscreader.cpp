...
*
* File Description:
*   UCSC-style region reader unit test.
*
* ===========================================================================
*/
...
void sUpdateCase(CDir& test_cases_dir, const string& test_name)
{
    ...
    CUCSCRegionReader reader;
    CNcbiIfstream ifstr(input.c_str());

    CRef<CSerialObject> annot;
    try {
        CStreamLineReader lr(ifstr);
        annot = reader.ReadObject(lr, &logger);
    }
    catch (...) {
        ifstr.close();
