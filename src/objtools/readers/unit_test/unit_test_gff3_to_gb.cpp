...
//  ----------------------------------------------------------------------------
void PostProcessAnnot(
    CSeq_annot& annot,
    CErrorLogger* pLogger)
//  ----------------------------------------------------------------------------
{
    static unsigned int startingLocusTagNumber = 1;
    static string locusTagPrefix = "LT";

    edit::CFeatTableEdit fte(
        annot, locusTagPrefix, startingLocusTagNumber, pLogger);
    fte.InferPartials();
    fte.GenerateMissingParentFeatures(true);
...
{   
    ...
    CGff3Reader reader(
        CGff3Reader::fGeneXrefs | 
        CGff3Reader::fGenbankMode);
    ...
    try {
        ...
        for (auto pAnnot: annots) {
            PostProcessAnnot(*pAnnot, &logger);
        }
    }
    catch (...) {
        ifstr.close();
