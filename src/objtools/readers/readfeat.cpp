...
static set<const char*, PCase_CStr>
    sc_SingleKeys{
...

{
    ...
    case eQual_transl_table:
        ...
        try {
            ...
            x_ProcessMsg(
                ILineError::eProblem_QualifierBadValue, eDiag_Error, kCdsFeatName, "transl_table", val);
            return true;
        }
        break;
...
bool CFeatureTableReader_Imp::ParseTrnaExtString(CTrna_ext&                         ext_trna,
                                                 const string&                      str,
                                                 const CSeq_id&                     seqid,
    ...
    if (NStr::StartsWith(normalized_string, "(pos:")) {
        ...
        if (pos_end != string::npos) {
            ...
            if (aa_start != string::npos) {
                ...
                size_t aa_length = (seq_start == NPOS) ? pos_str.size() - (aa_start + 3) : seq_start - (aa_start + 3);
    ...
    const string&   val)
{
    ...
    case CRNA_ref::eType_rRNA:
        ...
        case eQual_product: {
            ...
            if (exttype == CRNA_ref::C_Ext::e_TRNA)
                return false;
            tex.SetName(val);
            return true;
        }
    ...
    case CRNA_ref::eType_tRNA:
        ...
        case eQual_product: {
            ...
            if (aaval_it != sm_TrnaKeys.end()) {
                ...
                x_ProcessMsg(
                    ILineError::eProblem_QualifierBadValue, eDiag_Warning, "tRNA", "product", val);
            }
            return true;
        } break;
        case eQual_anticodon: {
            if (! ParseTrnaExtString(rrp.SetExt().SetTRNA(), val, *m_seq_id)) {
                x_ProcessMsg(
                    ILineError::eProblem_QualifierBadValue, eDiag_Error, "tRNA", "anticodon", val);
            }
            return true;
        } break;
    ...
    if ((subtype == CSeqFeatData::eSubtype_regulatory) ||
        ...
        if (qtype == eQual_regulatory_class) {
            if (val != "other") { // RW-374 "other" is a special case
                ...
                if (find(allowed_values.cbegin(), allowed_values.cend(), val) == allowed_values.cend()) {
                    return false;
                }
            }
            // okay
    ...
    const string& val)
{
    ...
    case eOrgRef_organelle: {
        ...
        if (g_iter != sm_GenomeKeys.end()) {
            ...
            x_ProcessMsg(
                ILineError::eProblem_QualifierBadValue, eDiag_Error, feat_name, "organelle", val);
        }
        return true;
    }
    case eOrgRef_div: {
    ...
    const TFlags      flags)
{
    ...
    for (TChoiceCI cds_choice_ci = cds_equal_range.first;
         cds_choice_ci != cds_equal_range.second;
         ++cds_choice_ci) {
        ...
        const string locus =
            pGeneXrefOnCDS->IsSetLocus() ? pGeneXrefOnCDS->GetLocus() : "";

        const string locus_tag =
            pGeneXrefOnCDS->IsSetLocus_tag() ? pGeneXrefOnCDS->GetLocus_tag() : "";
        ...
        {
            {
                ...
                if (locusGeneMatches.empty()) {
                    ...
                    set_intersection(
                        locusGeneMatches.begin(), locusGeneMatches.end(), locusTagGeneMatches.begin(), locusTagGeneMatches.end(), inserter(matchingGenes, matchingGenes.begin()));
                }
            }
        }
        ...
        if ((flags & CFeature_table_reader::fCreateGenesFromCDSs) != 0 &&
            ...
            if (FIELD_EQUALS(*cds_feat_ref, Partial, true))
                pNewGene->SetPartial(true);
    ...
    const string&   note)
{
    ...
    string comment = (sfp->CanGetComment()) ? sfp->GetComment() + "; " + note : note;
    ...
    const string&   val)
{
    ...
    if (qual != "note") {
        ...
        x_ProcessMsg(
            ILineError::eProblem_InvalidQualifier, eDiag_Warning, feat_name, qual, kEmptyStr, error_message);
    }
    return true;
}
...

{
    ...
    if (featType == CSeqFeatData::e_Biosrc) {
        ...
        if (o_iter != sm_OrgRefKeys.end()) {
            ...
            if (x_AddQualifierToBioSrc(sfdata, feat_name, rtype, val))
                return true;
        } else {
            ...
            if (s_iter != sm_SubSrcKeys.end()) {
                ...
                if (x_AddQualifierToBioSrc(sfdata, stype, val))
                    return true;
                ...
                if (m_iter != sm_OrgModKeys.end()) {
                    ...
                    if (x_AddQualifierToBioSrc(sfdata, mtype, val))
                        return true;
                }
            }
        }
    ...
    if (q_iter != sm_QualKeys.end()) {
        ...
        case CSeqFeatData::e_Gene:
            if (x_AddQualifierToGene(sfdata, qtype, val))
                return true;
            break;
        case CSeqFeatData::e_Cdregion:
            if (x_AddQualifierToCdregion(sfp, sfdata, qtype, val))
                return true;
            break;
        case CSeqFeatData::e_Rna:
            if (x_AddQualifierToRna(sfp, qtype, val))
                return true;
            break;
        case CSeqFeatData::e_Imp:
            if (x_AddQualifierToImp(sfp, sfdata, qtype, qual, val))
                return true;
            break;
        ...
        case eQual_inference: {
            ...
            if (! NStr::IsBlank(prefix)) {
                ...
                x_ProcessMsg(
                    ILineError::eProblem_QualifierBadValue, eDiag_Error, feat_name, qual, val);
            }
            return true;
        }
        case eQual_replace: {
        ...
        case eQual_transcript_id: {
            if (featType == CSeqFeatData::e_Rna &&
                ...
                CBioseq::TId ids;
                try {
                    CSeq_id::ParseIDs(ids, val, CSeq_id::fParse_ValidLocal | CSeq_id::fParse_PartialOK);
                } catch (CSeqIdException&) {
                    x_ProcessMsg(
                        ILineError::eProblem_QualifierBadValue, eDiag_Error, feat_name, qual, val, "Invalid transcript_id  : " + val);
                    return true;
                }
        ...
        case eQual_protein_id:
            ...
            if (featType == CSeqFeatData::e_Cdregion ||
                ...
                try {
                    ...
                    x_ProcessMsg(
                        ILineError::eProblem_QualifierBadValue, eDiag_Error, feat_name, qual, val, "Invalid protein_id  : " + val);
                    return true;
                }
...

{
    ...
    if (loc_info.is_point || start == stop) {
        ...
        if (loc_info.is_point) {
            ...
            if (stop != (start + 1)) {
                x_ProcessMsg(
                    ILineError::eProblem_BadFeatureInterval, eDiag_Warning, strFeatureName);
            }
        } else {
    ...
    ITableFilter*   filter)
{
    ...
    if (auto subtype = CSeqFeatData::SubtypeNameToValue(feat); subtype != CSeqFeatData::eSubtype_bad) {
        // check filter, if any
        // RW-2238: Only filter valid feature types
        if (filter) {
        ...
        CSeqFeatData::E_Choice typ = CSeqFeatData::GetTypeFromSubtype(subtype);
        ...
        if ((flags & CFeature_table_reader::fReportDiscouragedKey) != 0) {
            if (CSeqFeatData::IsDiscouragedSubtype(subtype)) {
                x_ProcessMsg(
    ...
    const ILineError::TVecOfLines& vecOfOtherLines)
{
    ...
    unique_ptr<CObjReaderLineException> pErr(
        CObjReaderLineException::Create(
            eSeverity, line_num, strErrorMessage, eProblem, m_real_seqid, strFeatureName, strQualifierName, strQualifierValue));
...
void CFeatureTableReader_Imp::x_GetPointStrand(const CSeq_feat& feat, CSeq_interval::TStrand& strand) const
{
    if (feat.IsSetLocation() && feat.GetLocation().IsMix()) {
        ...
        } else if (last.IsPnt() && last.GetPnt().IsSetStrand()) {
        ...
        for (auto pSeqLoc : feat.SetLocation().SetMix().Set()) {
            if (pSeqLoc->IsPnt()) {
                ...
                const auto old_strand =
                    seq_point.IsSetStrand() ? seq_point.GetStrand() : eNa_strand_plus;
                                              ...
                                              TFtable&         ftable)
{
    ...
    if (featData.GetSubtype() == CSeqFeatData::eSubtype_pub &&
        ...
        const int line_number = m_reader->AtEOF() ? x_GetLineNumber() : x_GetLineNumber() - 1;
    ...
    if (feat->IsSetLocation() && feat->GetLocation().IsMix()) {
        ...
        } else if (feat->GetLocation().GetMix().Get().size() == 1) {
    ...
    while (! m_reader->AtEOF()) {
        ...
        if (line[0] == '[') {
            ...
            if ((! feat.empty()) && loc_info.start_pos >= 0 && loc_info.stop_pos >= 0) {
                ...
                if (! (flags & CFeature_table_reader::fSuppressBadKeyWarning)) {
                    x_ProcessMsg(
                        ILineError::eProblem_FeatureBadStartAndOrStop, eDiag_Warning, feat);
                }
            }
        }
    }
...
void CFeatureTableReader_Imp::x_InitId(const CTempString& seq_id, const TFlags flags)
{
    if (! NStr::IsBlank(seq_id)) {
        CBioseq::TId ids;
        CSeq_id::ParseIDs(ids, seq_id, (flags & CFeature_table_reader::fAllIdsAsLocal) ? CSeq_id::fParse_AnyLocal : CSeq_id::fParse_Default);
...
{
    return m_reader ? static_cast<unsigned int>(m_reader->GetLineNumber()) : 0;
}
    ...
    const string&            seqid_prefix)
{
    ...
    if (seqid_prefix.empty()) {
        ...
        else if (NStr::StartsWith(orig_seqid, "lcl|")) {
...
void CFeature_table_reader::AddStringFlags(
...
struct SCSeqidCompare {
    inline bool operator()(const CSeq_id* left, const CSeq_id* right) const
    {
        return *left < *right;
    };
};
    ...
    ITableFilter*       filter)
{
    ...
    while (! reader.AtEOF()) {
        ...
        if (seq) {        // found a match
            ...
            ERR_POST_X(6, Warning << "ReadSequinFeatureTables: unable to find match for " << feat_id->AsFastaString());
...
CRef<CTrna_ext> CFeature_table_reader::ParseTrnaExtString(const string& ext, const CSeq_id& seqid, function<void(const string& msg)>* fpReportError)
{
...
bool CFeature_table_reader::ParseInitialFeatureLine(
    const CTempString& line_arg,
    CTempStringEx&     out_seqid,
    CTempStringEx&     out_annotname)
{
