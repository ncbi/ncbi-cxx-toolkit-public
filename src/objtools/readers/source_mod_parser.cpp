...
#include "mod_info.hpp"
    ...
    bool s_FindBrackets(const CTempString& str, size_t& start, size_t& stop, size_t& eq_pos)
    {
        size_t i = start;
        bool found = false;
    ...
    void s_AppendIfNonEmpty(string& s, const CTempString& o)
    {
        if (!o.empty())
        {
            if (!s.empty())
    ...
    size_t iMaxModsToParse )
{
        ...
        ++iModsFoundSoFar )
    {
        ...
        lb_pos = pos;
        if (s_FindBrackets(title, lb_pos, end_pos, eq_pos))
        {            
            ...
            s_AppendIfNonEmpty(stripped_title, skipped);
        ...
        { // rest of the title is unparsed
            ...
            s_AppendIfNonEmpty(stripped_title, rest);
            break;
        }
    }
...
void CSourceModParser::x_AddDescriptors(CBioseq& seq)
{
    ...
    {{
        ...
        // moltype, mol_type, tech, completeness, s_Mod_completedness
...
void CSourceModParser::x_ApplySeqInstMods(CSeq_inst& seq_inst)
{
    ...
    // molecule, mol, mol_type, mol_type
    x_AddMolType(seq_inst);
    // strand
    x_AddStrand(seq_inst);
    // secondar_accession, secondary_accessions - there could be multiple accessions
...
void CSourceModParser::x_ApplyOrgRefMods(CAutoInitRef<COrg_ref>& pOrgRef) 
{
    ...
    // handle orgmods
    static const auto& kModNameOrgSubtypeMap = s_InitSModOrgSubtypeMap();

    for(const auto & smod_orgsubtype : kModNameOrgSubtypeMap) {
        const auto& mod_name = smod_orgsubtype.first;
        const COrgMod::ESubtype e_subtype = smod_orgsubtype.second;
        if ((pMod = FindMod(mod_name)) != NULL) {
...
void CSourceModParser::x_GetSubtype(CBioSource::TSubtype& subtype) 
{
    const SMod* mod;
    // handle subsources
    static const auto& kModNameSubSrcSubtypeMap = s_InitSModSubSrcSubtypeMap();

    for( const auto & smod_subsrcsubtype : kModNameSubSrcSubtypeMap) {
        const auto& mod_name = smod_subsrcsubtype.first;
        if ((mod = FindMod(mod_name)) != NULL) {
            CRef<CSubSource> subsource(new CSubSource);
