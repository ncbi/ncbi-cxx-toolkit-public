...
CFindITSParser::CFindITSParser(const char* input, CSeq_entry_Handle tse) :
    m_istr(input),
    m_tse(tse)
{
...
CRef<CSeq_feat> CFindITSParser::x_ParseLine(const CTempString& line, CSeq_entry_Handle tse, CBioseq_Handle& bsh, bool& negative, string& msg)
{
    ...
    x_GetSpan(ssu, starts, stops, spans);
    x_GetSpan(its1, starts, stops, spans);
    x_GetSpan(r58S, starts, stops, spans);
    x_GetSpan(its2, starts, stops, spans);
    x_GetSpan(lsu, starts, stops, spans);
    ...
    case 0:
        comment = "does not contain rna label";
        break;
    case 1: {
    ...
    case 2:
        comment = "contains " + comments[0] + " and " + comments[1];
        break;
    default:
        comment = "contains " + comments[0];
        for (unsigned int j = 1; j < comments.size() - 1; j++)
            comment += ", " + comments[j];
        comment += ", and " + comments.back();
        break;
    }
    negative = strand == "1";
...
void CFindITSParser::x_GetSpan(const string& str, vector<int>& starts, vector<int>& stops, vector<bool>& spans)
{
    int            start{ 0 }, stop{ 0 };
    bool           span(false);
    vector<string> arr;
...
bool CFindITSParser::IsLengthTooLarge(const string& str, int max_length, int i, const vector<int>& starts, const vector<int>& stops, const vector<bool>& spans, int bioseq_length)
{
    if (spans[i]) {
        int start  = starts[i];
        int end    = stops[i];
