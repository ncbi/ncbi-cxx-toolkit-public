...
const CSeq_entry *ctx)
{
    ...
    FOR_EACH_SUBSOURCE_ON_BIOSOURCE(ssit, bsrc)
    {
        ...
        if ((*ssit)->IsRepliconSubSource() &&
            !CSubSource::IsRepliconSubSourceValid((*ssit)->GetName(), 
                                                  orgref.IsSetTaxname() ? orgref.GetTaxname() : kEmptyStr,
                                                  isViral)) {
            PostObjErr(eDiag_Error, eErr_SEQ_DESCR_BioSourceInconsistency,
                CSubSource::GetSubtypeName((*ssit)->GetSubtype()) + " value should start with letter or number",
                obj, ctx);
        }
