...
// demo for sequence validator
static bool DemoStrainCheckCallback1(const string& organism, const string& strain)
...
// demo for biosample validator - coerced to use vector-based request for testing
static bool DemoStrainCheckCallback2(const string& organism, const string& strain)

{
    CTaxon3 taxon3(CTaxon3::initialize::yes);
    auto responder = [&taxon3](const CRef<COrg_ref>& request)->CRef<CTaxon3_reply>
    {
        vector<CRef<COrg_ref>> rq;
        rq.push_back(request);
        CRef<CTaxon3_reply> reply = taxon3.SendOrgRefList(rq);
        return reply;
    };
    return CStrainRequest::StrainContainsTaxonInfo(organism, strain, responder);
}
    ...
    std::function<CRef<CTaxon3_reply>(const vector<CRef<COrg_ref>>&)> taxoncallback)
{
    ...
    if (StrainContainsTaxonInfo(organism, strain, taxoncallback)) {
        if (sd) {
            imp.PostObjErr(eDiag_Error, eErr_SEQ_DESCR_StrainContainsTaxInfo,
                "Strain '" + strain + "' contains taxonomic name information", *sd, &se);
        } else if (sf) {
            imp.PostObjErr(eDiag_Error, eErr_SEQ_DESCR_StrainContainsTaxInfo,
                        "Strain '" + strain + "' contains taxonomic name information", *sf, &se);
        } else {
            imp.PostErr(eDiag_Error, eErr_SEQ_DESCR_StrainContainsTaxInfo,
                        "Strain '" + strain + "' contains taxonomic name information", org);
        }
    }
}


void CStrainRequest::ExploreStrainsByTaxname(CTaxValidationAndCleanup& tval, CValidError_imp& imp, const CSeq_entry& se,
        std::function<CRef<CTaxon3_reply>(const vector<CRef<COrg_ref>>&)> taxoncallback)
{
    // get source descriptors
    FOR_EACH_DESCRIPTOR_ON_SEQENTRY(it, se)
    {
        const CRef<CSeqdesc> sd = (*it);
        if (sd->IsSource() && sd->GetSource().IsSetOrg()) {
            const COrg_ref& org = sd->GetSource().GetOrg();
            x_CheckOneStrain(tval, imp, org, sd, CRef<CSeq_feat>(), se, taxoncallback);
        }
    }
    // also get features
    FOR_EACH_ANNOT_ON_SEQENTRY(annot_it, se)
    {
        FOR_EACH_SEQFEAT_ON_SEQANNOT(feat_it, **annot_it)
        {
            const CRef<CSeq_feat> sf = (*feat_it);
            if (sf->IsSetData() && sf->GetData().IsBiosrc()
                && sf->GetData().GetBiosrc().IsSetOrg()) {
                const COrg_ref& org = sf->GetData().GetBiosrc().GetOrg();
                x_CheckOneStrain(tval, imp, org, CRef<CSeqdesc>(), sf, se, taxoncallback);
            }
        }
    }
    ...
    if (se.IsSet()) {
        FOR_EACH_SEQENTRY_ON_SEQSET(it, se.GetSet())
        {
            ExploreStrainsByTaxname(tval, imp, **it, taxoncallback);
        }
    }
}
        ...
        std::function<CRef<CTaxon3_reply>(const vector<CRef<COrg_ref>>&)> taxoncallback)
{
    ...
    // at least one strain in record returned a hit, check individual records by strain and taxname
    ExploreStrainsByTaxname(tval, imp, se, taxoncallback);
}
