...
CRef<CFlatFeature> CFeatureItemBase::Format(void) const
{
    ...
    m_FF.Reset(new CFlatFeature(GetKey(),
                                *new CFlatSeqLoc(*m_Loc, *m_Context), *m_Feat));
    x_AddQuals();
    x_FormatQuals();
    return m_FF;
}
...
void CFeatureItem::x_AddQuals(void) const
{
    ...
    if (m_Feat->IsSetProduct()) {
        if (m_IsProduct) {
            ...
            if ( !m_Context->IsProt() ) {
                ...
                x_AddQual(slot, new CFlatSeqDataQVal(m_Feat->GetProduct()));
            }
            try {
                ...
                if (data.IsCdregion()) {
                    x_AddQual(eFQ_protein_id, new CFlatSeqIdQVal(id));
                }
    ...
    if ( !data.IsGene() ) {
        ...
        if (gene_feat) {
            ...
            if ( !label.empty() ) {
                ...
                x_AddQual(eFQ_gene, new CFlatStringQVal(label));
            }
            ...
            if (gene.IsSetLocus_tag()  &&  gene.GetLocus_tag() != label) {
                x_AddQual(eFQ_locus_tag,
                          new CFlatStringQVal(gene.GetLocus_tag()));
            }
        }
    }
    if (m_Feat->IsSetQual()) {
    ...
    if (m_Feat->IsSetTitle()) {
        x_AddQual(eFQ_label, new CFlatLabelQVal(m_Feat->GetTitle()));
    }
    if (m_Feat->IsSetCit()) {
    ...
    if (m_Feat->IsSetPseudo()) {
        x_AddQual(eFQ_pseudo, new CFlatBoolQVal(m_Feat->GetPseudo()));
    }
    if (m_Feat->IsSetExcept_text()) {
        x_AddQual(eFQ_exception, new CFlatStringQVal(m_Feat->GetExcept_text()));
    }
    switch (data.Which()) {
...
void CFeatureItem::x_AddQuals(const CGene_ref& gene) const
{
    ...
    if (gene.IsSetLocus()  &&  !gene.GetLocus().empty() ) {
        x_AddQual(eFQ_gene, new CFlatStringQVal(gene.GetLocus()));
        got_name = true;
    }
    if (gene.IsSetDesc()  &&   !gene.GetDesc().empty() ) {
        x_AddQual(got_name ? eFQ_gene_desc : eFQ_gene,
                  new CFlatStringQVal(gene.GetDesc()));
        got_name = true;
    }
    ...
    if (gene.IsSetLocus_tag()) {
        x_AddQual(eFQ_locus_tag, new CFlatStringQVal(gene.GetLocus_tag()));
    }
}
...
void CFeatureItem::x_AddQuals(const CCdregion& cds) const
{
    ...
    if (prod) {
        ...
        x_AddQual(eFQ_cds_product, new CFlatStringQVal(label));
    }
    if (cds.IsSetFrame()) {
        x_AddQual(eFQ_codon_start, new CFlatIntQVal(cds.GetFrame()));
    }
    if (cds.IsSetCode()) {
        ...
        if ( id != 1  ||
             m_Context->GetFormat() == CFlatFileGenerator::eFormat_GBSeq) {
            ...
            if ( !std_ncbieaa.empty() ) {
                for (unsigned int i = 0;  i < ncbieaa.size();  ++i) {
                    if (ncbieaa[i] != std_ncbieaa[i]) {
                        x_AddQual(eFQ_codon, new CFlatCodonQVal(i, ncbieaa[i]));
                    }
                }
            }
        }
...
void CFeatureItem::x_AddQuals(const CProt_ref& prot) const
{
    bool got_name = false;
    ...
    if (prot.IsSetDesc()  &&   !prot.GetDesc().empty() ) {
        x_AddQual(got_name ? eFQ_prot_desc : eFQ_prot,
                  new CFlatStringQVal(prot.GetDesc()));
        got_name = true;
    }
    if (prot.IsSetSyn()  &&  !prot.GetSyn().empty() ) {
        ...
        if ( !got_name  &&  first != last) {
            x_AddQual(eFQ_prot, new CFlatStringQVal(*first));
            got_name = true;
            ++first;
        }
        while (first != last) {
            x_AddQual(eFQ_prot_syn, new CFlatStringQVal(*first));
            ++first;
        }
    }

    if (prot.IsSetAllele()  &&  !prot.GetAllele().empty() ) {
        x_AddQual(eFQ_prot_allele, new CFlatStringQVal(prot.GetAllele()));
    }
    if (prot.IsSetMaploc()  &&  !prot.GetMaploc().empty() ) {
        x_AddQual(eFQ_prot_map, new CFlatStringQVal(prot.GetMaploc()));
    }
    if (prot.IsSetDb()) {
        x_AddQual(eFQ_prot_xref, new CFlatXrefQVal(prot.GetDb()));
    }
    if (prot.IsSetLocus_tag()) {
        x_AddQual(eFQ_locus_tag, new CFlatStringQVal(prot.GetLocus_tag()));
    }
#endif
}
...
#define DO_NOTE(x) x_FormatNoteQual(eFQ_##x, #x)
    ...
    x_FormatNoteQual(eFQ_seqfeat_note, "note");
...
void CSourceFeatureItem::x_AddQuals(void) const
{
    const CSeqFeatData& data = m_Feat->GetData();
...
void CSourceFeatureItem::x_AddQuals(const COrg_ref& org) const
{
    {{
        string name;
        if (org.IsSetTaxname()) {
        ...
        if (org.IsSetCommon()) {
            ...
            x_AddQual(eSQ_common_name, new CFlatStringQVal(org.GetCommon()));
        }
        if ( !name.empty() ) {
            x_AddQual(eSQ_organism, new CFlatStringQVal(name));
        }
    }}
    if (org.IsSetDb()) {
...
void CSourceFeatureItem::x_AddQuals(const CBioSource& src) const
{
    x_AddQual(eSQ_organelle, new CFlatOrganelleQVal(src.GetGenome()));
...
#define DO_NOTE(x) x_FormatNoteQual(eSQ_##x, #x)
    if (m_WasDesc) {
        x_FormatNoteQual(eSQ_seqfeat_note, "note");
        DO_NOTE(orgmod_note);
...
CSourceFeatureItem::CSourceFeatureItem(const CBioSource& src,
                                               CFFContext& ctx)
    : CFeatureItemBase(*new CSeq_feat, ctx, ctx.GetLocation()),
      m_WasDesc(true)
{
...
CSourceFeatureItem::CSourceFeatureItem(const COrg_ref& org,
                                               CFFContext& ctx)
    : CFeatureItemBase(*new CSeq_feat, ctx, ctx.GetLocation()),
      m_WasDesc(true)
{
...
void CFeatureItem::x_FormatQual(EFeatureQualifier slot, const string& name,
