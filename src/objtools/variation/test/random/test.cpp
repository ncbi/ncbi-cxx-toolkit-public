...
private:
    ...
    bool ProcessHGVS(string &expression, CRef<CScope> scope, CHgvsReader &reader, int &num_tries, int &num_shifted, bool &is_deletion);
};
...
bool CAnalyzeShiftApp::ProcessHGVS(string &expression, CRef<CScope> scope, CHgvsReader &reader, int &num_tries, int &num_shifted, bool &is_deletion)
{
...
// cout <<  MSerial_AsnText << *a;
  ...
  for (int i=0; i<1000; i++)
  {
      ...
      if (type == CVariation_inst::eType_del && CVariationNormalization_base_cache::x_GetSeq(rand_left,ref.size()) != ref) continue;
      ...
      num_tries++;
      if (pos_left != rand_left || pos_right != rand_right)
          num_shifted++;
  }
...
int CAnalyzeShiftApp::Run() 
{
    ...
    while(!line_reader.AtEOF())
    {
        ...
        if (!line.empty())
        {
            ...
            int num_tries = 0;
            int num_shifted = 0;
            if (ProcessHGVS(hgvs,scope,reader,num_tries,num_shifted,is_deletion))
            {
                ostr <<snp_id<<"|"<<hgvs<<"|"<<num_tries<<"|"<<num_shifted<<"|";
                if (is_deletion)
                    ostr  << "deletion|";
                else
                    ostr << "insertion|";
