...
static void LogCallback(const CassLogMessage* message, void* data) {
	...
	try {
		if (data) {
			...
			stringstream s;
			s << "[" << message->time_ms << "] " << message->severity << ": " << message->message << NcbiEndl;
			(*of) << s.str();
			of->flush();
		}
		g_in_logging--;
...
void CCassConnection::SetLogging(int loglevel) {
	...
	cass_log_set_callback(LogCallback, enabled ? &CCassConnection::m_logstrm : nullptr);
	if (debug_enabled)
...
void CCassConnection::Connect() {
	...
	if (IsConnected())
		RAISE_ERROR(eSeqFailed, string("cassandra driver has already been connected"));
	if (m_host.empty())
...
void CCassConnection::Reconnect() {
	if (!m_cluster)
		RAISE_ERROR(eSeqFailed, string("invalid sequence of operations, driver is not connected, can't re-connect"));
	...
	unique_ptr<CassFuture, function<void(CassFuture*)> >future(__future, [](CassFuture* future) {
...
shared_ptr<CCassQuery> CCassConnection::NewQuery() {
	if (!m_session)
		RAISE_ERROR(eSeqFailed, string("invalid sequence of operations, driver is not connected"));
...
void CCassConnection::getTokenRanges(vector< pair<int64_t,int64_t> > &ranges) {
	...
	shared_ptr<CCassQuery> query(NewQuery());
	query->SetSQL("select data_center, schema_version, host_id, tokens from system.local", 0);
	...
	auto itr = peer_tokens.insert(make_pair(host_id, vector<int64_t>())).first;
	...
	while (retries < 3 && (query_host_count == 0 || peer_tokens.size() == query_host_count)) {
		...
		if (query_host_count != 0) {
			LOG3(("GET_TOKEN_MAP: Host_id count is too small. Retrying system.peers fetch. %u", retries));
		}
		query_host_count = 0;
		query_hosts.clear();
		...
		while (query->NextRow() == ar_dataready) {
			...
			if (datacenter == peer_dc && schema == peer_schema) {
				...
				if (itr == peer_tokens.end()) {
					itr = peer_tokens.insert(make_pair(peer_host_id, vector<int64_t>())).first;
					for(const auto& item:tokens) {
						int64_t value = strtol(item.c_str(), nullptr, 10);
						itr->second.push_back(value);
						cluster_tokens.insert(value);
	...
	for (int64_t token:cluster_tokens) {
		LOG5(("GET_TOKEN_MAP: token %" PRId64 " : %" PRId64, token, token - lower_bound));
...
const CassPrepared* CCassConnection::Prepare(const string& sql) {
	...
	if (it == m_prepared.end()) {
        ...
        if (rc != CASS_OK) {
            string msg = (rc == CASS_ERROR_SERVER_SYNTAX_ERROR || rc == CASS_ERROR_SERVER_INVALID_QUERY) ? string(", sql: ") + sql : string("");
...
void CCassConnection::Perform(CAppOp& op, unsigned int optimeoutms, const std::function<bool()>& PreLoopCB, const std::function<void(const EDatabase&)>& DbExceptCB, const std::function<bool(bool)>& OpCB) {
    int err_cnt = 0;
    ...
    while (!PreLoopCB || PreLoopCB()) {
        try {
            ...
            // log and ignore, app-specific layer is responsible for re-connetion if needed
            CAppPerf::Relax(&op);
            if (DbExceptCB)
                DbExceptCB(e);
            if (e.GetErrCode() == EDatabase::eQueryTimeout && ++err_cnt < 10) {
                LOG3(("CAPTURED TIMEOUT: %s, RESTARTING OP", e.TimeoutMsg().c_str()));
            }
            else if (e.GetErrCode() == EDatabase::eQueryFailedRestartable) {
                LOG3(("CAPTURED RESTARTABLE EXCEPTION: %s, RESTARTING OP", e.what()));
            }
            else { // timer exceeded (10 times we got timeout and havn't read anyting, or got another error -> try to reconnect             
...
void CCassPrm::Bind(CassStatement* statement, unsigned int idx) {
	...
	switch (m_type) {
		case CASS_VALUE_TYPE_UNKNOWN:
			if (!IsAssigned())
				RAISE_ERROR(eSeqFailed, string("invalid sequence of operations, Param #" + NStr::NumericToString(idx) + " is not assigned"));
			rc = cass_statement_bind_null(statement, idx);
			break;
		case CASS_VALUE_TYPE_INT:
			rc = cass_statement_bind_int32(statement, idx, (cass_int32_t) m_simpleval.i32);
			break;
		case CASS_VALUE_TYPE_BIGINT:
			rc = cass_statement_bind_int64(statement, idx, (cass_int64_t) m_simpleval.i64);
			break;

		/*
		 * @saprykin
		 * Removed silent binding null if string is empty. It creates a lot of tombstones in storage.
		 * Sometimes if user wants to write empty string it means that we should write empty string.
		...
		case CASS_VALUE_TYPE_BLOB: {
			if (m_bytes.size() > 0)
				rc = cass_statement_bind_bytes(statement, idx, reinterpret_cast<const unsigned char*>(m_bytes.c_str()), m_bytes.size());
			else
				rc = cass_statement_bind_null(statement, idx);
			break;
		}
	...
	if (rc != CASS_OK)
		RAISE_CASS_ERROR(rc, eBindFailed, string("Bind for (") + NStr::NumericToString(static_cast<int>(m_type)) + ") failed with rc= " + NStr::NumericToString(static_cast<int>(rc)));
}
...
void CCassQuery::InternalClose(bool closebatch) {
    LOG5(("CCassQuery::InternalClose: this: %p, fut: %p", LOG_CPTR(this), m_future));
	m_params.clear();
	if (m_future) {
	 ...
	 * @saprykin
	 * Commented out.
	 * InternalClose is called from SetSQL and reseting SerialConsistency level adds SetSQL very nasty side effect.
    ...
    if (m_cb_ref) {
        LOG5(("CCassQuery::InternalClose: cb_ref detach, this: %p, cb_ref: %p", LOG_CPTR(this), m_cb_ref.get()));
        m_cb_ref->Detach();
        m_cb_ref = nullptr;
    }
}
...
void CCassQuery::Bind() {
	if (!m_statement)
		RAISE_ERROR(eSeqFailed, string("invalid sequence of operations, query is closed"));
...
int32_t CCassQuery::ParamAsInt32(int iprm) {
	...
	if (!m_params[iprm].IsAssigned())
		RAISE_ERROR(eSeqFailed, string("invalid sequence of operations, Param #" + NStr::NumericToString(iprm) + " is not assigned"));
...
void CCassQuery::NewBatch() {
	if (!m_connection)
		RAISE_ERROR(eSeqFailed, string("invalid sequence of operations, DB connection closed"));
	if (m_batch)
		RAISE_ERROR(eSeqFailed, string("invalid sequence of operations, batch has already been started"));
	if (IsActive())
		RAISE_ERROR(eSeqFailed, string("invalid sequence of operations, query is active"));
...
void CCassQuery::Query(CassConsistency c, bool run_async, bool allow_prepared, unsigned int page_size) {
    LOG5(("CCassQuery::Query this: %p", LOG_CPTR(this)));
	if (!m_connection)
		RAISE_ERROR(eSeqFailed, string("invalid sequence of operations, DB connection closed"));
	if (m_sql.empty()) 
		RAISE_ERROR(eSeqFailed, string("invalid sequence of operations, SQL is not set"));
	if (m_batch)
		RAISE_ERROR(eSeqFailed, string("invalid sequence of operations, can't run select in batch mode"));
	if (IsActive())
		RAISE_ERROR(eSeqFailed, string("invalid sequence of operations, Query is active"));
	...
	try {
		...
		if (rc != CASS_OK)
			RAISE_CASS_ERROR(rc, eQueryFailed, string("Failed to set consistency level ") + NStr::NumericToString(static_cast<int>(c)));
		if (m_serial_consistency != CASS_CONSISTENCY_ANY) {
			rc = cass_statement_set_serial_consistency(m_statement, m_serial_consistency);
			if (rc != CASS_OK)
				RAISE_CASS_ERROR(rc, eQueryFailed, string("Failed to set serial consistency level ") + NStr::NumericToString(static_cast<int>(m_serial_consistency)));
		}
		if (page_size > 0) {
			rc = cass_statement_set_paging_size(m_statement, page_size);
			if (rc != CASS_OK)
				RAISE_CASS_ERROR(rc, eQueryFailed, string("Failed to set page size to ") + NStr::NumericToString(static_cast<int>(page_size)));
        }
...
void CCassQuery::Execute(CassConsistency c, bool run_async, bool allow_prepared) {
	...
	try {
		...
		if (m_batch) {
			...
			if (rc != CASS_OK)
				RAISE_CASS_ERROR(rc, eQueryFailed, string("Failed to add statement to batch, sql: ") + m_sql);
			cass_statement_free(m_statement);
			m_statement = nullptr;
		}
		m_page_start = false;
...
async_rslt_t CCassQuery::RunBatch() {
	...
	if (!m_batch)
		RAISE_ERROR(eSeqFailed, string("invalid sequence of operations, batch is not created"));
	if (m_async) {
		GetFuture();
	}
	else {
...
async_rslt_t CCassQuery::WaitAsync(unsigned int timeoutmks) {
	if (!m_async)
		RAISE_ERROR(eSeqFailed, string("attempt to wait on query in non-async state"));
...
void CCassQuery::GetFuture() {
	if (!m_connection)
		RAISE_ERROR(eSeqFailed, string("invalid sequence of operations, DB connection closed"));
	if (!IsActive() && !m_batch)
		RAISE_ERROR(eSeqFailed, string("invalid sequence of operations, Query is not active"));
	if (m_iterator || m_result)
		RAISE_ERROR(eSeqFailed, string("invalid sequence of operations, results already obtained"));
	...
	if (!m_future) {
        ...
        LOG5(("CCassQuery::GetFuture: this: %p, fut: %p", LOG_CPTR(this), m_future));
...
async_rslt_t CCassQuery::Wait(unsigned int timeoutmks) {
	...
	if (!rv) {
		...
		ProcessFutureResult();
		if ((m_statement || m_batch) && !m_results_expected) // next request was run in m_ondata event handler
			return ar_wait;
...
void CCassQuery::SetupOnDataCallback() {
	...
	if (!m_ondata && !m_ondata2)
		RAISE_ERROR(eSeqFailed, string("invalid sequence of operations, m_ondata is not set"));
    LOG5(("CCassQuery::SetupOnDataCallback: this: %p, fut: %p, ondata: %p, ondata2: %p", LOG_CPTR(this), m_future, m_ondata, m_ondata2));
...
void CCassQuery::ProcessFutureResult() {
	if (!m_future)
		RAISE_ERROR(eSeqFailed, string("invalid sequence of operations, m_future is not set"));
	if (m_iterator || m_result)
		RAISE_ERROR(eSeqFailed, string("invalid sequence of operations, results already obtained"));

	CassError rc;
	rc = cass_future_error_code(m_future);
	if (rc != CASS_OK) {
		string msg = (rc == CASS_ERROR_SERVER_SYNTAX_ERROR || rc == CASS_ERROR_SERVER_INVALID_QUERY) ? string(", sql: ") + m_sql : string("");
	...
	if (m_results_expected) {
		...
		if (!m_result)
   		...
   		if (m_iterator)
			RAISE_ERROR(eSeqFailed, string("iterator has already been allocated"));

		m_iterator = cass_iterator_from_result(m_result);
   		if (!m_iterator)
			RAISE_DB_ERROR(eRsrcFailed, string("failed to obtain cassandra query iterator"));
		/* 
			we release m_future here for statements that return dataset, otherwise we free it in the destructor,
			keeping m_future not null as an indication that we have already waited for query to finish
		*/
        LOG5(("CCassQuery::ProcessFutureResult: release future this: %p, fut: %p", LOG_CPTR(this), m_future));
		cass_future_free(m_future);
        --m_connection->m_active_statements;
		m_future = nullptr;
        if (m_cb_ref) {
            LOG5(("CCassQuery::ProcessFutureResult: cb_ref detach, this: %p, cb_ref: %p", LOG_CPTR(this), m_cb_ref.get()));
            m_cb_ref->Detach();
            m_cb_ref = nullptr;
        }
		m_futuretime = 0;
...
async_rslt_t CCassQuery::NextRow() {
    ...
    LOG5(("CCassQuery::NextRow: this: %p, fut: %p, m_row: %p, m_result: %p, m_iterator: %p", LOG_CPTR(this), m_future, m_row, m_result, m_iterator));
	if (!IsActive())
		RAISE_ERROR(eSeqFailed, string("invalid sequence of operations, Query is not active"));
	while(1) {
		m_row = nullptr;
		if (m_result == nullptr) {
			...
			if (wr != ar_dataready) {
                LOG4(("CCassQuery::NextRow: async=%d returning wr=%d b'ze of Wait", m_async, static_cast<int>(wr)));
				return wr;
            }
		}
		...
		if (m_iterator && m_result) {
			...
			if (!b) {
                LOG5(("CCassQuery::NextRow: this: %p, cass_iterator_next returned false", LOG_CPTR(this)));
				if (m_page_size > 0) {
					...
					if (!has_more_pages) {
						SetEOF(true);
                        LOG5(("CCassQuery::NextRow: this: %p, async=%d returning wr=ar_done b'ze has_more_pages==false", LOG_CPTR(this), m_async));
						return ar_done;
					}
					// go to above
					m_page_start = true; 
					...
					SetEOF(true);
                    LOG5(("CCassQuery::NextRow: this: %p, async=%d returning wr=ar_done b'ze m_page_size=%d", LOG_CPTR(this), m_async, m_page_size));
					return ar_done;
				}
			} else {
			...
			RAISE_ERROR(eSeqFailed, string("invalid sequence of operations, attempt to fetch next row on a closed query"));
	}
}
...
const CassValue* CCassQuery::GetColumn(int ifld) {
	...
	if (!clm)
		RAISE_ERROR(eSeqFailed, string("column is not fetched (index ") + NStr::NumericToString(ifld) + " beyound the range?)");
	return clm;
}
...
bool CassValueConvert<bool>(const CassValue* Val) {
    CassError err;
	if (!Val)
		RAISE_DB_ERROR(eFetchFailed, string("failed to fetch bool: cass value is NULL"));
	...
	switch (type) {
		case CASS_VALUE_TYPE_BOOLEAN:
			if ((err = cass_value_get_bool(Val, &cb)) != CASS_OK)
				RAISE_CASS_ERROR(err, eFetchFailed, string("failed to fetch boolean data"));
			rv = cb;
			break;
		default:
			if (type == CASS_VALUE_TYPE_UNKNOWN && (!Val || cass_value_is_null(Val)))
				return false;
			RAISE_DB_ERROR(eFetchFailed, string("failed to convert to bool: unsupported (") + NStr::NumericToString(static_cast<int>(type)) + ") data type");
	}
	return rv;
}
	...
	switch (type) {
		...
		case CASS_VALUE_TYPE_COUNTER: {
			...
			if ((err = cass_value_get_int64(Val, &rv64)) != CASS_OK)
				RAISE_CASS_ERROR(err, eFetchFailed, string("failed to fetch int32 data"));
			if (rv64 < INT_MIN || rv64 > INT_MAX) 
				RAISE_DB_ERROR(eFetchFailed, string("failed to fetch int32 data, fetched value is out of range"));
			rv = rv64;
			break;
		}
		case CASS_VALUE_TYPE_INT:
			if ((err = cass_value_get_int32(Val, &rv)) != CASS_OK)
				RAISE_CASS_ERROR(err, eFetchFailed, string("failed to fetch int data"));
			break;
		default:
			if (type == CASS_VALUE_TYPE_UNKNOWN && (!Val || cass_value_is_null(Val)))
				return 0;
			RAISE_DB_ERROR(eFetchFailed, string("failed to convert to int32: unsupported (") + NStr::NumericToString(static_cast<int>(type)) + ") data type");
	}
	return rv;
}
...
int64_t CassValueConvert<int64_t>(const CassValue* Val) {
	...
	int64_t rv = 0;
	if (!Val)
		RAISE_DB_ERROR(eFetchFailed, string("failed to fetch int64: cass value is NULL"));
	CassValueType type = cass_value_type(Val);
	switch (type) {
		case CASS_VALUE_TYPE_TIMESTAMP:
		case CASS_VALUE_TYPE_BIGINT:
		case CASS_VALUE_TYPE_COUNTER:
			if ((err = cass_value_get_int64(Val, &rv)) != CASS_OK)
				RAISE_CASS_ERROR(err, eFetchFailed, string("failed to fetch int64 data"));
			break;
		case CASS_VALUE_TYPE_INT: {
			int32_t rv32;
		...
		default:
			if (type == CASS_VALUE_TYPE_UNKNOWN && (!Val || cass_value_is_null(Val)))
				return 0;
			RAISE_DB_ERROR(eFetchFailed, string("failed to convert to int64: unsupported (") + NStr::NumericToString(static_cast<int>(type)) + ") data type");
	}
	return rv;
}
...
double CassValueConvert<double>(const CassValue* Val) {
	...
	cass_double_t rv = 0;
	if (!Val)
		RAISE_DB_ERROR(eFetchFailed, string("failed to fetch float: cass value is NULL"));
	if ((err = cass_value_get_double(Val, &rv)) != CASS_OK)
		RAISE_CASS_ERROR(err, eFetchFailed, string("failed to convert to float"));
	return rv;
}
...
string CassValueConvert<string>(const CassValue* Val) {
	...
	string value;
	if (!Val)
		RAISE_DB_ERROR(eFetchFailed, string("failed to fetch string: cass value is NULL"));
	CassValueType type = cass_value_type(Val);
	switch (type) {
		case CASS_VALUE_TYPE_BOOLEAN:
		...
		case CASS_VALUE_TYPE_CUSTOM: {
			...
			if ((err = cass_value_get_string(Val, &rv, &len)) != CASS_OK)
				RAISE_CASS_ERROR(err, eFetchFailed, string("failed to fetch string data"));
			value.assign(rv, len);
			break;
		}
		case CASS_VALUE_TYPE_TIMESTAMP: {
		...
		default:
			...
			RAISE_DB_ERROR(eFetchFailed, string("failed to convert to string: unsupported (") + NStr::NumericToString(static_cast<int>(type)) + ") data type");
	}
	return value;
}
...
void CassDataToCollection<bool>(CassCollection* coll, const bool& v) {
	...
	if ((err = cass_collection_append_bool(coll, b)) != CASS_OK)
		RAISE_CASS_ERROR(err, eBindFailed, string("failed to bind boolean data to collection"));
}
...
void CassDataToCollection<int32_t>(CassCollection* coll, const int32_t& v) {
	...
	if ((err = cass_collection_append_int32(coll, v)) != CASS_OK)
		RAISE_CASS_ERROR(err, eBindFailed, string("failed to bind int32 data to collection"));
}
...
void CassDataToCollection<int64_t>(CassCollection* coll, const int64_t& v) {
	...
	if ((err = cass_collection_append_int64(coll, v)) != CASS_OK)
		RAISE_CASS_ERROR(err, eBindFailed, string("failed to bind int64 data to collection"));
}
...
void CassDataToCollection<double>(CassCollection* coll, const double& v) {
	...
	if ((err = cass_collection_append_double(coll, v)) != CASS_OK)
		RAISE_CASS_ERROR(err, eBindFailed, string("failed to bind double data to collection"));
}
...
void CassDataToCollection<float>(CassCollection* coll, const float& v) {
    ...
    if ((err = cass_collection_append_float(coll, v)) != CASS_OK)
        RAISE_CASS_ERROR(err, eBindFailed, string("failed to bind float data to collection"));
}
...
void CassDataToCollection<string>(CassCollection* coll, const string& v) {
	...
	if ((err = cass_collection_append_string(coll, v.c_str())) != CASS_OK)
		RAISE_CASS_ERROR(err, eBindFailed, string("failed to bind string data to collection"));
}
