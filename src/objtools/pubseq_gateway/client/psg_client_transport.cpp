...
int SPSG_IoSession::OnFrameRecv(nghttp2_session*, const nghttp2_frame* frame)
{
    ...
    if ((m_Params.client_mode != EPSG_PsgClientMode::ePerformance) && (m_Params.debug_printout >= EPSG_DebugPrintout::eFrames)) {
...
void SPSG_IoImpl::OnQueue(uv_async_t* handle)
{
    ...
    while (available_servers && remaining_submits) {
        ...
        if (server_sessions.server.throttling.Active()) {
            PSG_IO_TRACE("Server '" << server_sessions.server.address << "' is throttled, ignoring");
            ignore_server();

        // If server has reached its limit (possibly in a different thread)
        } else if (server_sessions.server.available_streams <= 0) {
