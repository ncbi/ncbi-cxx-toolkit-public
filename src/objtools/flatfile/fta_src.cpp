...
using SourceFeatBlkList = forward_list<SourceFeatBlk>;
...
{
    for (auto sfbp = sfbl.begin(); sfbp != sfbl.end(); ++sfbp) {
...
static bool CheckSourceFeatLocFuzz(const SourceFeatBlkList& sfbl)
{
    ...
    ret = true;
    for (auto sfbp = sfbl.cbegin(); sfbp != sfbl.cend(); ++sfbp) {
...
static Int4 CheckSourceFeatCoverage(SourceFeatBlkList& sfbl, MinMaxList& mml, size_t len)
{
    ...
    loc = nullptr;
    for (auto tsfbp = sfbl.begin(); tsfbp != sfbl.end(); ++tsfbp) {
    ...
    sporg = 0;
    i     = 0;
    for (auto tsfbp = sfbl.begin(); tsfbp != sfbl.end(); ++tsfbp, i++) {
        if (! tsfbp->full)
            continue;
...
static Int4 CheckTransgenicSourceFeats(const SourceFeatBlkList& sfbl)
{
    ...
    bool tgs   = false;
    bool focus = false;
    for (auto tsfbp = sfbl.cbegin(); tsfbp != sfbl.cend(); ++tsfbp) {
        if (tsfbp->tg) {
            if (! tsfbp->full)
    ...
    auto tsfbp = next(sfbl.cbegin());
    if (tsfbp == sfbl.cend() || ! tgs)
        return (0);

    for (; tsfbp != sfbl.cend(); ++tsfbp)
...
static Int4 CheckFocusInOrgs(const SourceFeatBlkList& sfbl, size_t len, int* status)
{
    ...
    name = nullptr;
    pat  = "1.." + to_string(len);
    for (tsfbp = sfbl.cbegin(); tsfbp != sfbl.cend(); ++tsfbp) {
...
{
    if (sfbl.empty() || next(sfbl.cbegin()) == sfbl.cend())
        return nullptr;
...
static void CreateRawBioSources(ParserPtr pp, SourceFeatBlkList& sfbl, Int4 use_what)
{
    ...
    for (auto sfbp = sfbl.begin(); sfbp != sfbl.end(); ++sfbp) {
        ...
        for (auto tsfbp = next(sfbp); tsfbp != sfbl.end(); ++tsfbp) {
                                ...
                                SourceFeatBlkList::iterator what)
{
    ...
    auto tsfbp = next(prev);
    for (; tsfbp != sfbl.end(); ++tsfbp) {
        if (tsfbp == what) {
            sfbl.splice_after(sfbl.before_begin(), sfbl, prev);
            return;
        }
        prev = tsfbp;
    }
...
{
    for (auto prev = sfbl.begin(), tsfbp = next(prev); tsfbp != sfbl.end();) {
        if (! tsfbp->useit) {
            prev = tsfbp++;
            continue;
        }
...
static void PickTheDescrSource(SourceFeatBlkList& sfbl)
{
    ...
    res = sfbl.end();
    for (tsfbp = sfbl.begin(); tsfbp != sfbl.end(); ++tsfbp) {
        if (! tsfbp->focus)
            continue;
        res = tsfbp;
        if (! tsfbp->skip)
...
static bool CheckMoltypeConsistency(const SourceFeatBlkList& sfbl, string& moltype)
{
    ...
    name = tsfbp->moltype;
    for (ret = true, tsfbp = sfbl.cbegin(); tsfbp != sfbl.cend(); ++tsfbp) {
        if (! tsfbp->moltype) {
            ch = '\0';
            p  = tsfbp->location;
            if (p && StringLen(p) > 50) {
...
static bool CheckForENV(const SourceFeatBlkList& sfbl, IndexblkPtr ibp, Parser::ESource source)
{
    ...
    auto sfbp            = sfbl.cbegin();
    for (envs = 0, sources = 0; sfbp != sfbl.cend(); ++sfbp, sources++) {
                     ...
                     const string& source, TSeqFeatList& seq_feats)
{
    ...
    SourceFeatBlkList sfbl = CollectSourceFeats(dbp, dbp_end, type);
    if (sfbl.empty()) {
