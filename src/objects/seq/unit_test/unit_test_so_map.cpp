...
BOOST_AUTO_TEST_CASE(RW_2765)
{
    string so_type{ "pseudogene" };
    auto   pFeature = Ref(new CSeq_feat());
    auto   success  = CSoMap::SoTypeToRnaFeature(so_type, *pFeature);
    BOOST_CHECK_EQUAL(success, false);
    BOOST_CHECK_EQUAL(pFeature->GetData().GetSubtype(), CSeqFeatData::eSubtype_bad);

    so_type = "Pseudogenic_Transcript"; // case-insensitive
    success = CSoMap::SoTypeToRnaFeature(so_type, *pFeature);
    BOOST_CHECK_EQUAL(success, true);
    BOOST_CHECK_EQUAL(pFeature->GetData().GetSubtype(), CSeqFeatData::eSubtype_misc_RNA);
}
