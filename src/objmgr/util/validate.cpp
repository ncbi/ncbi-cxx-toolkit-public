...
private:
    ...
    void ValidateSeqIds(const CBioseq& seq, bool* pNTorNC, bool* pTpa, bool* pPdb);
    void ValidateSeqLoc(const CSeq_loc& loc, const CBioseq& seq, char* prefix);
...
(CObjectManager&     objmgr,
 ...
 bool                sup_context)
    : m_ObjMgr(&objmgr),
      ...
      m_NoPubs(false),
      m_NoBioSource(false),
      m_NonASCII(non_ascii),
      m_SpliceValidate(splice_validate),
      m_ValidateAlignments(val_align),
      m_SuppressContext(sup_context)
{
}
...
static TSeqPos s_GetDataLen(const CSeq_inst& inst)
{
    ...
    case CSeq_data::e_Iupacna:
        return inst.GetSeq_data().GetIupacna().Get().size();
    case CSeq_data::e_Iupacaa:
        return inst.GetSeq_data().GetIupacaa().Get().size();
    case CSeq_data::e_Ncbi2na:
        return inst.GetSeq_data().GetNcbi2na().Get().size();
    case CSeq_data::e_Ncbi4na:
        return inst.GetSeq_data().GetNcbi4na().Get().size();
    case CSeq_data::e_Ncbi8na:
        return inst.GetSeq_data().GetNcbi8na().Get().size();
    case CSeq_data::e_Ncbipna:
        return inst.GetSeq_data().GetNcbipna().Get().size();
    case CSeq_data::e_Ncbi8aa:
        return inst.GetSeq_data().GetNcbi8aa().Get().size();
    case CSeq_data::e_Ncbieaa:
        return inst.GetSeq_data().GetNcbieaa().Get().size();
    case CSeq_data::e_Ncbipaa:
        return inst.GetSeq_data().GetNcbipaa().Get().size();
    case CSeq_data::e_Ncbistdaa:
        return inst.GetSeq_data().GetNcbistdaa().Get().size();
    default:
        return 0;
    }
}
...
{
    for (CTypeConstIterator<CSeq_id> id(ConstBegin(se)); id; ++id) {
        switch (id->Which()) {
        case CSeq_id::e_Genbank:
        case CSeq_id::e_Embl:
        case CSeq_id::e_Ddbj:
...
{
    iterate (list< CRef<CSeq_id> >, it, seq.GetId()) {
        if (id.Match(**it)) {
            return true;
        }
    }
...
inline
static bool s_IsInGenProdSet(const CBioseq& seq)
{
    const CSeq_entry* se = seq.GetParentEntry();
    if (!se) {
        return false;
    }
    for (se = se->GetParentEntry(); se; se = se->GetParentEntry()) {
        if (!se->IsSet()) {
...
inline
static bool s_IsGPSorNTorNC(const CBioseq& seq)
{
    ...
    if (fi) {
       ...
       // Translate na CSeq_data
        ...
        if (itor) {
            ...
            const CSeq_data* in_data;
            CSeq_data c_data;
            if (itor->Which() != CSeq_data::e_Iupacna) {
                CSeqportUtil::Convert(*itor, &c_data, CSeq_data::e_Iupacna);
                in_data = &c_data;
            } else {
                in_data = &(*itor);
            }
            ...
            // just the last codon.
            CSeq_loc::TRange rng = fi->GetLocation().GetTotalRange();
            TSeqPos beg = rng.GetFrom();
            TSeqPos len = rng.GetTo() - beg + 1;
            if (gc) {
                CGencode::Translate(*in_data, &out_data, *gc, cb, beg, len,
...
static unsigned int s_GetSeqlocPartialInfo(const CSeq_loc& loc, CScope* scope)
{
    ...
    for (++i2; i2; ++i2) {
        ...
        case CSeq_loc::e_Whole:
        {
            ...
            // If there was no CMolInfo, try to loop through list<EGIBB_mod>
            CTypeConstIterator<CSeqdesc> sd(ConstBegin((*bc).GetDescr()));
            for (; sd; ++sd) {
                if (!(*sd).IsModif()) {
                    continue;
                }
...
inline
static bool s_IsGenProdSet(CSeq_entry* se)
{
    if (!se  ||  !se->IsSet()  ||  !se->GetSet().IsSetClass()  ||
...
// Returns true if an object of type T is embedded in se, else false
...
inline
static bool s_IsGPS(const CSeq_entry& se)
{
    if (se.IsSet() && se.GetSet().IsSetClass()) {
        return se.GetSet().GetClass() == CBioseq_set::eClass_gen_prod_set;
    }
    return false;
}
...
void CValidError_impl::ValidateSeqEntry(const CSeq_entry& entry)
{
    switch (entry.Which()) {
    case CSeq_entry::e_Seq:
        ValidateBioseq(entry.GetSeq());
        break;
    case CSeq_entry::e_Set:
        ValidateSeqSet(entry.GetSet());
        break;
    default:
        break;
    }
...
void CValidError_impl::ValidateSeqDescriptors(const CBioseq& seq)
{
    ...
    for (CTypeConstIterator<CSeqdesc> dt(ConstBegin(seq)); dt; ++dt) {
        ...
        case CSeqdesc::e_Method:
            ValidErr(eDiag_Error, eErr_SEQ_DESCR_Unknown,
                "Obsolete descriptor", *dt);
            break;
        case CSeqdesc::e_Prf:
            if (prf_found) {
 ...
 bool           isPdb)
{
    ...
    if (inst.GetMol() == CSeq_inst::eMol_aa) {
        ...
        ValidErr(eDiag_Error, eErr_SEQ_INST_MolNotSet, "Bioseq.mol not set",
            seq);
    } else if (inst.GetMol() == CSeq_inst::eMol_other) {
        ValidErr(eDiag_Error, eErr_SEQ_INST_MolOther,
                 "Bioseq.mol is type other", seq);
    } else if (inst.GetMol() == CSeq_inst::eMol_na) {
 ...
 bool*          pPdb)
{
    ...
    iterate (list< CRef<CSeq_id> >, i, seq.GetId()) {
        ...
        if ((**i).IsOther() && (**i).GetOther().IsSetAccession()) {
            ...
            isNTorNC = !NStr::CompareCase(acc, 0, 3, "NT_")  ||
                !NStr::CompareCase(acc, 0, 3, "NC_")
                ? true : false;
        } else if ((**i).IsGi()) {
            hasGi = true;
        }
...
{
    bool na = (options & eVal_non_ascii) != 0;
