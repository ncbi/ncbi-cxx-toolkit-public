                           ...
                           int/*bool*/           asis)
{
    ...
    for (;;) { /* timeslice loop */
        ...
        for (i = 0;  i < n;  i++) {
            ...
            event     = polls[i].event;
            writeable = sock->writeable ? eIO_Write : eIO_Open;
            ...
            if (type & eSocket) {
                ...
                case eIO_ReadWrite:
                    if (type == eDatagram
                        ||  (!writeable  &&  sock->w_status != eIO_Closed)) {
                        mask |= FD_WRITE;
                        if (!sock->connected)
                            mask |= FD_CONNECT;
                    }
                ...
                handle = sock->event;
                ResetEvent(handle);
                if (WSAEventSelect(sock->sock, handle, mask) != 0) {
            ...
            do {
                ...
                for (j = i;  j < n;  j++) {
                    ...
                    if (sock->type == eTrigger
                        &&  ((TRIGGER) sock)->fd == what[i]) {
                        polls[j].revent = polls[j].event;
                        ready = 1;
                        break;
                    } else if (sock->event == what[i]) {
                        WSANETWORKEVENTS e;
                        ...
                        if (e.lNetworkEvents &
                            ...
                            ready = 1;
                        } else if (e.lNetworkEvents & (FD_WRITE | FD_CONNECT)){
...
#  endif /*!NCBI_MSWIN && FD_SETSIZE*/
            ...
            type = (ESOCK_Type) sock->type;
            if (type == eTrigger  &&  *((void**) sock->host/*isset*/)) {
    ...
    for (;;) { /* until one full "tv" term expires or an error occurs */
        ...
        for (i = k;  i < n;  i++) {
            ...
            if (polls[i].event == eIO_Read  &&  polls[i].revent == eIO_Write) {
                ...
                s_WritePending(polls[i].sock, &zero, 1/*w*/, 0);
                polls[i].revent = eIO_Open;
                continue;
            }
                         ...
                         int         flag)
{
    ...
    for (;;) { /* optionally auto-resume if interrupted */
        ...
        if (x_written > 0  ||
            ...
            /* statistics & logging */
            if (sock->log == eOn  ||  (sock->log == eDefault && s_Log == eOn)){
                if (!sock->session  ||  x_written < 0  ||  flag > 0) {
                           ...
                           TSOCK_Flags     flags)
{
    ...
    *sock = 0;
    ...
    /* connect */
    status = s_Connect(x_sock, host, port, timeout, flags & fSOCK_Secure);
    if (status != eIO_Success)
        SOCK_Close(x_sock);
    else
        *sock = x_sock;
