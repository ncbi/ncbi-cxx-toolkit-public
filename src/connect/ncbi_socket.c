    ...
    if (status == eIO_Success  &&  n) {
        ...
        for (m = 0, i = 0;  i < n;  ++i) {
            ...
            if (sock  &&  polls[i].event) {
                ...
                if ((events & POLLIN)
                    &&  (revents & (POLLIN | POLLRDHUP | POLLPRI))) {
                    polls[i].revent = (EIO_Event)(polls[i].revent | eIO_Read);
                }
                if ((events & POLLOUT)
...
static EIO_Status s_Close(SOCK sock, TSOCK_Keep keep)
{
    EIO_Status status;

    status = s_Close_(sock, 0/*orderly*/, keep);
    if (s_ErrHook  &&  status != eIO_Success) {
        SSOCK_ErrInfo info;
        char          addr[40];
        memset(&info, 0, sizeof(info));
...
#  endif /*HAVE_SIN_LEN*/
    ...
    if ((fd = accept(lsock->sock, &addr.sa, &addrlen)) == SOCK_INVALID) {
        const char* strerr = SOCK_STRERROR(error = SOCK_ERRNO);
...
extern EIO_Status SOCK_CloseEx(SOCK sock, int/*bool*/ destroy)
{
    ...
    if (sock->sock == SOCK_INVALID)
        ...
        status = s_Close(sock, fSOCK_KeepNone);
    else {
        sock->sock = SOCK_INVALID;
        status = eIO_Success;
    }
