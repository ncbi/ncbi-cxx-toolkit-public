...
#ifdef TEST_MODULE__NCBI_BUFFER
...
#  define X_BYTES     (size_t)   (s_Rand() % X_MAX_READ)
  ...
  {{
    char charbuf[128];
    verify(BUF_PushBack(&buf, (const char*)"0", 1));
    verify(BUF_Write(&buf, (const char*)"1", 1));
    verify(BUF_Peek(buf, charbuf, sizeof(charbuf)) == 2);
    verify(BUF_PushBack(&buf, (const char*)"BB", 2));
    verify(BUF_PushBack(&buf, (const char*)"aa", 2));
    verify(BUF_Write(&buf, (const char*)"23", 3));
    verify(BUF_Read(buf, charbuf, sizeof(charbuf)) == 9);
...
\t\tBUF_Read %5lu\n", (unsigned long)n_bytes);
      ...
      if (s_Rand() % 3 == 0) {
        ...
        verify(BUF_PushBack(&buf, charbuf + n_bytes - n_pushback, n_pushback));
        verify(BUF_Read(buf, charbuf + n_bytes - n_pushback, n_pushback));
      }
      
      /* write the read data to the output stream */
      verify(n_bytes == fwrite(charbuf, 1, n_bytes, stdout));
      fprintf(stderr, "\
\t\tSTDOUT   %5lu\n", (unsigned long)n_bytes);
    }
  }
  ...
  while ( BUF_Size(buf) ) {
    ...
    {{
      ...
      verify(BUF_PushBack(&buf, charbuf + n_bytes - n_pushback, n_pushback));
      verify(BUF_Read(buf, tmp, n_pushback)
             == n_pushback);
...
#endif /* TEST_MODULE__NCBI_BUFFER */
