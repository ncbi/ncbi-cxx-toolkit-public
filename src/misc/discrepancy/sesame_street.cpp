...
template<>
class CDiscrepancyPrivateData<eTestNames::DEPRECATED>
{
public:
    void* m_UserData = nullptr;
};
...
DISCREPANCY_CASE(DEPRECATED, BIOSRC, eDisc | eOncaller | eSubmitter | eSmart | eBig | eFatal, "Some animals are more equal than others...")
{
    for (const CBioSource* biosrc : context.GetBiosources()) {
        CRef<CDiscrepancyObject> disc_obj(context.BiosourceObjRef(*biosrc));
        m_Objs["deprecated"].Add(*disc_obj);
        if (biosrc->CanGetSubtype()) {
            for (auto& it : biosrc->GetSubtype()) {
                const CSubSource::TSubtype& subtype = it->GetSubtype();
                if (it->CanGetName()) {
...
DISCREPANCY_SUMMARIZE(DEPRECATED)
{
    ...
    CReportNode report, final_report;
    CReportNode::TNodeMap& the_map = m_Objs.GetMap();
    TReportObjectList& all = m_Objs[key].GetObjects();
    size_t total = all.size();
    TReportObjPtrMap all_missing;
    for (auto it: all) {
        all_missing[it] = it;
    ...
    for (auto it: the_map) {
        ...
        size_t uniq = 0;
        size_t num = 0;
        size_t pres = m_Objs[key][qual].GetObjects().size();
        size_t mul = m_Objs[key][qual]["*"].GetObjects().size();
        TReportObjPtrMap missing = all_missing;
        CReportNode::TNodeMap& sub = it.second->GetMap();
        TStringStringObjVectorMap capital;
        for (auto jj: sub) {
        ...
        // CODE TO MARK AS AUTOFIXABLE GOES HERE
...
DISCREPANCY_AUTOFIX(DEPRECATED)
{
    ...
    // CODE TO DELETE DEPRECATED SUBSOURCE OR ORGMOD GOES HERE

    if (changed) {
        return CRef<CAutofixReport>(new CAutofixReport("DEPRECATED: [n] qualifier[s] " + qual + " (" + val + ") fixed", (unsigned)(added + changed)));
    }
    else if (added) {
        return CRef<CAutofixReport>(new CAutofixReport("DEPRECATED: [n] missing qualifier[s] " + qual + " (" + val + ") added", (unsigned)added));
    }
    return CRef<CAutofixReport>();
}
