...
void CBioseq_NO_PRODUCT_STRING :: TestOnObj(const CBioseq& bioseq)
{
   ITERATE (vector <const CSeq_feat*>, it, prot_feat) {
     ...
     if (prot.CanGetName() && !prot.GetName().empty()) {
       if ((*it)->GetData().GetSubtype() == CSeqFeatData::eSubtype_prot) {
          ...
          const CSeq_feat* cds = sequence::GetCDSForProduct(seq_hl); 
          if (cds) {
             ITERATE (list <string>, nit, prot.GetName()) {
                if ( (*nit) == "no product string in file") {
...
void CBioseq_NO_PRODUCT_STRING :: GetReport(CRef <CClickableItem> c_item)
{
   c_item->obj_list = thisInfo.test_item_objs[GetName()];
   c_item->description = GetHasComment(c_item->item_list.size(), "product") 
...
static const string suspect_rna_product_names[] = {
   "gene", "genes"
};
...
void CBioseq_ADJACENT_PSEUDOGENES :: TestOnObj(const CBioseq& bioseq)
{
   ...
   ITERATE (vector <const CSeq_feat*>, it, gene_feat) {
     ...
     if (!match_txt.empty()) {
       thisInfo.test_item_list[GetName()]
                  .push_back(match_txt + "$" + GetDiscItemText(**it));
       thisInfo.test_item_list[GetName()]
                  .push_back(match_txt + "$" + GetDiscItemText(**jt));

       thisInfo.test_item_objs[GetName() + "$" + match_txt]
                 .push_back(CConstRef <CObject>(*it));
       thisInfo.test_item_objs[GetName() + "$" + match_txt]
...
void CBioseq_DISC_FEAT_OVERLAP_SRCFEAT :: GetReport(CRef <CClickableItem> c_item)
{
   ...
   ITERATE (Str2Strs, it, biosrc2feats) {
     ...
     c_item->description 
         = GetOtherComment(c_item->item_list.size()-1, 
...
int CBioseqTestAndRepData :: DistanceToUpstreamGap(const unsigned& pos, const CBioseq& bioseq)
{
   ...
   ITERATE (list <CRef <CDelta_seq> >, it, bioseq.GetInst().GetExt().GetDelta().Get()) {
     if ( (*it)->IsLoc()) {
          offset += sequence::GetCoverage((*it)->GetLoc(), thisInfo.scope);
     }
...
void CBioseq_FIND_OVERLAPPED_GENES :: GetReport(CRef <CClickableItem> c_item)
{
    c_item->obj_list = thisInfo.test_item_objs[GetName()];
    c_item->description 
        = GetOtherComment(c_item->item_list.size(), "gene", "genes")
...
void CBioseq_TEST_UNUSUAL_NT :: GetReport(CRef <CClickableItem> c_item)
{
   c_item->obj_list = thisInfo.test_item_objs[GetName()];
   c_item->description 
       = GetContainsComment(c_item->item_list.size(), "sequence")
...
void CBioseq_MISSING_LOCUS_TAGS :: GetReport(CRef <CClickableItem> c_item)
{
   c_item->obj_list = thisInfo.test_item_objs[GetName()];
   c_item->description 
      = GetHasComment(c_item->item_list.size(), "gene") + "no locus tags.";
};
...
void CSeqEntry_TEST_ALIGNMENT_HAS_SCORE :: TestOnObj(const CSeq_entry& seq_entry)
{
   ...
   if (a_ci) {
       x_CheckSeqEntryAlign(seq_entry);
   }
};
...
void CBioseq_JOINED_FEATURES :: GetReport(CRef <CClickableItem> c_item)
{
   ...
   ITERATE (Str2Strs, it, exp2feats) {
      ...
      AddSubcategory(c_item, GetName() + "$" + it->first, &(it->second), 
              "feature", "joined location " + cmt_txt, e_HasComment); 
   }
   c_item->description
...
void CBioseq_DUPLICATE_GENE_LOCUS :: GetReport(CRef <CClickableItem> c_item)
{
   ...
   if (!c_item->subcategories.empty()) {
       c_item->description 
            = GetHasComment(c_item->item_list.size(), "gene")
...
void CBioseqSet_DISC_SEGSETS_PRESENT :: GetReport(CRef <CClickableItem> c_item)
{
   c_item->obj_list = thisInfo.test_item_objs[GetName()];
   c_item->description 
        = GetIsComment(c_item->item_list.size(), "segset") + "present.";
};
...
{
   c_item->obj_list = thisInfo.test_item_objs[GetName()];
   c_item->description = GetIsComment(c_item->item_list.size(), "set") 
                           + "of type eco, mut, phy or pop.";
};
// new comb
...
// new method
bool CSeqEntry_test_on_tax_cflts :: x_StringHasVoucherSN(const string& vou_nm)
{
...
void CSeqEntry_test_on_tax_cflts :: TestOnObj(const CSeq_entry& seq_entry)
{
   ...
   ITERATE (vector <const CSeq_feat*>, it, biosrc_orgmod_feat) {
      RunTests( (*it)->GetData().GetBiosrc(), 
                     GetDiscItemText(**it), CConstRef <CObject> (*it));
   }
...
{
   has_seq = (primer->CanGetSeq() && !primer->GetSeq().Get().empty());
   has_name = (primer->CanGetName() && !primer->GetName().Get().empty());
};
...
void CSeqEntry_TEST_MISSING_PRIMER :: GetReport(CRef <CClickableItem> c_item)
{
   ...
   c_item->description 
      = GetHasComment(c_item->item_list.size(),"biosource") 
         + "primer sets with missing values";
};
...
void CSeqEntry_DISC_MAP_CHROMOSOME_CONFLICT :: GetReport(CRef <CClickableItem> c_item)
{
   ...
   c_item->description 
       = GetOtherComment(c_item->item_list.size(), 
                          "source on eukaryotic sequence has", 
                           "sources on eukaryotic sequences have") 
...
CFlatFileConfig::CGenbankBlockCallback::EBioseqSkip CFlatfileTextFind::notify_bioseq(CBioseqContext& ctx )
{
   ...
   m_bsq = bioseq_hl.GetCompleteObject();
   m_bioseq_desc = GetDiscItemText(*m_bsq);
...
{
  // ORGANISM_BLOCK in C is omited
  block_text = block_text.substr(0, block_text.find("ORGANISM")); 
...
CConstRef <CObject> CFlatfileTextFind:: x_GetObjRef(const string& desc, CSeqdesc_CI seq_desc)
{
  ...
  for (CSeqdesc_CI it = seq_desc; it; ++it) {
     switch (it->Which()) {
        case CSeqdesc::e_Source:
            strtmp = GetDiscItemText(*it, *m_bsq);
            obj_ref = CConstRef <CObject>(&(*it));
            break;
...
void CFlatfileTextFind :: x_GetDescAndObjRef4Seqdesc(CConstRef <CBioseq> bioseq, CSeqdesc :: E_Choice seqdesc_type, CSeqdesc& seqdesc, string& desc, CConstRef <CObject> obj_ref) 
{
   ...
   CSeq_entry* entry = bioseq->GetParentEntry();
   string str0, str1;
   switch(seqdesc_type) {
     case CSeqdesc::e_Pub: 
     case CSeqdesc::e_Source:
          str0 = Blob2Str(seqdesc);
          break;
     case CSeqdesc::e_Genbank:
       {
          if (seqdesc.GetGenbank().CanGetKeywords()) {
              list <string>& 
                ls = const_cast<list <string>& >(seqdesc.GetGenbank().GetKeywords());
              if (!ls.empty()) {
                 ls.sort();
                 ITERATE (list <string>, it, ls) {
                    str0 += *it + ",";
                 }
                 str0 = str0.substr(0, str0.size()-1);
              }
          }
          break;
       }
     default: str0 = kEmptyStr;
   }
   while (entry) {
      CSeq_entry_Handle seq_entry_h = thisInfo.scope->GetSeq_entryHandle(*entry);
      for (CSeqdesc_CI seqdesc_it(seq_entry_h, seqdesc_type, 1); 
                                                seqdesc_it; ++seqdesc_it) {
        str1 = kEmptyStr;
        switch (seqdesc_type) {
           case CSeqdesc::e_Pub: 
           case CSeqdesc::e_Source: 
                str1 = Blob2Str(*seqdesc_it);
                break;
           case CSeqdesc::e_Genbank:
             {
               
               if (seqdesc_it->GetGenbank().CanGetKeywords()) {
                  list <string>& ls = const_cast<list <string>&> (
                                         seqdesc_it->GetGenbank().GetKeywords());
                  if (!ls.empty()) {
                     ls.sort();
                     ITERATE (list <string>, it, ls) {
                        str1 += (*it + ",");
           ...
           default: str1 = kEmptyStr;
                    break;
        }
        if (str0 == str1 && !str1.empty()) {
             desc = GetDiscItemText(seqdesc, *entry);
             obj_ref.Reset(&(*seqdesc_it));
             break;
        }
      };
      if (desc.empty()) {
     ...
     if (CTestAndRepData :: DoesStringContainPhrase( 
       ...
       switch (which_block) {
         case CFlatFileConfig::fGenbankBlocks_Locus: {
            ...
            x_GetDescAndObjRef4Seqdesc(
                m_bsq, CSeqdesc::e_Molinfo, seqdesc, desc, obj_ref);
            break;
         }
         case CFlatFileConfig::fGenbankBlocks_Keywords: {
            const CKeywordsItem&
               kw_item = dynamic_cast<const CKeywordsItem&>(flat_item);
            const vector <string>& kwds = kw_item.GetKeywords();
            CGB_block gb_blk;
            ITERATE (vector <string>, it, kwds) {
                 gb_blk.SetKeywords().push_back(*it);
            }
            CSeqdesc seqdesc;
            seqdesc.SetGenbank(gb_blk);
            x_GetDescAndObjRef4Seqdesc(m_bsq, CSeqdesc::e_Genbank, seqdesc, desc,obj_ref);
            break;
         }
         case CFlatFileConfig::fGenbankBlocks_Reference: {
            const CReferenceItem& 
               ref_item = dynamic_cast<const CReferenceItem&>(flat_item);
            const CPubdesc& pub = ref_item.GetPubdesc();
            CSeqdesc seqdesc;
            seqdesc.SetPub().Assign(pub);
            x_GetDescAndObjRef4Seqdesc(m_bsq, CSeqdesc::e_Pub, seqdesc, desc, obj_ref);
            break;
         }
         case CFlatFileConfig::fGenbankBlocks_Source: {
            ...
            if (biosrc) {
               ...
               x_GetDescAndObjRef4Seqdesc(
                     m_bsq, CSeqdesc::e_Source, seqdesc, desc, obj_ref);
            }
            break;
         }
         case CFlatFileConfig::fGenbankBlocks_Sourcefeat: 
            {
              ...
              const CBioSource& biosrc = sf.GetData().GetBiosrc();
              CSeqdesc seqdesc;
              seqdesc.SetSource().Assign(biosrc);
              x_GetDescAndObjRef4Seqdesc(
                        m_bsq, CSeqdesc::e_Source, seqdesc, desc, obj_ref);
         ...
         case CFlatFileConfig::fGenbankBlocks_FeatAndGap:
            {
              const CFeatureItemBase& 
                feat_item = dynamic_cast<const CFeatureItemBase&> (flat_item);
              const CSeq_feat& sf = feat_item.GetFeat().GetOriginalFeature();
              desc = GetDiscItemText(sf);
              obj_ref.Reset(&sf);
              break;
            }
         default: desc = m_bioseq_desc;
                  obj_ref = CConstRef <CObject>(m_bsq.GetPointer());
       };
       if (!desc.empty()) {
       strtmp 
         = thisInfo.fix_data[it->first].s_val + "$" + it->first + "#" + desc;
...
void CSeqEntry_DISC_FLATFILE_FIND_ONCALLER :: TestOnObj(const CSeq_entry& seq_entry)
{
   ...
   // not sure if the fViewFirst works as expected, so still keep the rm redundancy.
...
void CSeqEntry_DISC_TITLE_ENDS_WITH_SEQUENCE :: GetReport(CRef <CClickableItem> c_item)
{
  c_item->obj_list = thisInfo.test_item_objs[GetName()];
  c_item->description = GetOtherComment(c_item->item_list.size(), 
                                         "defline appears", "deflines appear")
...
{
   c_item->description = GetOtherComment(c_item->item_list.size(), 
                                   "title on sets was", "titles on sets were") 
                          + " found";
...
void CBioseq_DISC_FEATURE_COUNT :: GetReport(CRef <CClickableItem> c_item)
{
   ...
   ITERATE (Str2Strs, it, feat2cnt) {
      ...
      feat_nm = it->first.substr(3); 
      c_item->description = feat_nm + ": " + NStr::UIntToString(cnt) 
