		    ...
		    int SingleWindow, int DoubleWindow)
{
	...
	if(iSearch/10000*10000 == iSearch) ERR_POST(Info << "sequence " << 
						  iSearch);
	header = 0;
	readdb_get_header_ex(rdfp, iSearch, &header, &sip,
			...
			double ThreshInc)
{
	...
	for(iScoreList = ScoreList.begin(), iSearch = 0;
	    ...
	    header = 0;
	    readdb_get_header(rdfp, MSHit->GetSeqIndex(), &header, &sip,
			      &blastdefline);
	    bestid = SeqIdFindBest(sip, SEQID_GI);
	    if(!bestid) continue;
	    ...
	    if(bestid->data.intvalue == 6321737 || bestid->data.intvalue ==6322014  || bestid->data.intvalue == 6322017)
		...
		Hit->SetEvalue(Score);
		Hit->SetPvalue(Score/
			       Peaks->GetPeptidesExamined(MSHit->GetCharge()));	   
