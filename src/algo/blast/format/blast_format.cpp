...
void CBlastFormat::PreFetchSequenceData(const CRef<blast::CSearchResultSet>& results)
{
    ...
    CScope::TBioseqHandles bhs = m_Scope->GetBioseqHandles(ids);
    ...
    // Add it to the scope
    CBioseq_Handle top_bh = m_Scope->AddBioseq(*top_seq);
    ...
    top_bh.GetSeqMap().CanResolveRange(&*m_Scope, sel);
}
