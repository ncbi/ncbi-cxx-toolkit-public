...
private:
    ...
    CRef<CBlast4Options>
    GetRemoteObj(EProgram          prog,
                 CRef<CBioseq_set> bss,
                 const string    & dblist,
                 double            evalue);
...
void CRemote_BlastApplication::Init(void)
{
    ...
    arg_desc->SetUsageContext(GetArguments().GetProgramBasename(),
                              "Searches database(s) for a query, using the BLAST Remote API");
    ...
    // Subject(s) sequence(s) or database(s)
    // For now, only DB is available.
    
    //arg_desc->AddKey("subject", "s", "Subject(s) file name",
    //        CArgDescriptions::eInputFile, CArgDescriptions::fPreOpen);
    arg_desc->AddDefaultKey("db", "db", "Subject database",
                            CArgDescriptions::eString, "nr");
    
    // Copied from blast_app
    //
    // Many of these will be omitted for the nonce.  In the near
    // future, we may see blast_client code colonizing this area.
...
void 
CRemote_BlastApplication::InitObjMgr(void)
{
...
EProgram
CRemote_BlastApplication::GetBlastProgramNum(const string& prog)
{
    if (prog == "blastp")
        return eBlastp;
    if (prog == "blastn")
...
void
CRemote_BlastApplication::ProcessCommandLineArgs(CBlastOptions& opt)
{
    CArgs args = GetArgs();
...
FILE*
CRemote_BlastApplication::GetOutputFilePtr(void)
{
    FILE *retval = NULL;
...
CRef<CBlast4Options>
CRemote_BlastApplication::GetRemoteObj(EProgram          prog,
                                       CRef<CBioseq_set> bss,
                                       const string    & dblist,
                                       double            evalue)
{
    CRef<CBlast4Options> b4o;
    
    switch(prog) {
    case eBlastp:
        {
            CRef< CBlastProteinOptionsHandle >
                bpoh(new CBlastProteinOptionsHandle(CBlastOptions::eRemote));
            
            if (evalue != 0.0)
                bpoh->SetEvalueThreshold(evalue);
            
            b4o.Reset( new CBlast4Options(&(*bpoh)) );
        }
        break;
        
    case eBlastn:
        {
            CRef< CBlastNucleotideOptionsHandle >
                bpoh(new CBlastNucleotideOptionsHandle);
...
int CRemote_BlastApplication::Run(void)
{
    ...
    {
        int flagp = fReadFasta_AssumeProt;
        int flagn = fReadFasta_AssumeNuc;
        
        int flag = fReadFasta_AllSeqIds | fReadFasta_NoParseID;
        ...
        istream & inp_file( args["query"].AsInputFile() );
        CRef<CSeq_entry> seqentry = ReadFasta( inp_file, flag, 0, 0 );
        
        seqset->SetSeq_set().push_back(seqentry);
    }
    
    CRef<CBlast4Options> b4opt =
        GetRemoteObj(prog,
                     seqset,
                     args["db"].AsString(),
                     args["evalue"].AsDouble());
    
    b4opt->SetVerbose(verbose);

    b4opt->SubmitSync();
    
    double t = sw.Elapsed();
    cerr << "remote_blast run took " << t << " seconds" << endl;
...
void CRemote_BlastApplication::Exit(void)
{
    SetDiagStream(0);
}
 ...
 * ===========================================================================
 * $Log$
 * Revision 1.1  2004/02/06 00:45:09  bealer
 * - Add remote_blast project as demonstration of the Remote BLAST API code.
 *
