	...
	{{
		...
		if(!Seq.IsNull()) { 
			...
			if(CanMap == e_Yes) { // Not Up, but Yes cause its the parent already
				...
				if(GiLoc) {
					Result = x_Map_Up(*GiLoc, *Seq, Spec);
				}
				else {
					Result = x_Map_Up(Loc, *Seq, Spec);
				}
				if(!Result.IsNull() && !Result->IsNull()) {
					return Result;
        		}
...
bool CGencollIdMapper::x_NCBI34_Guess(const CSeq_id& Id, SIdSpec& Spec) const
{
...
CConstRef<CSeq_id> CGencollIdMapper::x_NCBI34_Map_IdFix(CConstRef<CSeq_id> SourceId) const
{
    if(m_Assembly->GetTaxId() != 9606 || m_Assembly->GetName() != "NCBI34")
        return SourceId;
...
void CGencollIdMapper::x_RecursiveSeqFix(CGC_Sequence& Seq)
{
...
void CGencollIdMapper::x_FillGpipeTopRole(CGC_Sequence& Seq)
{
	bool SeqHasGi;
...
void CGencollIdMapper::x_FillChromosomeIds()
{
...
void CGencollIdMapper::x_PrioritizeIds()
{
	CTypeIterator<CGC_Sequence> SeqIter(*m_Assembly);
	while(SeqIter) {
...
void CGencollIdMapper::x_PrioritizeIds(CGC_Sequence& Sequence)
{
...
CConstRef<CSeq_id> CGencollIdMapper::x_FixImperfectId(CConstRef<CSeq_id> Id, const SIdSpec& Spec) const
{
...
CConstRef<CSeq_id> CGencollIdMapper::x_ApplyPatternToId(CConstRef<CSeq_id> Id, const SIdSpec& Spec) const
{
...
int CGencollIdMapper::x_GetRole(const objects::CGC_Sequence& Seq) const
{
	int SeqRole = 0;
	
	SeqRole = SIdSpec::e_Role_NotSet;
...
bool CGencollIdMapper::x_HasTop(const objects::CGC_Sequence& Seq, int Top) const
{
...
void CGencollIdMapper::x_AddSeqToMap(const CSeq_id& Id,
                                     CConstRef<CGC_Sequence> Seq)
{
...
void CGencollIdMapper::x_BuildSeqMap(const CGC_Sequence& Seq)
{
	if(Seq.CanGetSeq_id()) {
        int IdCount = 0;
		CTypeConstIterator<CSeq_id> IdIter(Seq);
    ...
    if(Seq.CanGetSeq_id_synonyms() &&
        (Seq.GetSeq_id_synonyms().size() >= 1 || Seq.CanGetStructure()) ) {
        ...
        ITERATE (CGC_Sequence::TSeq_id_synonyms, it, Seq.GetSeq_id_synonyms()) {
            ...
            case CGC_TypedSeqId::e_Genbank:
                if((*it)->GetGenbank().CanGetPublic())
                    x_AddSeqToMap((*it)->GetGenbank().GetPublic(), SeqRef);
                if((*it)->GetGenbank().CanGetGpipe())
                    x_AddSeqToMap((*it)->GetGenbank().GetGpipe(), SeqRef);
                if((*it)->GetGenbank().CanGetGi())
                    x_AddSeqToMap((*it)->GetGenbank().GetGi(), SeqRef);
                break;
            case CGC_TypedSeqId::e_Refseq:
                if((*it)->GetRefseq().CanGetPublic())
                    x_AddSeqToMap((*it)->GetRefseq().GetPublic(), SeqRef);
                if((*it)->GetRefseq().CanGetGpipe())
                    x_AddSeqToMap((*it)->GetRefseq().GetGpipe(), SeqRef);
                if((*it)->GetRefseq().CanGetGi())
                    x_AddSeqToMap((*it)->GetRefseq().GetGi(), SeqRef);
                break;
            case CGC_TypedSeqId::e_External:
...
void CGencollIdMapper::x_BuildSeqMap(const CGC_AssemblyUnit& assm)
{
    if (assm.IsSetMols()) {
...
void CGencollIdMapper::x_BuildSeqMap(const CGC_Assembly& assm)
{
    if (assm.IsUnit()) {
...
CConstRef<CSeq_id> CGencollIdMapper::x_GetIdFromSeqAndSpec(const CGC_Sequence& Seq,
...
int CGencollIdMapper::x_CanSeqMeetSpec(const CGC_Sequence& Seq, const SIdSpec& Spec, int Level) const
{
	if(Level == 3) {
		return e_No;
	}
	...
	if(Seq.CanGetSequences() && Spec.Top != true) {
		ITERATE(CGC_Sequence::TSequences, TagIter, Seq.GetSequences()) {
			if((*TagIter)->GetState() != CGC_TaggedSequences::eState_placed)
				continue;
			ITERATE(CGC_TaggedSequences::TSeqs, SeqIter, (*TagIter)->GetSeqs()) {
				int Child = x_CanSeqMeetSpec(**SeqIter, Spec, Level+1);
				if(Child == e_Yes)
					return e_Down;
			}
		}
...
bool CGencollIdMapper::x_MakeSpecForSeq(const CSeq_id& Id, const CGC_Sequence& Seq, SIdSpec& Spec) const
{
...
bool CGencollIdMapper::x_IsParentSequence(const CSeq_id& Id,
                                          const CGC_Sequence& Parent) const
{
...
CConstRef<CGC_Sequence> CGencollIdMapper::x_FindChromosomeSequence(const CSeq_id& Id, const SIdSpec& Spec) const
{
    if(Id.IsGi() && Id.GetGi() > 50)
...
CRef<CSeq_loc> CGencollIdMapper::x_Map_OneToOne(const CSeq_loc& SourceLoc,
...
CRef<CSeq_loc> CGencollIdMapper::x_Map_Up(const CSeq_loc& SourceLoc,
                                          const CGC_Sequence& Seq,
                                          const SIdSpec& Spec) const
{
...
CRef<CSeq_loc> CGencollIdMapper::x_Map_Down(const CSeq_loc& SourceLoc,
                                            const CGC_Sequence& Seq,
                                            const SIdSpec& Spec) const
{
   	if(!Seq.CanGetStructure())
    ...
    if(Found != m_IdToSeqMap.end()) {
		...
		ITERATE(CDelta_ext::Tdata, SeqIter, DeltaExt.Get()) {
			...
			if(DeltaSeq.IsLoc()) {
				...
				CRange<TSeqPos> GapRange(Start, 
									Start+DeltaSeq.GetLiteral().GetLength()-1);
				Start += DeltaSeq.GetLiteral().GetLength();
				CRange<TSeqPos> Intersect;
				Intersect = LocRange.IntersectionWith(GapRange);
				if(Intersect.Empty()) {
					continue;
				} else if(Intersect == GapRange) {
					return e_Spans;
				} else if(Intersect == LocRange) {
					return e_Contained;
				} else {
					return e_Overlaps;
				}
		...
		CRange<TSeqPos> Intersect;
		Intersect = LocRange.IntersectionWith(ExtendRange);
