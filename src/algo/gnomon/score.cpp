...
/*
void CAlignVec::GetScore(const CGnomonEngine& engine, bool uselims)
{
    const CTerminal& acceptor    = *engine.m_acceptor;
    const CTerminal& donor       = *engine.m_donor;
    const CTerminal& stt         = *engine.m_start;
    const CTerminal& stp         = *engine.m_stop;
    const CCodingRegion& cdr     = *engine.m_cdr;
    const CNonCodingRegion& ncdr = *engine.m_ncdr;

    int len = engine.m_seq.size();
    int num = size();
    EStrand strand = Strand();
    const vector<CAlignExon>& exons = *this;

    const CDoubleStrandSeq& ds = *engine.m_ds;
    CEResidueVec cds, cds_extra;
    TIVec cdsmap;
    GetSequence(ds[ePlus], cds, &cdsmap);
    ...
    else
    {
        ...
        {
            if(int(CdsLimits().GetFrom()) == cdsmap[k]) lim_start = k;  
            if(int(CdsLimits().GetTo()) == cdsmap[k]) lim_stop = k;
        }
    }
    ...
    if(Type() == eProt)
    {
        ...
        if((CdsLimits().NotEmpty() && Strand() == ePlus && CdsLimits().GetTo() != MaxCdsLimits().GetTo()) ||
           (CdsLimits().NotEmpty() && Strand() == eMinus && CdsLimits().GetFrom() != MaxCdsLimits().GetFrom())) 
        {
    ...
    if(strand == ePlus)
    {
        ...
        for(int i = 1; i < num; ++i)
        {
            shift += FShiftedLen(exons[i-1].GetFrom(),exons[i-1].GetTo());
    ...
    for(int frame = 0; frame < 3; ++frame)
    {
        for(int i = (int)starts[frame].size()-1; i >= 0; --i)
        {
            ...
            if(s > Score())
            {
                ...
                if(strand == ePlus) SetMaxCdsLimits(TSignedSeqRange(cdsmap[utr_start],cdsmap[utr_stop]));
                else                SetMaxCdsLimits(TSignedSeqRange(cdsmap[utr_stop],cdsmap[utr_start]));

                SetScore(s);
                
                while(cdsmap[start] < 0 && start < (int)cds.size()-1) ++start;  // in case they point to deletion  
                while(cdsmap[stop] < 0 && stop > 0) --stop; 
                if(strand == ePlus) SetCdsLimits(TSignedSeqRange(cdsmap[start],cdsmap[stop]));
                else                SetCdsLimits(TSignedSeqRange(cdsmap[stop],cdsmap[start]));
            }
        }
    }
}
