...
void MarkovChain<0>::InitScore(CNcbiIfstream& from)
{
    Init(from);
    if (from) {
        toScore();
...
void MarkovChain<0>::Init(CNcbiIfstream& from)
{
    from >> score[nA];
    from >> score[nC];
    from >> score[nG];
    ...
    double sum = score[nA]+score[nC]+score[nG]+score[nT];
    if (sum != 0) {
        score[nA] /= sum;
        score[nC] /= sum;
        score[nG] /= sum;
...
pair<int,int> InputModel::FindContent(CNcbiIfstream& from, const string& label, int cgcontent)
{
    string str;
    while (from >> str) {
        int low,high;
        if (str == label) {
            ...
            if ( cgcontent >= low  &&  cgcontent < high) {
...
double MDD_Donor::Score(const IVec& seq, int i) const
{
    int first = i - left + 1;
    int last = i + right;
      ...
      contig(cntg)
{
    ...
    for (CClusterSet::ConstIt it_cls = cls.begin();  it_cls != cls.end();  ++it_cls) {
        ...
        for (CCluster::ConstIt it = it_cls->begin();  it != it_cls->end();  ++it) {
            ...
            for (int k = 0;  k < it->size();  ++k) {
                ...
                for (int i = a;  i <= b;  ++i) {
                    // ignoring repeats in alignmnets
                    laststop[Plus ][0][i] = -1;
                    laststop[Plus ][1][i] = -1;
                    laststop[Plus ][2][i] = -1;
    ...
    for (CClusterSet::ConstIt it_cls = cls_local.begin();  it_cls != cls_local.end();  ++it_cls) {
        for (CCluster::ConstIt it = it_cls->begin();  it != it_cls->end();  ++it) {
            ...
            if (type != AlignVec::Prot) {
                if (strand == Plus) {
                    // accept NONCONSENSUS alignment splices
                ...
                for (int k = 0;  k < algn.size();  ++k) {
                    ...
                    if (strand == Plus  &&  k == algn.size() - 1  &&  isStop(exonb + 1,strand)) {
                        exona = pnt;
            ...
            for (int i = a;  i <= b;  ++i) {
                ...
                for (int frame = 0;  frame < 3;  ++frame) {
                    notinexon[otherstrand][frame][i] = i;
                }
            }

            if (strand == Plus) {
                a += 3;
            }    // start now is a part of intergenic!!!!
            else {
                b -= 3;
            }
    ...
    if (leftwall) {
        notinintron[Plus][0] = notinintron[Minus][0] = 0;
    }
    if (rightwall) {
        notinintron[Plus][len - 1] = notinintron[Minus][len - 1] = len - 1;
    }
    ...
    for (int strand = 0;  strand < 2;  ++strand) {
        ...
        if (strand == Plus) {
            ...
            for (int i = 0;  i < len;  ++i) {
                ...
                if (ascr[strand][i] != BadScore) {
                    if (s[ii + 1] == nA  &&  s[ii + 2] == nA) {
                        asplit[strand][0][i] |= stpT;
                    }
                    if (s[ii + 1] == nA  &&  s[ii + 2] == nG) {
                        asplit[strand][0][i] |= stpT;
                    } 
                    if (s[ii + 1] == nG  &&  s[ii + 2] == nA) {
                        asplit[strand][0][i] |= stpT;
                    }
                    if (s[ii + 1] == nA) {
                        asplit[strand][1][i] |= stpTA|stpTG;
                ...
                if (dscr[strand][i] != BadScore) {
                    ...
                    if (s[ii - 1] == nT  &&  s[ii] == nA) {
                        dsplit[strand][1][i] |= stpTA;
                    }
                    if (s[ii - 1] == nT  &&  s[ii] == nG) {
                        dsplit[strand][1][i] |= stpTG;
                    }
                }
            ...
            if (type == AlignVec::Prot) {
                for (int k = 0;  k < algn.size();  ++k)   // suppressing STOPS in prot align
                {
                    ...
                    for (int i = exona;  i < exonb;  i += 3) {
                        if (strand == Plus) {
                            if (i > 0) {
                                stpscr[strand][i - 1] = BadScore;
                            }  // score on last coding base
                        } else {
                            if (i + 2 < len - 1) {
                                stpscr[strand][i + 2] = BadScore;
                            }  // score on last intergenic(first stop) base
                        }
                    }
                }
            }
    ...
    for (int strand = 0;  strand < 2;  ++strand) {
        ...
        for (int i = 0;  i < len;  ++i) {
            ...
            if (i > 0) {
                ncdrscr[strand][i] += ncdrscr[strand][i - 1];
            }
            ...
            if (i > 0) {
                ingscr[strand][i] += ingscr[strand][i - 1];
            }
        }
        ...
        for (int frame = 0;  frame < 3;  ++frame) {
            for (int i = 0;  i < len;  ++i)
            {
                int codonshift,ii;
                if (strand == Plus) {
                    // left end of codon is shifted by frame bases to left
                    codonshift = (frame + i)%3;
                    ii = i;
                } else {
                    // right end of codon is shifted by frame bases to right
                    codonshift = (frame - i)%3;
                    if (codonshift < 0) {
                        codonshift += 3;
                    }
