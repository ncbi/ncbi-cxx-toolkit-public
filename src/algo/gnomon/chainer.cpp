...
private:
    ...
    typedef set<pair<Int8,string>> TIDs;
	...
	typedef vector<STGRInfo> TGRInfoVec;
    typedef map<int,TGRInfoVec> TGRInfoByStrand;
	TGRInfoByStrand tgr_info;
...
struct SChainMember
{
    SChainMember() :
		...
		m_trusted_group(0), m_left_trusted_group(0),  m_right_trusted_group(0), m_cds_from_trusted(false), m_excluded_readthrough(false) {}
...
void CChainer::CChainerImpl::FindOverlappingWithTrusted(CChainMembers& pointers) {
	for(SChainMember* p : pointers) {
        SChainMember& mbr = *p;
        CGeneModel& align = *mbr.m_align;
		//		if(mbr.m_type != eCDS || align.TrustedGroup() != 0)
		if(mbr.m_type != eCDS)
			continue;

        int strand = align.Strand();
		if(!tgr_info.count(strand))
			continue;

		const TGRInfoVec& tinfo = tgr_info[strand];
		auto first = lower_bound(tinfo.begin(), tinfo.end(), align.Limits().GetFrom(),
								 [](const STGRInfo& ti, TSignedSeqPos p) { return ti.m_limits.GetTo() < p; }); 
		auto second = upper_bound(tinfo.begin(), tinfo.end(), align.Limits().GetTo(),
								  [](TSignedSeqPos p, const STGRInfo& ti) { return p < ti.m_limits.GetFrom(); });
		if(first == second)
			continue;

		// align overlaps with at least one trusted group

		TSignedSeqRange acds = mbr.m_cds_info->Cds();
		TIVec acds_map;
		if(acds.NotEmpty()) {
        ...
        int count = 0;
        bool paralogs = false;
        set<string> prev_accessions;
		for(auto it = first; it != second; ++it) {
			TSignedSeqRange cds = it->m_cds_limits;
			if(!acds.IntersectingWith(cds))
				continue;

			int tgr = it-tinfo.begin()+1;
			if(strand == eMinus)
				tgr = -tgr;

			int atgr = 0;
			for(const TIVec& tcds_map : it->m_cds_maps) {
				int long_enough = 10;
			...
			if(atgr == tgr) {
				...
				for(auto& id : it->m_ids)
					prev_accessions.insert(id.second);
				if(num+it->m_ids.size() != prev_accessions.size()) // overlapping ids
					paralogs = true;
				++count;
				mbr.m_trusted_group = tgr;                        // shares CDS
			} else if(tgr == align.TrustedGroup()) {
				++count;
			}
		}
        if(count > 1) {
            ...
            if(align.Type()&CGeneModel::eSR)
                ...
                flag = "Prot";
            cerr << "Chimeric chain member " << align.ID() << " " << (paralogs ? "paralogs" : "unrelated") << " " << flag << endl;
			mbr.m_marked_for_deletion = true;
			if(mbr.m_copy !=nullptr) {
				for(auto p : *mbr.m_copy)
...
void CChainer::CChainerImpl::LRIinit(SChainMember& mi, const TContained& micontained) {
	...
	if(mi.m_trusted_group != 0) { // shares CDS with trusted
		tgr = mi.m_trusted_group;	
		mi.m_cds_from_trusted = true;
	} else if(align.TrustedGroup() != 0) {
		tgr = align.TrustedGroup();
		mi.m_cds_from_trusted = (mi.m_cds_info->Cds()&align.TrustedCds()).NotEmpty();
	}			
...
void CChainer::CChainerImpl::RightLeft(TContained& pointers)
{
    ...
    NON_CONST_ITERATE(TContained, i, pointers) {
		...
		mi.m_right_trusted_group = 0;
		mi.m_cds_from_trusted = false;
		if(mi.m_trusted_group != 0) { // shares CDS with trusted
			mi.m_right_trusted_group = mi.m_trusted_group;
			mi.m_cds_from_trusted = true;
		} else if(ai.TrustedGroup() != 0) {
			mi.m_right_trusted_group = ai.TrustedGroup();
			mi.m_cds_from_trusted = (mi.m_cds_info->Cds()&ai.TrustedCds()).NotEmpty();
		}
...
void CChainer::CChainerImpl::CDNACounts(TGeneModelList& clust) {
...
TGeneModelList CChainer::CChainerImpl::MakeChains(TGeneModelList& clust)
{
    ...
    LOG_POST("Find overlapping with trusted in " << sw.Elapsed());
    sw.Restart();
...
void CChain::CalculateSupportAndWeightFromMembers(bool keep_all_evidence) {
    ...
    for(SChainMember* mi : m_members) {
        ...
        if(ai->Status()&CGeneModel::eRightFlexible) {
            ...
            if(ai->Exons().front().GetTo() < m_supported_range.GetFrom() || ai->Exons().back().GetFrom() > m_supported_range.GetTo())  // some exons clipped
                not_wanted = true;
        }
    ...
    for(SLinker& sl : linkers) {
        ...
        if(mi->m_unmd_align) {
			...
			bool all_introns_included = true;
            for(int i = 1; all_introns_included && i < (int)align.Exons().size(); ++i) {
                if(align.Exons()[i-1].m_ssplice && align.Exons()[i].m_fsplice)
                    all_introns_included = chain_introns.count(TSignedSeqRange(align.Exons()[i-1].GetTo(),align.Exons()[i].GetFrom()));
            }
            if(!all_introns_included) {   // intron was clipped by UTR clip
                sl.m_not_wanted = true;
	...
	set<SLinker*> sp_core;
    for(SLinker* slp = best_right; slp != 0; slp = slp->m_left) {
        SChainMember* mi = slp->m_member;
		if(mi == nullptr)
			continue;
        CGeneModel& align = *mi->m_align;
		if(!(align.Status()&(CGeneModel::eLeftFlexible|CGeneModel::eRightFlexible)))
    ...
    for(SLinker& sl : linkers) {
		...
		if(keep) {
			...
			if(id != m_gapped_helper_align.ID()) {                    // ghelper is already included
				...
				if(!core && !support.count(CSupportInfo(id, true)))   // include if no core part
					support.emplace(id, false);
			}
		} else {
			Status() |= CGeneModel::eChangedByFilter;
		}
	...
	ReplaceSupport(support);
	if(m_gapped_helper_align.ID() == 0 && (sp_core.empty() || support.empty())) {
		Status() |= CGeneModel::eSkipped;
		AddComment("Represented by fexibles only");
	}
...
void CChainer::CChainerImpl::FilterOutChimeras(TGeneModelList& clust)
{
    ...
    for(auto& strand_vec :  trusted_aligns) {
		...
		for(const TGeneModelCluster& cls : clset) {
			...
			auto& last = tgr_info[strand].back();
			last.m_limits = cls.Limits();
			for(const CGeneModel& talign : cls) {
				//splices
				for(auto& e : talign.Exons()) {
                    if(e.m_fsplice)
                        last.m_splices.insert(e.GetFrom());
                    if(e.m_ssplice)
                        last.m_splices.insert(e.GetTo());                       
                }
				//ids
                for(auto& cref : talign.TrustedProt())
					last.m_ids.emplace(talign.ID(),CIdHandler::ToString(*cref)); 
                for(auto& cref : talign.TrustedmRNA())
					last.m_ids.emplace(talign.ID(),CIdHandler::ToString(*cref));
				//cds and cds_maps
				TSignedSeqRange tcds = talign.GetCdsInfo().Cds();
				if(tcds.NotEmpty()) {
					last.m_cds_limits += tcds;
					CAlignMap tmap(talign.Exons(), talign.FrameShifts(), talign.Strand(), tcds);
					last.m_cds_maps.emplace_back(tmap.FShiftedLen(tcds)/3,0);
					for(auto& e : talign.Exons()) {
						auto overlap = (e.Limits()&tcds);
						for(TSignedSeqPos k = overlap.GetFrom(); k <= overlap.GetTo(); ++k) {
							TSignedSeqPos p =  tmap.MapOrigToEdited(k);
							if(p >= 0 && p%3 == 0) {
								_ASSERT(p/3 < (int)last.m_cds_maps.back().size());
								last.m_cds_maps.back()[p/3] = k;
							}
						}
					}
				}
    ...
    for(auto& ts : tgr_info) {
		...
		for(int i = 0; i < (int)spl.size(); ++i) {
			...
			if(strand == '-')
				tgr = -tgr;
			trusted_group_cds[tgr] = spl[i].m_cds_limits;
			for(auto& id : spl[i].m_ids)
				cerr << "TGR: " << strand << " " << tgr << " " << id.first << " " << id.second << endl;	
		}
    }
    ...
    for(TGeneModelList::iterator it_loop = clust.begin(); it_loop != clust.end(); ) {
		...
		for(auto it = first; it != second; ++it) {
            ...
            for(unsigned int i = 0; i < align.Exons().size() && atgr != tgr; ++i) {
				...
				if((e.m_fsplice && it->m_splices.count(e.GetFrom())) || (e.m_ssplice && it->m_splices.count(e.GetTo())))
					atgr = tgr;
            }
			TSignedSeqRange cds = it->m_cds_limits;
