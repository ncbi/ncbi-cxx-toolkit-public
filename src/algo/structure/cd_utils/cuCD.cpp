...
CRef< COrg_ref > GetCommonTax(CCdCore* cd, bool useRootWhenNoTaxInfo)
{
	...
	for (int i = 0; i < num; i++)
	{
		...
		if (taxid > ZERO_TAX_ID)
		{
			if (comTax == ZERO_TAX_ID)
				comTax = taxid;
			else
			{
				TTaxId joined = taxServer.Join(comTax, taxid);
				if (joined == ZERO_TAX_ID)
				{
		...
		if (comTax == TAX_ID_CONST(1)) //reach root
			break;
	}
	...
	if (comTax == ZERO_TAX_ID && useRootWhenNoTaxInfo)
		comTax = TAX_ID_CONST(1);
