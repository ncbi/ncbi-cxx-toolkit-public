        ...
        switch(it->m_type) {
            ...
            case eSP:
                ...
                if( (ali.cnseq->GetNuc(nulpos) == nG) && (ali.cnseq->GetNuc(nulpos+1) == nT) 
                    ...
                    score -= CScoring::sm_ICAT;
                } else score -= CScoring::sm_ICANY;
...
int CAliUtil::CountIScore(const CAli& ali, int mode)
{
    //    CScoring::Init();
    CheckValidity(ali);
    CNSeq tseq;
    ...
    //intron score
    int score = CutIntrons(tali, ali);
    return score + CountFrIScore(tali, mode);
}

//same as before, version without introns
//CHECKS THAT THERE ARE NO INTRONS 
int CAliUtil::CountFrIScore(const CAli& ali, int mode, bool lgap, bool rgap)
{
    //    CScoring::Init();
    CheckValidity(ali);
        ...
        switch (it->m_type) {
            case eVP :
                if( !first || mode == 0 || mode == 1 || lgap) {
                    score -= CScoring::sm_Ine * it->m_len;
                    if(it->m_len%3) score -= CScoring::sm_If;
                    else score -= CScoring::sm_Ig;
                }
                nultripos += it->m_len;
                break;
            case eHP :
    ...
    if(ali.m_ps.size() > 0) {//adjust score of the last gap
        if( (mode != 0 && ali.m_ps.back().m_type == eHP) ||
            (!rgap && mode == 2 && ali.m_ps.back().m_type == eVP ) ) {
                score += ali.m_ps.back().m_len * CScoring::sm_Ine;
                    if(ali.m_ps.back().m_len%3) score += CScoring::sm_If;
                    else score += CScoring::sm_Ig;
        }
    }
    return score;
