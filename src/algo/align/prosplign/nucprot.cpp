...
void CInfo::InitAlign()
{
    //int width, bool info_only    
  vector<char>& gpseq = m_Gpseq;
  vector<bool>& gnseq = m_Gnseq;
  const CAli& ali(m_Ali);

  //init gpseq and gnseq
  vector<CAliPiece>::const_iterator iit, eit, it = ali.m_ps.begin();
  for(;it != ali.m_ps.end(); ++it) {
      if(it->m_type == eVP) gnseq.insert(gnseq.end(), it->m_len, false);
      else gnseq.insert(gnseq.end(), it->m_len, true);
  }

  bool first_eHP = true;
  for(it = ali.m_ps.begin() ;it != ali.m_ps.end(); ) {
      if(it->m_type == eSP) {
          gpseq.insert(gpseq.end(), it->m_len, 'i');
          ++it;
      } else if(it->m_type == eHP) {      
          eit = it;
          int len = 0;
          do {
              if(eit->m_type == eHP) len += eit->m_len;
              ++eit;              
          } while(eit != ali.m_ps.end() && ( eit->m_type ==  eHP ||  eit->m_type ==  eSP ) );
          //show translation?
          if(first_eHP || len%3 || eit == ali.m_ps.end()) {//frameshift or flanking gap, no translation
              for(iit = it; iit != eit; ++iit) {
                        if(iit->m_type == eSP) gpseq.insert(gpseq.end(), iit->m_len, 'i');
                        else gpseq.insert(gpseq.end(), iit->m_len, 'g');
              }
          } else {//gap translation 3t or 3T
            int frame = 0;
            EAliPieceType prev_mode;
            for(iit = it; iit != eit; ++iit) {
              if(iit->m_type == eSP) {
                  gpseq.insert(gpseq.end(), iit->m_len, 'i');
                  prev_mode = eSP;
              } else {
                  int rest = iit->m_len;
                  //end of aa over splice
                  if(frame && rest) {
                      if(prev_mode == eSP) gpseq.push_back('t');
                      else gpseq.push_back('T');
                      rest--;
                      frame++;
                      if(frame == 3) frame = 0;
                  }
                  ...
                  if(rest) {
                  //0 frame here
                    if(frame != 0) throw runtime_error("frame counting error");
                    //unspliced
                    int aabasenum = (rest/3)*3;
                    if(aabasenum) gpseq.insert(gpseq.end(), aabasenum, 'T');
                    //end piece
                    frame = rest - aabasenum;
                    if(frame) {
                        vector<CAliPiece>::const_iterator iit1 = iit;
                        iit1++;
                        if(iit1 != ali.m_ps.end() && iit1->m_type !=  eSP) {
                            gpseq.insert(gpseq.end(), frame, 'T');
                        } else {//splice next, over splice
                            gpseq.insert(gpseq.end(), frame, 't');
                        }
                    }
                  }
                  prev_mode = iit->m_type;
              }
            }
          }
          it = eit;
          ...
          int len = 0;
          do {
              if( eit->m_type ==  eVP || eit->m_type ==  eMP ) len += eit->m_len;
              ++eit;             
          } while(eit != ali.m_ps.end() && ( eit->m_type ==  eVP || eit->m_type ==  eMP || eit->m_type ==  eSP ) );
          if(len%3) throw runtime_error("wrong alignment structure \"CAli\"");
          int frame = 0;
          EAliPieceType prev_mode = eMP;//means not eSP
          for(iit = it; iit != eit; ) {
              if(iit->m_type == eSP) {
                  gpseq.insert(gpseq.end(), iit->m_len, 'i');
                  prev_mode = eSP;
                  ...
                  do {
                      rest += iit->m_len;
                      ++iit;
                  } while(iit != eit && ( iit->m_type ==  eVP || iit->m_type ==  eMP ) );
                  //end of aa over splice
                  if(frame && rest) {
                      if(prev_mode == eSP) gpseq.push_back('l');
                      else gpseq.push_back('u');
                      rest--;
                      frame++;
                      if(frame == 3) frame = 0;
                  }
                  ...
                  //0 frame here
                    ...
                    //unspliced
                    int aabasenum = (rest/3)*3;
                    if(aabasenum) gpseq.insert(gpseq.end(), aabasenum, 'u');
                    //end piece
                    frame = rest - aabasenum;
                    if(frame) {
                        if(iit != ali.m_ps.end() && iit->m_type !=  eSP) {//iit points to next at the point
                            gpseq.insert(gpseq.end(), frame, 'u');
                        } else {//splice next, over splice
                            gpseq.insert(gpseq.end(), frame, 'l');
                        }
                    }
                  }
                  prev_mode = eMP;//means not eSP
              }
          }
          it = eit;
      }   
  ...
  PSEQ& pseq = ali.cpseq->seq;
  CNSeq& nseq = *ali.cnseq;

  //inf<<"score: "<<ali.score<<endl;
    ...
    //prot string
    int i=0, l=0, gl=0, u=0;
    string::size_type tranp1, tranp2;
    string::size_type gtranp1, gtranp2;
    int splnuc1, splnuc2;
    int gsplnuc1, gsplnuc2;
    for(vector<char>::iterator rit = gpseq.begin(); rit != gpseq.end(); ++rit) {
        int len;
        switch (*rit) {
            case 'u':
                u++;
            ...
            case 't':
            case 'T':
                //handle over-splice translation
              {
                gl++;
                outp += "-";
    ...
    */
    ...
    for(string::size_type sn = 0; sn < outp.size(); ++sn) {
        ...
        if(matrix.matrix[outp[sn]][outr[sn]] > 0) {
            ...
            else if(islower(outp[sn]))  match[sn] = po;
        }
    }
}

// void CInfo::PrintAlign(ostream& out, int width, bool info_only)
// {
//     InitAlign();
...
//     if(!info_only) {
//         string show_good;
//         show_good.resize(outp.size(), ' ');
//         // good piece string
//         for(list<CNPiece>::iterator it = m_AliPiece.begin(); it != m_AliPiece.end(); ++it) {
//             for(int ti = it->beg; ti < it->end; ++ti) {
//                 show_good[ti] = '*';
//             }
//         }

//         int prot_beg_pos = ProtBegOnAliCoord();
//         int prot_end_pos = ProtEndOnAliCoord();

//         for(i=0;i<(int)outn.length(); i+=width) {
//             int apos = i+width-1;
//             if(apos >= (int)outn.length()) apos = (int)outn.length() - 1;
//             int npos2 = ncoor[apos];
//             int npos1 = NextNucNullBased(i-1) + 1;
//             //throw out head and tail lines with a complete row gap
//             if(apos < prot_beg_pos) continue;
//             if(i >= prot_end_pos) break;
//             //int ppos2 = pcoor[apos];
//             //int ppos1 = pcoor[i];//problem if it is inside splice, to be implemented
//             if(npos1 <= npos2) out<<left<<setw(12)<<ali.NucPosOut(npos1)<</*setw(width)<<*/outn.substr(i, width)<<"   "<<ali.NucPosOut(npos2)<<endl;
//             else { //complete row is a nucleotide gap
//                 out<<left<<setw(12)<<"-"<</*setw(width)<<*/outn.substr(i, width)<<"   "<<"-"<<endl;
//             }
//             out<<setw(12)<<" "<<outr.substr(i, width)<<endl;
//             out<<setw(12)<<" "<<show_match.substr(i, width)<<endl;
//             out<<setw(12)<<" "<</*setw(width)<<*/outp.substr(i, width)/*<<"   "<<ppos2*/<<endl;
//             out<<setw(12)<<" "<<show_good.substr(i, width)<<endl<<endl;
//             //out<<setw(12)<<" "<<match.substr(i, width)<<endl<<endl;
//         }
//     }
// }
