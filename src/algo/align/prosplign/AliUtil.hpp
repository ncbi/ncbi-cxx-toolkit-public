...
//mode 0 : full scoring at the end
    static int CountIScore(const CAli& ali, int mode = 2);
    ...
    static int CountFrIScore(const CAli& ali, int mode = 2, bool lgap = false, bool rgap = false);

private:
    //produces new alignment with intron cut out, returns intron 'IScore'
    static int CutIntrons(CAli& new_ali, const CAli& ali);
