...
public:
  ...
  CCompartmentFinder(vector<CHit>::const_iterator start,
                     vector<CHit>::const_iterator finish);

  // setters and getters
  void SetIntronLimits(size_t intr_min, size_t intr_max) {
    m_intron_min = intr_min;
    m_intron_max = intr_max;
  }

  static size_t GetDefaultIntronMin(void) {
    return 0;
  }

  static size_t GetDefaultIntronMax(void) {
    return 750000;
  }
  ...
  public:
    ...
    const vector<const CHit*>& GetMembers(void) {
      return m_members;
    }
    ...
    friend bool PLowerSubj(const CCompartment& c1, const CCompartment& c2) {
...
private:
  ...
  size_t                m_intron_min, m_intron_max; // intron limits
  size_t                m_penalty;                  // penalty per compartment
  size_t                m_min_coverage;

  vector<const CHit*>   m_hits;         // input hits
...
public:
  ...
  CCompartmentAccessor(vector<CHit>::iterator start,
                       vector<CHit>::iterator finish,
                       size_t comp_penalty_bps,
                       size_t min_coverage);
  
  bool GetFirst(vector<CHit>& compartment);
  bool GetNext(vector<CHit>& compartment);

  size_t GetCount(void) const {
    return m_pending.size();
  }

  void Get(size_t i, vector<CHit>& compartment) const {
