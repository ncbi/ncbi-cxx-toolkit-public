...
{
    static CSafeStatic<CVersionAPI> s_Version(s_CreateVersion, 0);
...
{    
    ...
    try {
        ...
        if(phitrefs->size() > 1) {     
            ...
            //left      
            if(hitmin > smin) {
                CSeq_loc tmp_loc(*tmp_id, smin, hitmin - 1, eNa_strand_plus);
            ...
            //right     
            if(smax > hitmax) {
                CSeq_loc tmp_loc(*tmp_id, hitmax + 1, smax, eNa_strand_plus);
                CConstRef<CSeqMap> smap = CSeqMap::GetSeqMapForSeq_loc(tmp_loc, GetScope());
                if(smap) {
                                                  ...
                                                  EStatFlags flags)
{
    ...
    if(flags & eSF_BasicNonCds) {
       CSeq_align::TScore score_vec;
       CScoreBuilderBase().AddSplignScores(*sa, score_vec);
