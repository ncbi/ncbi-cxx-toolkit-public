    ...
    const TMemberIndex pos /*= kInvalidMember*/)
{
    ...
    if (!id.Empty()) {
        ...
        if (pos != kInvalidMember) {
            idx = info.Find(CLightString(id_alt),pos);
        } else {
            idx = info.Find(CLightString(id_alt));
        }
