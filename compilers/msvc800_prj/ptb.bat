...
if "%PREBUILT_PTB_EXE%"=="" (
  if "%PTB_PLATFORM%"=="x64" (
    set PREBUILT_PTB_EXE=\\snowman\win-coremake\App\Ncbi\cppcore\ptb\msvc8.64\project_tree_builder.exe
  ) else (
    set PREBUILT_PTB_EXE=\\snowman\win-coremake\App\Ncbi\cppcore\ptb\msvc8\project_tree_builder.exe
  )
) else (
