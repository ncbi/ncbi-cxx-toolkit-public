...
for i in 'Debug' 'Release' ; do
  for j in '' 'DLL' ; do
    if test -d "$builddir"/compilers/$compiler/lib/$i$j ; then
      makedir "$target/$i$j" -p
      cd "$builddir"/compilers/$compiler/lib/$i$j
      cp -p *.lib "$target/$i$j"
    fi
...
makedir "$bindir" -p
for i in 'DLL' '' ; do
  if test -d "$builddir"/compilers/$compiler/bin/Release$i ; then
    if ls "$builddir"/compilers/$compiler/bin/Release$i/*.exe >/dev/null 2>&1 ; then
      cd "$builddir"/compilers/$compiler/bin/Release$i
