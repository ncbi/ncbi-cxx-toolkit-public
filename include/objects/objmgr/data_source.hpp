...
private:
    ...
    TBioseqCore GetBioseqCore(const CBioseqHandle& handle);
    ...
    /// Get iterator to traverse descriptor sets 
    CDesc_CI BeginDescr(const CBioseqHandle& handle);

    /// Get iterator to traverse annotations
    CAnnot_CI BeginAnnot(CHandleRangeMap& loc, SAnnotSelector selector);
