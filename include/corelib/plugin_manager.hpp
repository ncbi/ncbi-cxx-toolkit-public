    ...
    NStr::ECase                    use_case)
{
    ...
    ITERATE ( list<string>, it, drivers ) {
        ...
        const TPluginManagerParamTree* driver_params = params ?
            params->FindNode(drv_name, use_case) : 0;
        try {
    ...
    NStr::ECase                    use_case)
{
    ...
    const TPluginManagerParamTree* driver_node = params->FindNode(driver_key, use_case);
    if ( !driver_node ) {
        return drv;
    }
    string driver_name = driver_node->GetValue().value;
    // Driver name may contain a list of drivers
    drv = CreateInstanceFromList(params, driver_name, version, use_case);
    return drv;
}
