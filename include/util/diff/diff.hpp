...
///
/// CDiff          - The class to diff two texts and finds the differences.
/// CDiffList      - The list of differences (diff operations).
/// CDiffOperation - The storage class for one diff operation.
/// CDiffException - Define exceptions generated by Diff API.
///
...
public:
    ...
    enum EType {
        eDelete,  ///< String that exist in old text, and deleted in new one
        eEqual,   ///< String exists in both texts
        eInsert   ///< String that exist in new text, and deleted in old one
    };
    ...
    struct TPos {
        ...
        TPos(size_type p1 = NPOS, size_type p2 = NPOS) : first(p1), second(p2) {}
    };
    ...
    const CTempString& GetStr() const { return m_String; }

    /// Get string length.
    ///
    /// For block-based diffs it is the same as GetStr().length().
    /// But in the line-based diffs the string can be truncated
    /// and do not have ending EOL. So, the original string can be
    /// reconstructed using:
    ///   CTempString(op.GetStr().data(), op.GetLength())
    /// 
    size_type GetLength() const { return m_Length; }

    /// Get offset of the current difference in the original text.
    ///
    /// GetOffset() return structure with offsets in the first (original)
    /// and in the second (result) strings.
    ///
    /// For performance reasons, Diff() do not operate with offsets
    /// of the substrings and do not store them, but as original strings
    ...
    ///    - GetOffset().first represent an offsets for DELETE and EQUAL
    ///      type operations only, GetOffset().second for INSERT and EQUAL.
    /// @sa
    ///   GetOperation(), GetStr(), GetLine(),
    ///   CDiff::Diff(), CDiffList::CalculateOffsets()
    TPos GetOffset() const;
    ...
    ///      type operations only, GetLine().second for INSERT and EQUAL,
    ///      or NPOS otherwise.
    /// @sa
    ///   GetOperation(), GetStr(), GetOffset(), CDiff::DiffLines()
    TPos GetLine(void) const;
    ...
    operator string(void) const { return m_String; }

protected:
    // Setters
    void SetOperation(EType operation);
    void SetStr(CTempString str, size_type len = NPOS);
    void SetLength(size_type length);
    void SetOffset(TPos offset);
    void SetLine(TPos line);
...
private:
    ...
    TPos m_Offset;  ///< The offsets of the current string in the original string (or NPOS)
    TPos m_Line;    ///< Line numbers of the current diff (or NPOS) -- line-mode only
};
...
public:
    ...
    ///   common line.
    const CTempString& GetLongestCommonSubstring(void) const;
...
private:
    ...
    friend class CDiff;
    /// Empty string for the sake of GetLongestCommonSubstring()
    CTempString m_kEmptyStr; 
};


/////////////////////////////////////////////////////////////////////////////
///
/// CDiff -- Diff and compare texts
///
/// Throw exception on error.

class NCBI_XUTIL_EXPORT CDiff
{
public:
    /// Type definition
    typedef CDiffOperation::size_type size_type;

    /// Diff processing flags.
    /// If some flag is not supported by some Diff() method, it will be ignored.
    enum EFlags {
        /// Automatically call CleanupAndMerge() for the generated list
        /// of differences after Diff*() to have a shorter and cleaner list
        /// of results. It is on by default for regular non-line-based diff.
        ...
        /// which can be obtained using CDiffOperation::GetStr().
        /// This flag is incompatible with fCleanup, exception will be
        /// thrown if they where used together. The fRemoveEOL affect a visual
        /// appearance of the string, and do not have an influence on how
        /// lines with different kind of EOLs compares, use fIgnoreEOL
        /// in addition if necessary.
        /// (line-mode only)
        fRemoveEOL        = 1 << 3,
        /// Default for Diff()
        fDiffDefault      = fCleanup,
        /// Default for DiffLines() -- non-optimal line-based diff that have all EOLs
        fDiffLinesDefault = 0,
    };
    ...
    /// @sa SetTimeout(), DiffLines()
    CDiffList& Diff(CTempString s1, CTempString s2, TFlags flags = fDiffDefault);

    /// Find the differences between two texts (line mode).
    ///
    /// Run a shorter version of line-based diff, that operates with whole
    /// lines, and not trying to find a differences inside added/deleted
    /// blocks of lines. 
    /// @param s1 
    ...
    /// @sa SetTimeout(), Diff()
    CDiffList& DiffLines(CTempString s1, CTempString s2, TFlags flags = fDiffLinesDefault);
...
inline
CDiffOperation::TPos CDiffOperation::GetOffset(void) const
{
    return m_Offset;
}

inline
CDiffOperation::TPos CDiffOperation::GetLine(void) const
{
    return m_Line;
}
...
inline
void CDiffOperation::SetStr(CTempString str, size_type len)
{
    m_String = str;
...
inline
void CDiffOperation::SetOffset(TPos offset)
{
    m_Offset = offset;
}

inline
void CDiffOperation::SetLine(TPos line)
{
    m_Line = line;
}
...
inline
bool CDiff::x_IsTimeoutExpired() const
{
    if (!m_Deadline) {
        return false;
    }
