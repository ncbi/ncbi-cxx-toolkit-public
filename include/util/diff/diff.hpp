...
/// @file diff.hpp
///
/// API to diff two texts and to find the differences.
///
/// CDiff          - The class to diff two texts and to find the differences.
...
public:
    ...
    /// Get string length.
    ///
    /// For block-based diffs it is the same as GetString().length().
...
public:
    ...
    typedef int TBlockFlags;  ///< Bitwise OR of "EBlockFlags"

    /// DiffLines() processing flags.
    enum ELineFlags {
        /// Automatically call CleanupAndMerge() for the generated list
        /// of differences after DiffLines() to have a shorter and cleaner
        /// list of results. It is off by default for line-based diff.
        fCleanup          = 1 << 4,
        /// Automatically call CalculateOffests() for the generated list
        /// of differences.
        fCalculateLineOffsets = fCalculateOffsets,
    ...
    typedef int TLineFlags;   ///< Bitwise OR of "ELineFlags"
    ...
    CDiffList& Diff(CTempString s1, CTempString s2, TBlockFlags flags = 0);
    ...
    /// @sa SetTimeout(), Diff()
    CDiffList& DiffLines(CTempString s1, CTempString s2, TLineFlags flags = 0);
