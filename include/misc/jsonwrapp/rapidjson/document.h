...
#else
protected:
	GenericValue() : flags_(kNullFlag), allocator_(0) {
    }
public:
    GenericValue(Allocator* allocator) : flags_(kNullFlag), allocator_(allocator) {
    }
#endif
...
//NCBI: added object method
	SizeType SizeObject() const { RAPIDJSON_ASSERT(IsObject()); return data_.o.size; }
...
//NCBI: added friendship for AssignCopy to work
